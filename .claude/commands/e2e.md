...
test.describe('Student Enrollment Flow', () => {
  ...
  test('should enroll new student successfully', async ({ page }) => {
    ...
    await expect(page.locator('[role="alert"]'))
      .toContainText('Student enrolled successfully');
...
test.describe('Multi-Tenant Student Management', () => {
  test('should isolate data between schools', async ({ browser }) => {
    ...
    await expect(page2.locator('[role="alert"]'))
      .toContainText('Student enrolled successfully');
  });
});
```
...
test.describe('Accessibility', () => {
  test('should be keyboard navigable', async ({ page }) => {
    ...
    await expect(page.locator(':focus')).toHaveAttribute('href', '/students/new');

    await page.keyboard.press('Tab');
    await expect(page.locator(':focus')).toHaveAttribute('placeholder', 'Search students...');
  ...
  test('should work with screen reader', async ({ page }) => {
    ...
    await expect(page.locator('[role="table"]')).toHaveAttribute('aria-label', 'Students list');
    await expect(page.locator('[role="button"]')).toHaveAttribute('aria-label', /Add new student/);
  });
});
```
