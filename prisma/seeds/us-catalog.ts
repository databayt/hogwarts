 ...
 *
 *   ~220 Subjects → ~800 Chapters → ~4000 Lessons
 *
 * Uses upsert with slug for idempotency. Updates denormalized counts.
 * Slug convention: us-g{N}-{subject} (e.g., us-g3-math)
 *
 * Usage: pnpm db:seed:single us-catalog
 */
...
/** Generate grade-specific subject slug: us-g3-math */
function toGradeSubjectSlug(name: string, grade: number): string {
  const nameSlug = name
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, "")
    .replace(/\s+/g, "-")
  return `us-g${grade}-${nameSlug}`
}
...
export async function seedUsCatalog(prisma: PrismaClient): Promise<void> {
  ...
  if (usCurriculum) {
    console.log(`  Linked to Curriculum record: ${usCurriculum.id}`)
  }
  ...
  for (const s of oldSubjects) {
    if (s.slug.match(/^us-.+-grade-\d+$/)) {
      const nameSlug = s.name
        .toLowerCase()
        .replace(/[^a-z0-9\s-]/g, "")
        .replace(/\s+/g, "-")
      const newSlug = `us-g${s.grades[0]}-${nameSlug}`
