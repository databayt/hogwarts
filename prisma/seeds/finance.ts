...
import type { SeedPrisma, StudentRef, TeacherRef, UserRef } from "./types"
...
export async function seedFinance(
  ...
  console.log(
    "💰 Creating finance module (12 months, SDG - Bilingual AR/EN)..."
  )
    ...
    if (!period) {
      period = await prisma.timesheetPeriod.create({
        data: {
          ...
          closedAt: isPast
            ? new Date(endDate.getTime() + 5 * 24 * 60 * 60 * 1000)
            : null,
        },
      })
            ...
            for (let day = 0; day < 4; day++) {
              ...
              if (
                ...
                entries.push({
                  ...
                  approvedAt: new Date(
                    entryDate.getTime() + 2 * 24 * 60 * 60 * 1000
                  ),
                })
              }
            }
    ...
    if (!existingRun) {
      ...
      const run = await prisma.payrollRun.create({
        data: {
          ...
          processedAt: isPaid
            ? new Date(payDate.getTime() - 5 * 24 * 60 * 60 * 1000)
            : null,
          approvedBy: isPaid ? adminUser.id : null,
          approvedAt: isPaid
            ? new Date(payDate.getTime() - 2 * 24 * 60 * 60 * 1000)
            : null,
        },
      })
      ...
      for (const { id: structureId, baseSalary, teacher } of salaryStructures) {
        ...
        const transportAllowance = adjustedBase * 0.1
        ...
        await prisma.salarySlip.create({
          data: {
            ...
            allowances: [
              ...
              ...(eidBonus > 0
                ? [{ name: "مكافأة العيد | Eid Bonus", amount: eidBonus }]
                : []),
            ],
            overtime:
              faker.number.int({ min: 0, max: 8 }) * (adjustedBase / 176), // Occasional overtime
            bonus: eidBonus,
            grossSalary,
            taxAmount,
  ...
  const existingTxCount = await prisma.transaction.count({
    where: { schoolId, bankAccountId: mainBank.id },
  })
  if (existingTxCount < 200) {
    ...
    const transactionTypes = [
      // Income transactions
      {
        ...
        category: "Tuition",
        names: [
          "دفع رسوم دراسية | Tuition Payment",
          "قسط فصلي | Term Installment",
          "تسديد رسوم | Fee Settlement",
        ],
      },
      {
        type: "credit",
        category: "Registration",
        names: ["رسوم تسجيل | Registration Fee", "رسوم قبول | Admission Fee"],
      },
      {
        type: "credit",
        category: "Transport",
        names: [
          "رسوم النقل | Transport Fee",
          "اشتراك الباص | Bus Subscription",
        ],
      },
      {
        type: "credit",
        category: "Activities",
        names: ["رسوم الأنشطة | Activity Fee", "رسوم الرحلات | Trip Fee"],
      },
      // Expense transactions
      {
        ...
        category: "Salaries",
        names: [
          "رواتب المعلمين | Teacher Salaries",
      ...
      },
      {
        ...
        names: [
          ...
          "مستلزمات تنظيف | Cleaning Supplies",
          "مستلزمات صحية | Health Supplies",
        ],
      },
      {
        type: "debit",
      ...
      },
      {
        ...
        names: [
          ...
          "معدات معملية | Lab Equipment",
          "وسائل تعليمية | Teaching Aids",
        ],
      },
      {
        type: "debit",
        category: "Vendor",
        names: [
          "دفع لمورد | Vendor Payment",
          "مستحقات متعهد | Contractor Payment",
        ],
      },
    ]
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await prisma.walletTransaction.create({
        data: {
          ...
          amount,
          balanceAfter:
            30000000 + (type === TransactionType.CREDIT ? amount : -amount),
          description:
            type === TransactionType.CREDIT
              ? "استلام دفعة | Payment received"
              : "معالجة استرداد | Refund processed",
          sourceModule: "fees",
  ...
  if (!existingBudgets) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const categoryId = Array.from(createdCategories.values())[
        i % createdCategories.size
      ]
      const allocated = faker.number.float({
        min: 3000000,
        max: 18000000,
        multipleOf: 0.01,
      }) // SDG
    ...
    for (let i = 0; i < 30; i++) {
      await prisma.expense.create({
        data: {
          ...
          paidAt:
            i < 20
              ? faker.date.between({ from: "2025-07-01", to: new Date() })
              : null,
        },
      })
    }
  ...
  const existingInvoices = await prisma.userInvoice.findFirst({
    where: { schoolId },
  })
  if (!existingInvoices) {
  ...
  const existingReports = await prisma.financialReport.findFirst({
    where: { schoolId },
  })
  if (!existingReports) {
    const reportTypes = [
  ...
  if (existingJournals < 50) {
    ...
    console.log(
      `   ✅ Created ${journalCount} journal entries with ${ledgerCount} ledger entries`
    )
  }

  console.log(
    `   ✅ Created: Chart of accounts, Payroll (12 months), Banking (200+ tx), Budgets, Expenses, Invoices, Ledger\n`
  )
}
