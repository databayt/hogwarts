...
const ASSESSMENT_TYPES = [
  ...
  { titleEn: "Homework Assignment", titleAr: "واجب منزلي", maxScore: 20, frequency: 8 },
  { titleEn: "Weekly Quiz", titleAr: "اختبار أسبوعي", maxScore: 25, frequency: 4 },
  { titleEn: "Class Participation", titleAr: "مشاركة صفية", maxScore: 10, frequency: 2 },
  // Monthly Assessments
  { titleEn: "Monthly Test", titleAr: "اختبار شهري", maxScore: 50, frequency: 2 },
  { titleEn: "Oral Assessment", titleAr: "تقييم شفهي", maxScore: 25, frequency: 2 },
  // Projects & Reports
  { titleEn: "Project Submission", titleAr: "تسليم مشروع", maxScore: 50, frequency: 1 },
  { titleEn: "Lab Report", titleAr: "تقرير معملي", maxScore: 30, frequency: 1 },
  { titleEn: "Research Paper", titleAr: "بحث علمي", maxScore: 40, frequency: 1 },
  { titleEn: "Book Report", titleAr: "تقرير كتاب", maxScore: 30, frequency: 1 },
  { titleEn: "Group Project", titleAr: "مشروع جماعي", maxScore: 50, frequency: 1 },
  // Major Exams
  { titleEn: "Midterm Exam", titleAr: "اختبار نصف الفصل", maxScore: 50, frequency: 1 },
  { titleEn: "Final Exam", titleAr: "الاختبار النهائي", maxScore: 100, frequency: 1 },
  // Subject-Specific
  { titleEn: "Practical Test", titleAr: "اختبار عملي", maxScore: 50, frequency: 1 },
  { titleEn: "Quran Recitation", titleAr: "تلاوة القرآن", maxScore: 25, frequency: 2 },
  { titleEn: "Arabic Dictation", titleAr: "إملاء عربي", maxScore: 20, frequency: 4 },
  { titleEn: "Essay Writing", titleAr: "مقال كتابي", maxScore: 40, frequency: 2 },
  { titleEn: "Math Problem Set", titleAr: "مسائل رياضيات", maxScore: 30, frequency: 4 },
  { titleEn: "Science Experiment", titleAr: "تجربة علمية", maxScore: 35, frequency: 2 },
  { titleEn: "English Composition", titleAr: "إنشاء إنجليزي", maxScore: 30, frequency: 2 },
  { titleEn: "Presentation", titleAr: "عرض تقديمي", maxScore: 25, frequency: 1 },
];
...
const FEEDBACK_TEMPLATES = {
  excellent: [
    ...
    { en: "Consistently brilliant work. Ma sha Allah!", ar: "عمل رائع باستمرار. ما شاء الله!" },
    { en: "Shows leadership in academic excellence.", ar: "يُظهر ريادة في التفوق الأكاديمي." },
  ...
  good: [
    ...
    { en: "Above average performance. Keep pushing.", ar: "أداء فوق المتوسط. استمر." },
    { en: "Good grasp of the material.", ar: "استيعاب جيد للمادة." },
    { en: "Commendable effort. Build on this.", ar: "جهد يستحق الإشادة. ابنِ على هذا." },
  ],
  average: [
    ...
    { en: "Room for improvement. Stay focused.", ar: "هناك مجال للتحسن. ابقَ مركزاً." },
    { en: "Meeting minimum requirements. Aim higher.", ar: "تحقيق الحد الأدنى. اطمح للأعلى." },
    { en: "Adequate work. More effort will show results.", ar: "عمل كافٍ. المزيد من الجهد سيُظهر النتائج." },
  ],
  needsImprovement: [
    ...
    { en: "Please review fundamentals and practice more.", ar: "يرجى مراجعة الأساسيات والممارسة أكثر." },
    { en: "Attend remedial classes for better understanding.", ar: "احضر الحصص التعويضية لفهم أفضل." },
    { en: "Work closely with teacher for improvement.", ar: "اعمل مع المعلم عن قرب للتحسن." },
  ],
};
...
function generateScore(profile: { baseLevel: number; variation: number }, maxScore: number): number {
  const percentage = Math.max(0, Math.min(100,
    profile.baseLevel + faker.number.int({ min: -profile.variation, max: profile.variation })
  ));
...
function getSubjectAssessments(subjectName: string): typeof ASSESSMENT_TYPES {
  const baseAssessments = ASSESSMENT_TYPES.filter(a =>
  ...
  if (lowerName.includes("arabic") || lowerName.includes("عربي")) {
    return [...baseAssessments, ...ASSESSMENT_TYPES.filter(a =>
      a.titleEn.includes("Dictation") || a.titleEn.includes("Essay")
    )];
  }
  if (lowerName.includes("quran") || lowerName.includes("islamic") || lowerName.includes("قرآن") || lowerName.includes("إسلام")) {
    return [...baseAssessments, ...ASSESSMENT_TYPES.filter(a =>
      a.titleEn.includes("Quran") || a.titleEn.includes("Oral")
    )];
  }
  if (lowerName.includes("math") || lowerName.includes("رياضيات")) {
    return [...baseAssessments, ...ASSESSMENT_TYPES.filter(a =>
      a.titleEn.includes("Problem Set")
    )];
  }
  if (lowerName.includes("science") || lowerName.includes("physics") || lowerName.includes("chemistry") || lowerName.includes("biology") ||
      lowerName.includes("علوم") || lowerName.includes("فيزياء") || lowerName.includes("كيمياء") || lowerName.includes("أحياء")) {
    return [...baseAssessments, ...ASSESSMENT_TYPES.filter(a =>
      a.titleEn.includes("Lab") || a.titleEn.includes("Experiment") || a.titleEn.includes("Practical")
    )];
  }
  if (lowerName.includes("english") || lowerName.includes("إنجليزي")) {
    return [...baseAssessments, ...ASSESSMENT_TYPES.filter(a =>
      a.titleEn.includes("Composition") || a.titleEn.includes("Essay")
    )];
  }

  // Default: add projects and presentations
  return [...baseAssessments, ...ASSESSMENT_TYPES.filter(a =>
    a.titleEn.includes("Project") || a.titleEn.includes("Presentation")
  )];
}
...
export async function seedGrades(
  ...
  console.log("📝 Creating academic results (50,000+ records, Bilingual AR/EN)...");
  ...
  if (existingCount >= 45000) {
    console.log(`   ⏭️ Already have ${existingCount.toLocaleString()} results, skipping...\n`);
    return;
  }
  ...
  const subjectNameMap = new Map(subjectDetails.map(s => [s.id, s.subjectName]));
  ...
  const studentProfiles = new Map<string, { baseLevel: number; variation: number }>();
  ...
  for (const classObj of classes) {
    ...
    const subjectName = subjectId ? subjectNameMap.get(subjectId) || "General" : "General";
    ...
    for (const studentId of classStudents) {
      ...
      for (const termInfo of terms) {
        ...
        const isCurrentTerm = termInfo.year === currentYear && termInfo.term === 2;
        const currentMonth = new Date().getMonth() + 1;
        const effectiveEndMonth = isCurrentTerm ? Math.min(currentMonth, termInfo.endMonth) : termInfo.endMonth;
        ...
        for (const assessment of applicableAssessments) {
          ...
          for (let i = 0; i < assessment.frequency; i++) {
            ...
            const gradingMonth = termInfo.startMonth + Math.floor(Math.random() * monthRange);
            const gradingYear = termInfo.term === 2 && termInfo.startMonth === 1 ? termInfo.year :
                               termInfo.term === 1 ? termInfo.year : termInfo.year;
            ...
            if (Math.random() > 0.92 && !assessment.titleEn.includes("Exam")) continue;
            // 97% attendance for exams
            if (Math.random() > 0.97 && assessment.titleEn.includes("Exam")) continue;
  ...
  for (let i = 0; i < results.length; i += BATCH_SIZE) {
    ...
    console.log(`   📊 Inserted batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(results.length / BATCH_SIZE)} (${inserted.count} records)`);
  }
  ...
  console.log(`   ✅ Created: ${totalInserted.toLocaleString()} academic results (of ${results.length.toLocaleString()} generated)`);
  console.log(`      - Coverage: ${students.length} students × ${subjects.length} subjects × 4 terms`);
  console.log(`      - Average percentage: ${avgPercentage}%`);
  console.log(`      - Grade distribution:`);
  console.log(`        A (A+/A/A-): ${aGrades.toLocaleString()} (${Math.round(aGrades/results.length*100)}%)`);
  console.log(`        B (B+/B/B-): ${bGrades.toLocaleString()} (${Math.round(bGrades/results.length*100)}%)`);
  console.log(`        C (C+/C/C-): ${cGrades.toLocaleString()} (${Math.round(cGrades/results.length*100)}%)`);
  console.log(`        D (D+/D):    ${dGrades.toLocaleString()} (${Math.round(dGrades/results.length*100)}%)`);
  console.log(`        F:          ${fGrades.toLocaleString()} (${Math.round(fGrades/results.length*100)}%)\n`);
}
