  ...
  try {
    switch (phase) {
      case "school": {
        ...
        console.log(
          `✅ School created: ${school.name} (domain: ${school.domain})`
        )
        break
      }
      ...
      case "academic": {
        ...
        console.log(
          `   - Year Levels: ${result.yearLevels.length} (KG1 - Grade 12)`
        )
      ...
      case "departments": {
        ...
        console.log(
          `   - Subjects: ${result.subjects.length} (with AR/EN names)`
        )
        break
      }
      ...
      case "people": {
        const school = await getSchool()
        const { schoolYear, yearLevels, departments } = await getCoreData(
          school.id
        )
      ...
      case "classes": {
        const school = await getSchool()
        const { term1, periods, classrooms, subjects, teachers, students } =
          await getCoreData(school.id)
        if (!term1) throw new Error("❌ Run 'academic' phase first")
        const result = await seedClasses(
          prisma,
          school.id,
          term1.id,
          periods,
      ...
      case "finance": {
        ...
        const adminUsers = users.filter((u) =>
          ["DEVELOPER", "ADMIN", "ACCOUNTANT", "STAFF"].includes(u.role)
        )
        await seedFinance(
          prisma,
          school.id,
      ...
      case "exams": {
        const school = await getSchool()
        const { classes, subjects, students, teachers } = await getCoreData(
          school.id
        )
        await seedExams(
          prisma,
      ...
      case "attendance-advanced": {
        ...
        console.log(
          "✅ Advanced attendance created (devices, cards, biometrics)"
        )
        break
      }
