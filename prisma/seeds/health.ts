...
const HEALTH_RECORD_TYPES = [
  {
    ...
    descriptions: [
      {
        en: "Student received annual flu vaccination as part of school health program.",
        ar: "تلقى الطالب التطعيم السنوي ضد الإنفلونزا كجزء من البرنامج الصحي المدرسي.",
      },
      {
        en: "Vaccination administered as per national immunization schedule.",
        ar: "تم إعطاء اللقاح وفقاً لجدول التطعيمات الوطني.",
      },
    ],
    severity: null,
  },
  {
    ...
    descriptions: [
      {
        en: "Routine annual health screening completed. All parameters within normal range.",
        ar: "تم إكمال الفحص الصحي السنوي الروتيني. جميع المؤشرات ضمن المعدل الطبيعي.",
      },
      {
        en: "Student passed vision screening with 20/20 vision.",
        ar: "اجتاز الطالب فحص الرؤية بنظر 20/20.",
      },
    ],
    severity: "Low",
  },
  {
    ...
    descriptions: [
      {
        en: "Student sustained minor scrape during recess. First aid applied.",
        ar: "أصيب الطالب بخدش طفيف أثناء الاستراحة. تم تطبيق الإسعافات الأولية.",
      },
      {
        en: "Minor injury during PE class. Ice pack applied, no further treatment needed.",
        ar: "إصابة طفيفة أثناء حصة التربية البدنية. تم تطبيق كيس ثلج، لا حاجة لعلاج إضافي.",
      },
    ],
    severity: "Low",
  },
  {
    ...
    descriptions: [
      {
        en: "Student sent home with fever. Parent notified and picked up student.",
        ar: "تم إرسال الطالب للمنزل بسبب الحمى. تم إخطار ولي الأمر وجاء لاستلامه.",
      },
      {
        en: "Student complained of stomach ache. Rested in nurse's office, felt better.",
        ar: "اشتكى الطالب من ألم في المعدة. استراح في مكتب الممرضة وتحسنت حالته.",
      },
    ],
    severity: "Medium",
  },
  {
    ...
    descriptions: [
      {
        en: "Parent provided updated allergy information. EpiPen kept in nurse's office.",
        ar: "قدم ولي الأمر معلومات محدثة عن الحساسية. يتم الاحتفاظ بـ EpiPen في مكتب الممرضة.",
      },
      {
        en: "Allergy information updated in student records. Staff notified.",
        ar: "تم تحديث معلومات الحساسية في سجلات الطالب. تم إخطار الموظفين.",
      },
    ],
    severity: "High",
  },
...
const ACHIEVEMENT_TEMPLATES = [
  {
    ...
    titles: [
      ...
      {
        en: "Math Olympiad Participation",
        ar: "المشاركة في أولمبياد الرياضيات",
      },
...
const DISCIPLINARY_TEMPLATES = [
  {
    ...
    actions: [
      {
        en: "Verbal warning given. Student reminded of class rules.",
        ar: "تم إعطاء تحذير شفهي. تم تذكير الطالب بقواعد الصف.",
      },
      {
        en: "Written warning sent home. Parent signature required.",
        ar: "تم إرسال تحذير مكتوب للمنزل. يلزم توقيع ولي الأمر.",
      },
    ],
    resolutions: [
      {
        en: "Student acknowledged the warning and promised improvement.",
        ar: "أقر الطالب بالتحذير ووعد بالتحسن.",
      },
      {
        en: "Parent contacted and aware of the situation.",
        ar: "تم التواصل مع ولي الأمر وهو على علم بالموقف.",
      },
    ],
  },
  {
    ...
    actions: [
      {
        en: "One hour after-school detention assigned.",
        ar: "تم تعيين حبس لمدة ساعة بعد المدرسة.",
      },
      {
        en: "Lunchtime detention for one week.",
        ar: "حبس وقت الغداء لمدة أسبوع.",
      },
    ],
    resolutions: [
      {
        en: "Student completed detention. Behavior improved.",
        ar: "أكمل الطالب فترة الحبس. تحسن السلوك.",
      },
      {
        en: "Parent meeting held. Improvement plan in place.",
        ar: "تم عقد اجتماع مع ولي الأمر. خطة تحسين قيد التنفيذ.",
      },
    ],
  },
]
...
export async function seedHealth(
  ...
  console.log(
    "🏥 Creating health records, achievements, and disciplinary records..."
  )
  ...
  const existingDisciplinary = await prisma.disciplinaryRecord.count({
    where: { schoolId },
  })

  if (
    existingHealth >= 100 &&
    existingAchievements >= 50 &&
    existingDisciplinary >= 20
  ) {
    console.log(
      `   ✅ Records already exist (health: ${existingHealth}, achievements: ${existingAchievements}, disciplinary: ${existingDisciplinary}), skipping\n`
    )
    return
  }
    ...
    for (const student of students) {
      ...
      for (let i = 0; i < numRecords; i++) {
        const recordTemplate =
          HEALTH_RECORD_TYPES[
            Math.floor(Math.random() * HEALTH_RECORD_TYPES.length)
          ]
        const titleTemplate =
          recordTemplate.titles[
            Math.floor(Math.random() * recordTemplate.titles.length)
          ]
        const descTemplate =
          recordTemplate.descriptions[
            Math.floor(Math.random() * recordTemplate.descriptions.length)
          ]
        ...
        healthRecords.push({
          ...
          doctorName:
            recordTemplate.type === "Medical Check-up"
              ? "Dr. Ahmed Hassan"
              : null,
          hospitalName:
            recordTemplate.type === "Medical Check-up"
              ? "Khartoum Medical Center"
              : null,
          recordedBy,
        })
      }
    ...
    for (const student of achievingStudents) {
      const template =
        ACHIEVEMENT_TEMPLATES[
          Math.floor(Math.random() * ACHIEVEMENT_TEMPLATES.length)
        ]
      const titleTemplate =
      ...
      achievements.push({
        ...
        position:
          template.positions[
            Math.floor(Math.random() * template.positions.length)
          ],
        issuedBy:
          template.issuers[Math.floor(Math.random() * template.issuers.length)],
    ...
    for (const student of studentsWithIncidents) {
      const template =
        DISCIPLINARY_TEMPLATES[
          Math.floor(Math.random() * DISCIPLINARY_TEMPLATES.length)
        ]
      const incidentTemplate =
        template.incidents[
          Math.floor(Math.random() * template.incidents.length)
        ]
      const actionTemplate =
        template.actions[Math.floor(Math.random() * template.actions.length)]
      const resolutionTemplate =
        template.resolutions[
          Math.floor(Math.random() * template.resolutions.length)
        ]
      const useArabic = Math.random() > 0.5
  ...
  console.log(
    `      - Health records: ${healthCount} (vaccinations, checkups, incidents)`
  )
  console.log(
    `      - Achievements: ${achievementCount} (academic, sports, arts, leadership)`
  )
  console.log(
    `      - Disciplinary: ${disciplinaryCount} (warnings, resolutions)\n`
  )
}
