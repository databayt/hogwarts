...
# to the pilot origin, Vercel/Turbo/Nx build noise is dropped, Sentry is
# blanked (no pilot noise in the prod project), and DATABASE_URL may be
...
for (const k of Object.keys(out)) if (/^(VERCEL|TURBO_|NX_)/.test(k)) delete out[k];
