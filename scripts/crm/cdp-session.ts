 ...
 */
export function requireScrapePort(): {
  port: number
  profile: string
  delayMs: number
} {
  ...
  const fail = (why: string): never => {
    ...
    console.error(
      "   This job drives a logged-in Chrome. It must run on a DEDICATED Facebook"
    )
    console.error(
      "   account in its own profile — never the session vault on 9222, which is"
    )
    console.error(
      "   Abdout's own browser and carries the social pipeline's Page tokens.\n"
    )
    ...
    console.error(
      "     bash ~/.claude/bin/chrome-debug.sh 9333   # then log in as that account\n"
    )
    process.exit(1)
  }
  ...
  if (!Number.isInteger(port) || port <= 0)
    fail(`FB_SCRAPE_PORT is not a port: ${raw}`)
  if (port === 9222) fail("FB_SCRAPE_PORT is 9222 — that is the session vault")
  if (!profile) fail("FB_SCRAPE_PROFILE is not set")
  if (profile.includes("chrome-debug-profile"))
    fail(`FB_SCRAPE_PROFILE points at the session vault: ${profile}`)
  return {
    port,
    profile,
    delayMs: Number(process.env.FB_SCRAPE_DELAY_MS ?? 8000),
  }
}
...
export class CdpSession extends EventEmitter {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.ws.onerror = (e) =>
        reject(new Error(`CDP socket error: ${String(e)}`))
      this.ws.onmessage = (event) => {
        try {
          ...
          /* a malformed frame is not worth killing the run */
        }
      }
...
export async function createCdpSession(
  ...
  const ver = await fetch(`http://127.0.0.1:${port}/json/version`).catch(
    () => null
  )
  if (!ver || !ver.ok) {
    throw new Error(
  ...
  if (!tab.webSocketDebuggerUrl || !tab.id)
    throw new Error(`could not open a tab: ${JSON.stringify(tab)}`)
