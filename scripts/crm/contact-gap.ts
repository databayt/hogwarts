...
const url = (l: Link | null | undefined): string =>
...
const tally = <T>(rows: T[], key: (r: T) => string): [string, number][] =>
  [
    ...rows.reduce(
      (m, r) => m.set(key(r), (m.get(key(r)) ?? 0) + 1),
...
async function main(): Promise<void> {
  ...
  if (country)
    rows = rows.filter((c) => (c.country ?? "").toUpperCase() === country)
  ...
  for (const l of order) {
    console.log(
      `  ${l.padEnd(12)} ${String(lane(l).length).padStart(5)}  ${pct(lane(l).length, total)}   ${label[l]}`
    )
  }
  ...
  const workable = lane("CONTACTABLE").filter(
    (c) =>
      ["COLD", "PROSPECT"].includes(c.stage ?? "") &&
      (c.tier === "A" || c.tier === "B")
  )
  console.log(
  ...
  if (workable.length > 10)
    console.log(`      … and ${workable.length - 10} more`)
  ...
  const payload = {
    ...
    fbQueue: lane("FB_PAGE").map((c) => ({
      id: c.id,
      name: c.name,
      country: c.country,
      url: url(c.facebook),
    })),
    webQueue: lane("WEBSITE").map((c) => ({
      id: c.id,
      name: c.name,
      country: c.country,
      url: url(c.domainName),
    })),
  }
