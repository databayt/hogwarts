...
async function main(): Promise<void> {
  ...
  const harvest = JSON.parse(readFileSync(`${DATA}/schools.json`, "utf8")) as {
    schools: Harvested[]
  }
  ...
  for (const c of live) {
    ...
    if (ref.startsWith("sudafoot:") || ref.startsWith("fb:"))
      byExternalId.set(ref, c)
  }
  ...
  for (const s of schools) {
    ...
    if (!s.sourceUrl || !/^https?:\/\//.test(s.sourceUrl)) {
      throw new Error(
        `${s.name}: no fetched source URL — refusing to write a record that cannot be traced`
      )
    }
    ...
    if (!match) {
      ...
      if (!match) {
        for (const c of live) {
          if (
            txt(c.originCountry) !== "SD" &&
            c.country === s.country &&
            nameMatches(s.name, c.name ?? "")
          ) {
            nearMisses.push(
              `${s.name.slice(0, 40)}  ~  ${(c.name ?? "").slice(0, 40)}  [not merged: not a Sudanese row]`
            )
          }
        }
      }
      if (!match && s.lat != null) {
        const near = live.find(
          (c) =>
            c.address?.addressLat != null &&
            haversineKm(
              s.lat!,
              s.lng!,
              c.address.addressLat!,
              c.address.addressLng!
            ) < 0.15
        )
        if (near) {
          match = near
    ...
    if (match) {
      ...
      const offer = (
        ...
        if (isContact && m.contactVerified) {
        ...
        if ((m.enrichmentNotes ?? "").includes(`sudafoot:${field}`)) {
          bump(`alreadyNoted.${field}`)
          return
        }
        conflicts.push(
          `${today} sudafoot:${field} — found "${value}", CRM has "${liveVal}", not overwritten`
        )
        bump(`conflict.${field}`)
      }
      ...
      offer(
        ...
        primary?.e164,
        () => {
          patch.schoolPhone = primary!.e164
        },
        true
      )
      offer(
        "email",
        txt(m.principalContact),
        email,
        () => {
      ...
      offer("principalName", txt(m.principalName), s.contactPerson, () => {
        patch.principalName = s.contactPerson
      })
      offer("grades", txt(m.grades), s.grades, () => {
        patch.grades = s.grades
      })
      offer("curriculum", txt(m.curriculum), curriculum ?? undefined, () => {
        patch.curriculum = curriculum
      })
      offer("originCountry", txt(m.originCountry), "SD", () => {
        patch.originCountry = "SD"
      })
      offer("operationalStatus", txt(m.operationalStatus), "OPERATING", () => {
        patch.operationalStatus = "OPERATING"
      })

      if (street && !txt(m.address?.addressStreet1)) {
        address.addressStreet1 = street
        bump("filled.street")
      }
      if (s.section && !txt(m.address?.addressCity)) {
        address.addressCity = s.section
        bump("filled.city")
      }
      ...
      if (conflicts.length)
        patch.enrichmentNotes = [txt(m.enrichmentNotes), ...conflicts]
          .filter(Boolean)
          .join("\n")
    ...
    const body: Record<string, unknown> = {
      ...
      sourceUrl: {
        ...
        secondaryLinks: [s.placeUrl ?? s.mapsUrl]
          .filter(Boolean)
          .map((u) => ({ label: "Google Maps", url: u })),
      },
    }
    ...
    if (s.lat != null) {
      addr.addressLat = s.lat
      addr.addressLng = s.lng
    }
    body.address = addr
    creates.push({ s, body })
    bump("create")
  ...
  if (updates.length) {
    ...
    for (const u of updates.slice(0, 12)) {
      console.log(
        `     [${u.why}] ${u.s.name.slice(0, 44).padEnd(44)} → ${Object.keys(u.patch).join(", ")}`
      )
    }
  }
  console.log(`\n  sample of new rows:`)
  for (const c of creates.slice(0, 10)) {
    console.log(
      `     ${c.s.country} ${String(c.body.tier)} ${c.s.name.slice(0, 44).padEnd(44)} ${c.body.schoolPhone ?? "—"}`
    )
  }
  ...
  writeFileSync(
    ...
    JSON.stringify({ creates, updates, skipped }, null, 2)
  )
  console.log(`\n  → ${DATA}/upsert-plan.json`)
  ...
  for (const c of creates) {
    try {
      await rest("POST", "companies", c.body)
      created++
    } catch (e) {
      failures.push(`create ${c.s.name}: ${e instanceof Error ? e.message : e}`)
    }
  }
  for (const u of updates) {
    try {
      ...
      failures.push(`update ${u.s.name}: ${e instanceof Error ? e.message : e}`)
    }
  }
  console.log(
    `  ${created}/${creates.length} created · ${updated}/${updates.length} updated`
  )
