...
async function main(): Promise<void> {
  if (!existsSync(LEDGER)) {
    console.error(`no ledger at ${LEDGER} — run enrich-fb-about.ts first`)
    process.exit(1)
  }
  const results = readFileSync(LEDGER, "utf8")
    .split("\n")
    .filter(Boolean)
  ...
  console.log(
    `Read ${results.length} ledger line(s) → ${latest.size} distinct page(s)`
  )
  ...
  for (const r of latest.values()) {
    ...
    if (!c) {
      bump("row gone from CRM")
      continue
    }
    if (!r.ok) {
      bump(`no content: ${r.why ?? "unknown"}`)
      continue
    }
    ...
    const offer = (
      ...
      if (!value) return
      if (isContact && c.contactVerified) {
      ...
      if ((c.enrichmentNotes ?? "").includes(`fb:${field}`)) {
      ...
      conflicts.push(
        `${today} fb:${field} — found "${value}", CRM has "${liveVal}", not overwritten`
      )
      bump(`conflict.${field}`)
    }
     ...
     */
    const noted = (marker: string): boolean =>
      (c.enrichmentNotes ?? "").includes(marker)
    ...
    if (phone) {
      ...
      if (want && !phone.e164.startsWith(`+${want}`)) {
        ...
        if (noted("fb:country") || noted("fb:phone")) {
          bump("alreadyAdjudicated.country")
          ...
          mismatched.push(
            `${r.name.slice(0, 40)} — ${phone.e164} vs country=${c.country}`
          )
    ...
    if (foreign && noted("fb:country")) {
      ...
      mismatched.push(
        `${r.name.slice(0, 40)} — domain implies ${implied}, filed ${rowCountry}`
      )
    ...
    if (
      txt(c.operationalStatus) === "UNVERIFIED" &&
      (r.phones.length || r.emails.length || r.website)
    ) {
      patch.operationalStatus = "OPERATING"
      bump("filled.operationalStatus")
    }
    if (!txt(c.lastSeenAt)) {
      patch.lastSeenAt = r.capturedAt
      bump("filled.lastSeenAt")
    }
  ...
  if (other.length) {
    console.log(`\n  other outcomes:`)
    for (const [k, v] of other.sort((a, b) => b[1] - a[1]))
  ...
  for (const u of updates) {
    try {
      ...
      if (/duplicate entry/i.test(msg) && u.patch.domainName) {
        ...
        const url =
          (domainName as { primaryLinkUrl?: string }).primaryLinkUrl ?? ""
        rest2.enrichmentNotes = [
          String(rest2.enrichmentNotes ?? ""),
          `${today} fb:website — "${url}" already belongs to another school (a group shares one site); not set here`,
        ]
          .filter(Boolean)
          .join("\n")
        try {
          await rest("PATCH", `companies/${u.id}`, rest2)
          degraded++
        } catch (e2) {
          fails.push(`${u.name}: ${e2 instanceof Error ? e2.message : e2}`)
        }
  ...
  if (degraded)
    console.log(
      `  ${degraded} written without a contested unique field (noted, not lost)`
    )
