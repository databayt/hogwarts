...
const LIMIT = Number(
  args.find((a) => a.startsWith("--limit="))?.split("=")[1] ?? WAVE_SIZE
)
...
function main() {
  ...
  if (!all.length)
    throw new Error("no workableNow rows — run contact-gap.ts first")
  ...
  console.log(
    `  dropped: landline ${landline}   ← never attempted, they read as disinterest`
  )
  ...
  for (const s of sheet) {
    ...
    console.log(
      `      → ${s.sendTo}${TEST_TO ? `  (real: ${s.realNumber})` : ""}`
    )
