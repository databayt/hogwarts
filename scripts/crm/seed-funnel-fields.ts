...
async function main() {
  ...
  console.log(
    APPLY ? "── APPLY ──" : "── DRY RUN (no writes; pass --apply to execute) ──"
  )
  ...
  const objRes = await t.rest<{ data?: MetaObject[] } | MetaObject[]>(
    "GET",
    "metadata/objects?limit=200"
  )
  ...
  if (!objects.length)
    throw new Error(
      "metadata/objects returned nothing — check TWENTY_API_URL (3100, never 3000) and the key"
    )
  ...
  for (const [objectName, fields] of Object.entries(FUNNEL_SCHEMA) as [
    string,
    FieldDef[],
  ][]) {
    ...
    if (!obj) {
      findings.push(
        `object "${objectName}" not found in this workspace — skipped ${fields.length} field(s)`
      )
      continue
    }
    ...
    for (const f of fields) {
      ...
      console.log(
        `  + ${f.name.padEnd(20)} ${f.type}${f.options ? ` (${f.options.length} options)` : ""}`
      )
      created++
      if (!APPLY) continue
  ...
  if (!stage) {
    findings.push(
      "company.stage not found — the gate mirror cannot be seeded; investigate before applying"
    )
  } else {
    ...
    if (lost.length) {
      ...
      findings.push(
        `company.stage is MISSING expected live option(s): ${lost.join(", ")} — investigate, do not auto-restore`
      )
    }
    ...
    if (!toAdd.length && !orderChanged) {
      ...
      if (toAdd.length) {
        console.log(
          `  + append: ${toAdd.join(", ")}  (${live.size} existing options preserved)`
        )
        created += toAdd.length
      }
      if (orderChanged)
        console.log(`  ~ reorder: ${merged.map((o) => o.value).join(" → ")}`)
      if (APPLY)
        await t.rest("PATCH", `metadata/fields/${stage.id}`, {
          options: merged,
        })
    }
  }
