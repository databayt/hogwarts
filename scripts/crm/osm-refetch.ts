...
const UA =
  "databayt-hogwarts-crm/1.0 (school lead enrichment; osmanabdout@hotmail.com)"
...
async function overpass(query: string, label: string): Promise<OsmElement[]> {
  for (let attempt = 0; attempt < MIRRORS.length * 2; attempt++) {
    const ep = [...MIRRORS].sort(
      (a, b) => (mirrorFailures.get(a) ?? 0) - (mirrorFailures.get(b) ?? 0)
    ...
    try {
      ...
      if (!res.ok || !ct.includes("json")) {
        const hint = /rate_limit|too many|busy|slow down/i.test(body)
          ? "rate limited"
          : `HTTP ${res.status}`
      ...
      console.warn(
        `    ! ${label}: ${why} on ${new URL(ep).host} — retrying in ${backoff / 1000}s`
      )
      await sleep(backoff)
    }
  }
...
async function fetchAll(
  ...
  for (const [i, batch] of batches.entries()) {
    ...
    try {
      ...
      const withContact = els.filter(
        (e) =>
          e.tags &&
          ["phone", "contact:phone", "website", "email", "contact:email"].some(
            (k) => e.tags![k]
          )
      ).length
      console.log(
      ...
      console.error(
        `  ✗ batch ${i + 1} FAILED: ${e instanceof Error ? e.message : e}`
      )
      console.error(
        `    dropped ids: ${batch.map((b) => `${b.type}/${b.id}`).join(",")}`
      )
      failed.push(i)
    }
  }
  ...
  console.log(
    `\n  cached ${byKey.size}/${refs.length} elements` +
      (failed.length ? ` — ${failed.length} batch(es) FAILED` : "")
  )
  if (failed.length)
    console.log(
      `  re-run to retry the failed batches (cached ones are skipped)`
    )
  return byKey
}
...
function plan(rows: Company[], osm: Map<string, OsmElement>): Plan {
  ...
  const unmapped: Record<string, Record<string, number>> = {
    operatorType: {},
    phone: {},
  }
  ...
  for (const c of rows) {
    ...
    if (!m) {
      noOsmId.push({ id: c.id, name: c.name ?? "" })
      continue
    }
    const key = `${m[1].toLowerCase()}/${m[2]}`
    const el = osm.get(key)
    if (!el) {
      missingFromOverpass.push(key)
      continue
    }
    const tags = el.tags ?? {}
    ...
    const offer = (
      ...
      if ((c.enrichmentNotes ?? "").includes(`osm:${field}`)) {
        bump(`alreadyAdjudicated.${field}`)
        return
      }
      if (!live) {
        write()
      ...
      conflicts.push(
        `${today} osm:${field} — found "${value}", CRM has "${live}", not overwritten`
      )
      bump(`conflict.${field}`)
    }
    ...
    if (rawPhone) {
      ...
      if (n.e164) {
        phoneReach = n.reach
        offer("phone", txt(c.schoolPhone), n.e164, () => {
          patch.schoolPhone = n.e164
        })
      } else {
    ...
    offer("nameEn", txt(c.nameEn), pick(tags, "name:en"), () => {
      patch.nameEn = pick(tags, "name:en")
    })
    offer(
      "grades",
      txt(c.grades),
      pick(tags, "grades", "school:grades"),
      () => {
        patch.grades = pick(tags, "grades", "school:grades")
      }
    )
    offer("iscedLevel", txt(c.iscedLevel), pick(tags, "isced:level"), () => {
      patch.iscedLevel = pick(tags, "isced:level")
    })
    offer("gender", txt(c.gender), pick(tags, "school:gender"), () => {
      patch.gender = pick(tags, "school:gender")
    })
    offer("operator", txt(c.operator), pick(tags, "operator"), () => {
      patch.operator = pick(tags, "operator")
    })
    ...
    if (opType) {
      ...
      if (mapped)
        offer("schoolType", txt(c.schoolType), mapped, () => {
          patch.schoolType = mapped
        })
      else
        unmapped.operatorType[opType] = (unmapped.operatorType[opType] ?? 0) + 1
    }

    // -- address + coordinate ------------------------------------------------
    const street = pick(tags, "addr:street")
    if (street && !txt(c.address?.addressStreet1)) {
      address.addressStreet1 = street
      gained.push("street")
      bump("gained.street")
    }
    const city = pick(tags, "addr:city")
    if (city && !txt(c.address?.addressCity)) {
    ...
    if (postcode && !txt(c.address?.addressPostcode)) {
      ...
      gained.push("postcode")
      bump("gained.postcode")
    }
    ...
    if (conflicts.length) {
      patch.enrichmentNotes = [txt(c.enrichmentNotes), ...conflicts]
        .filter(Boolean)
        .join("\n")
    }
...
async function main(): Promise<void> {
  ...
  const gains = Object.entries(p.stats)
    .filter(([k]) => k.startsWith("gained."))
    .sort((a, b) => b[1] - a[1])
  for (const [k, v] of gains)
    console.log(`  +${String(v).padStart(5)}  ${k.replace("gained.", "")}`)

  const contactGain = ["phone", "email"].reduce(
    (n, f) => n + (p.stats[`gained.${f}`] ?? 0),
    0
  )
  console.log(
    `\n  ${contactGain} schools gain a directly contactable field (phone or email)`
  )
  const reach = p.writes
    .filter((w) => w.phoneReach)
    .reduce(
      (m, w) => {
        m[w.phoneReach!] = (m[w.phoneReach!] ?? 0) + 1
        return m
      },
  ...
  if (Object.keys(reach).length) {
    console.log(
      `  phone reach: ${Object.entries(reach)
        .map(([k, v]) => `${k} ${v}`)
        .join("  ")}`
    )
  }

  const conflicts = Object.entries(p.stats).filter(([k]) =>
  ...
  if (p.missingFromOverpass.length) {
    console.log(
      `    ${p.missingFromOverpass.length} OSM ids returned nothing (deleted upstream since the import)`
    )
  }
  for (const [k, v] of Object.entries(p.unmapped)) {
    ...
    if (items.length)
      console.log(
        `    unmapped ${k}: ${items.map(([a, b]) => `${a} (${b})`).join(", ")}`
      )
  }
  console.log(`\n  → ${PLAN_FILE}`)
  ...
  for (const w of p.writes) {
    try {
      ...
      if (/duplicate entry/i.test(msg) && w.patch.domainName) {
        ...
        rest2.enrichmentNotes = [String(rest2.enrichmentNotes ?? ""), note]
          .filter(Boolean)
          .join("\n")
        try {
    ...
    if ((ok + degraded) % 100 === 0)
      console.log(`  ${ok + degraded}/${p.writes.length} …`)
  }
  console.log(`\n  ${ok}/${p.writes.length} updated.`)
  if (degraded) {
    console.log(
      `  ${degraded} written WITHOUT a contested unique field (recorded as a note, not lost)`
    )
  }
  if (failures.length) {
