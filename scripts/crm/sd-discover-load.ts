...
const PLACE_COUNTRY: [RegExp, string][] = [
  [
    /مصر|القاهرة|الجيزة|الإسكندرية|أسوان|أكتوبر|فيصل|مدينة نصر|عين شمس|المهندسين|دمياط|بورسعيد/,
    "EG",
  ],
  [
    /السعودية|الرياض|جدة|مكة|المدينة المنورة|الدمام|الخبر|الطائف|أبها|جازان|تبوك/,
    "SA",
  ],
...
const countryOf = (f: Found): string =>
  f.kind === "sudan"
    ? "SD"
    : (PLACE_COUNTRY.find(([re]) => re.test(f.query))?.[1] ?? "OTHER")
...
async function main(): Promise<void> {
  if (!existsSync(LEDGER)) {
    console.error(`no ledger at ${LEDGER} — run discover-fb-pages.ts first`)
    process.exit(1)
  }
  const raw = readFileSync(LEDGER, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((l) => JSON.parse(l) as Found)
  ...
  console.log(
    `Read ${raw.length} ledger line(s) → ${byUrl.size} distinct page(s)`
  )
  ...
  const fbUrls = new Set(
    live
      .map((c) => pageSlug(txt(c.facebook?.primaryLinkUrl)).toLowerCase())
      .filter(Boolean)
  )
  ...
  for (const f of byUrl.values()) {
    if (!SCHOOLish.test(f.name)) {
      rej("not school-shaped")
      continue
    }
    if (NOT_A_SCHOOL.test(f.name)) {
      rej("sports / alumni / news / course page")
      continue
    }
    if (TRADE.test(f.name)) {
      rej("a trade, not a school")
      continue
    }

    const slug = pageSlug(f.url).toLowerCase()
    if (!slug) {
      rej("no usable page slug")
      continue
    }
    if (refs.has(`fb:${slug}`) || fbUrls.has(slug)) {
      rej("already in the CRM")
      continue
    }
    if (seenSlug.has(slug)) {
      rej("duplicate page in this batch")
      continue
    }
    if (names.has(foldAr(f.name))) {
      rej("a row with this exact name already exists")
      continue
    }
    /**
     ...
     * first, log the rest.
     */
    if (seenName.has(foldAr(f.name))) {
      rej("same name as another page in this batch")
      continue
    }
    seenSlug.add(slug)
    ...
    creates.push({
      ...
      body: {
        ...
        sourceUrl: {
          primaryLinkUrl: f.url,
          primaryLinkLabel: `Facebook page search: ${f.query}`,
          secondaryLinks: [],
        },
        facebook: {
          primaryLinkUrl: f.url,
  ...
  const byCountry = creates.reduce(
    (m, c) => {
      const k = String(c.body.country)
      m[k] = (m[k] ?? 0) + 1
      return m
    },
  ...
  for (const c of creates.slice(0, 12))
    console.log(
      `    ${String(c.body.country).padEnd(3)} ${c.f.name.slice(0, 56)}`
    )
  ...
  for (const c of creates) {
    try {
      ...
      fails.push(
        `${c.f.name.slice(0, 40)}: ${e instanceof Error ? e.message : e}`
      )
    }
  }
