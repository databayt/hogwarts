...
const DIASPORA_PLACES: { country: string; places: string[] }[] = [
  ...
  {
    country: "الإمارات",
    places: ["دبي", "أبوظبي", "الشارقة", "عجمان", "العين", "رأس الخيمة"],
  },
  { country: "قطر", places: ["الدوحة", "الريان", "الوكرة"] },
  {
    country: "الكويت",
    places: ["حولي", "الفروانية", "الجهراء", "الأحمدي", "السالمية"],
  },
...
const SCHOOLish =
  /مدرس|مدارس|أكاديمي|اكاديمي|روضة|روضه|رياض أطفال|مجمع تربوي|تعليم|school|academy|kindergarten|college|institute|معهد/i
const NOT_A_SCHOOL =
...
const isPageUrl = (u: string): boolean =>
  ...
  !/\/(groups|events|marketplace|watch|photo|videos|posts|permalink|story\.php|hashtag|search|help|policies|privacy|login|reel)\b/i.test(
    u
  ) &&
  !/\.php\?(?!id=)/i.test(u)
...
async function searchOnce(
  ...
  kind: string,
  delayMs: number
): Promise<{ found: Found[]; stop?: string }> {
  ...
  if (r.loginWall)
    return { found: [], stop: "login wall — the scrape session is signed out" }
...
async function main(): Promise<void> {
  ...
  if (!DIASPORA_ONLY) {
    for (const p of SUDAN_PLACES)
      for (const w of SCHOOL_WORDS.slice(0, 3))
        queries.push({ q: `${w} ${p}`, kind: "sudan" })
  }
  for (const c of DIASPORA_CITIES)
    for (const t of DIASPORA_TERMS)
      queries.push({ q: `${t} ${c}`, kind: "diaspora" })

  mkdirSync(DATA, { recursive: true })
  const done = new Set<string>(
    existsSync(CHECKPOINT)
      ? (JSON.parse(readFileSync(CHECKPOINT, "utf8")) as string[])
      : []
  )
  ...
  if (existsSync(LEDGER)) {
    for (const line of readFileSync(LEDGER, "utf8")
      ...
      .filter(Boolean)) {
      try {
        const f = JSON.parse(line) as Found
        known.set(f.url, f)
      } catch {
        /* skip */
      }
  ...
  const todo = queries
    .filter((x) => !done.has(x.q))
    .slice(0, limit === Infinity ? undefined : limit)
  console.log(
    `Scrape browser: port ${port}, profile ${profile}, delay ~${delayMs}ms`
  )
  console.log(
    `${queries.length} queries in the matrix · ${done.size} already done · ${todo.length} to run`
  )
  ...
  try {
    for (const [i, x] of todo.entries()) {
      ...
      console.log(
        `  ${String(i + 1).padStart(3)}/${todo.length} ${x.kind.padEnd(8)} ${String(fresh).padStart(2)} new  ${x.q.slice(0, 40)}`
      )
      await sleep(jitter(delayMs))
    }
  } finally {
  ...
  const byKind = [...known.values()].reduce(
    (m, f) => {
  ...
  if (stopped) {
    ...
    console.log(
      `     Re-run to resume — the checkpoint skips completed queries.`
    )
  }
  console.log(`\n  → ${LEDGER}`)
  console.log(
    "  Nothing written to Twenty. Load with sd-fb-discover-load.ts.\n"
  )
}
