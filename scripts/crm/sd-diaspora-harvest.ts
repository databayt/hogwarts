...
const UA =
  "databayt-hogwarts-crm/1.0 (school lead research; osmanabdout@hotmail.com)"
...
const CURRICULUM_HINT =
  /منهج|المنهج|انترناشونال|إنترناشونال|دولي|بريطاني|أمريكي|سوداني/
...
async function resolveMaps(
  url: string
): Promise<{ lat?: number; lng?: number; placeUrl?: string }> {
  try {
    const res = await fetch(url, {
      method: "GET",
      redirect: "follow",
    ...
    const hit =
      /@(-?\d+\.\d+),(-?\d+\.\d+)/.exec(res.url) ??
      /!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/.exec(res.url)
    if (hit)
      return { lat: Number(hit[1]), lng: Number(hit[2]), placeUrl: res.url }
...
function parse(html: string, s: Source, link: string): DiasporaSchool[] {
  const blocks = [
    ...html.matchAll(/<(h[1-6]|p|li)\b[^>]*>([\s\S]*?)<\/\1>/gi),
  ].map((m) => ({
    tag: m[1].toLowerCase(),
    text: strip(m[2]),
  }))
  ...
  for (const b of blocks) {
    ...
    if (b.tag === "h4") {
      ...
      const seen = out.filter(
        (o) => o.externalId === base || o.externalId.startsWith(`${base}#`)
      ).length
      cur = {
    ...
    const maps =
      /https?:\/\/(?:maps\.app\.goo\.gl|goo\.gl\/maps|g\.co\/kgs|www\.google\.[a-z.]+\/maps)\/\S+/.exec(
        b.text
      )
    if (maps) {
      cur.mapsUrl = maps[0]
    ...
    if (BRANCH_HINT.test(b.text)) {
      cur.branches.push(b.text)
      claimed = true
    }
    if (
      !cur.curriculum &&
      CURRICULUM_HINT.test(b.text) &&
      !LOC_HINT.test(b.text)
    ) {
      cur.curriculum = b.text
      claimed = true
    }
    if (!cur.grades && GRADES_HINT.test(b.text) && b.text.length < 80) {
      cur.grades = b.text
      claimed = true
    }
    if (!cur.studyMode && STUDY_HINT.test(b.text)) {
      cur.studyMode = b.text
      claimed = true
    }
    if (LOC_HINT.test(b.text)) {
      cur.addressLines.push(b.text)
      claimed = true
    }
    if (!cur.contactPerson && PERSON_HINT.test(b.text)) {
      cur.contactPerson = b.text
      claimed = true
    }
...
async function main(): Promise<void> {
  ...
  for (const c of ["EG", "SA"] as const) {
    ...
    const mob = rows.filter((r) =>
      r.phones.some((p) => p.reach === "MOBILE")
    ).length
    console.log(
  ...
  if (foreign.length) {
    console.log(
      `  ${foreign.length} number(s) whose country differs from the directory's — kept and tagged, not dropped`
    )
  }
  ...
  for (const r of noContact)
    console.log(`      ${r.country} ${r.name.slice(0, 60)}`)
  const unparsed = all.flatMap((r) =>
    r.unparsed.map((u) => `${r.name.slice(0, 24)} :: ${u.slice(0, 70)}`)
  )
  console.log(
    `\n  ${unparsed.length} line(s) the parser could not classify (kept, not dropped):`
  )
  for (const u of unparsed.slice(0, 15)) console.log(`      ${u}`)
  if (unparsed.length > 15)
    console.log(`      … and ${unparsed.length - 15} more (all in the JSON)`)
  ...
  writeFileSync(
    out,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), schools: all },
      null,
      2
    )
  )
