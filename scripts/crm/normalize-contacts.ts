...
const MARKETS: Record<
  string,
  { cc: string; mobile: RegExp; nsnLen: number[] }
> = {
...
const BY_CC = Object.fromEntries(
  Object.entries(MARKETS).map(([iso, m]) => [m.cc, iso])
)
...
export function normalizePhone(raw: string, countryHint: string): Normalized {
  ...
  const explicitIntl =
    trimmed.trimStart().startsWith("+") || digits.startsWith("00")
  ...
  const split = (d: string): { cc: string; nsn: string } | null => {
    for (const c of CCS) {
      ...
      if (MARKETS[BY_CC[c]].nsnLen.includes(rest.length))
        return { cc: c, nsn: rest }
    }
    return null
  }
  ...
  if (!market.nsnLen.includes(nsn.length)) {
    return {
      e164: null,
      reach: "NOT_DIALABLE",
      why: `wrong length for ${iso} (${nsn.length}d)`,
    }
  }
  ...
  return market.mobile.test(nsn)
    ...
    : {
        e164,
        reach: "LANDLINE",
        why: `${iso} landline — callable, not WhatsApp`,
      }
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `  ${changes.filter((c) => c.reach === "MOBILE").length} of those are mobile → WhatsApp-reachable`
  )
  console.log(
    `  ${emails.length} rows carry an email in principalContact → the email lane\n`
  )

  for (const c of changes.slice(0, 12)) {
    console.log(
      `   ${c.from.padEnd(18)} → ${c.to.padEnd(15)} ${c.reach.padEnd(9)} ${c.name.slice(0, 34)}`
    )
  }
  ...
  writeFileSync(
    "scripts/crm/.data/normalize-contacts.json",
    JSON.stringify(
      { generatedAt: new Date().toISOString(), tally, changes, emails },
      null,
      2
    )
  )
  ...
  for (const c of changes.slice(
    0,
    limit === Infinity ? changes.length : limit
  )) {
    try {
