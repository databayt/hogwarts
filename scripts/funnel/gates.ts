...
async function main() {
  ...
  const rows = countryFilter
    ? all.filter((c) => (c.country ?? "").toUpperCase() === countryFilter)
    : all

  // ── Gates ──────────────────────────────────────────────────────────────────
  const gates = Object.fromEntries(LADDER.map((g) => [g, 0])) as Record<
  ...
  const prisma = await prismaCounts().catch((e) => {
    console.warn(
      `  ! Postgres unreadable (${e instanceof Error ? e.message.split("\n")[0] : e}) — reporting Twenty only`
    )
    return null
  })
  ...
  console.log(
    "  " +
      LADDER.map((g) => `${g} ${gates[g]}`).join(" · ") +
      (unknownStage ? ` · (no stage) ${unknownStage}` : "")
  )
  console.log(`\n  Reach (schoolPhone + principalContact):`)
  console.log(
    `    whatsapp lane (verified mobile)  ${whatsappLane}   (${reach.mobileAndEmail} also have email)`
  )
  console.log(`    email lane (email, no mobile)    ${reach.emailOnly}`)
  console.log(
    `    landline only                    ${reach.landlineOnly}   ← never WhatsApped, never counted reachable`
  )
  console.log(
    `    no usable contact                ${reach.none}   ← /scrape's gap, not this lane's`
  )
  console.log(
    `    by rail: sd ${reach.byRail.sd} · gulf ${reach.byRail.gulf} · eg ${reach.byRail.eg} · other ${reach.byRail.other}`
  )
  console.log(`\n  The roll's queue (reachable AND still COLD/PROSPECT):`)
  console.log(
    `    whatsapp ${reach.lanePending.whatsapp} · email ${reach.lanePending.email}`
  )
  if (prisma) {
    console.log(`\n  Postgres [${dbTag}]:`)
    console.log(
      `    Prospect ${JSON.stringify(prisma.prospect)} · Lead ${JSON.stringify(prisma.lead)}`
    )
    console.log(
      `    Inbox    ${JSON.stringify(prisma.inbox)}   ← pending = received, not yet applied`
    )
  }
  ...
  const out = writeGatesArtifact({
    ...
    source: {
      twenty: process.env.TWENTY_API_URL,
      companies: all.length,
      reachScope: countryFilter || "ALL",
    },
  })
