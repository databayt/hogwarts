...
/**
 * The files a set of sources pulls in by RELATIVE import, resolved one level
 * deep. Only relative specifiers are followed: a feature that delegates its
 * translation does so to a sibling or parent module (`../shared/…`), never to
 * a package. Unresolvable specifiers are skipped rather than thrown on — this
 * is an audit, and a missing file is the other detectors' problem.
 */
function relativeImportTargets(srcs: string[]): string[] {
...
function findFieldOffenders(): FeatureGap[] {
  ...
  for (const [dir, models] of Object.entries(DIR_MODELS)) {
    ...
    // A feature counts as translating if any of ITS files calls a display
    // helper, OR if it delegates to a module it imports relatively. The
    // second half matters: transportation/routes/ translates every row
    // through `../shared/translate-display`'s `translateRoutes`, which wraps
    // `getLabels` — a dir-only scan called that feature untranslated when it
    // is in fact fully covered, and a detector that cries wolf gets muted.
    const translates =
      srcs.some((f) => DISPLAY_HELPERS.test(readFileSync(f, "utf8"))) ||
      relativeImportTargets(srcs).some((f) =>
...
const PREWARM_EXEMPT: Record<string, string> = {
  ...
  "src/components/school-dashboard/live/demo-clock.ts":
    "demo fixture repair: also called by prisma/seeds with its own client, where after() has no request scope; writes `lang` with the row",
  "src/components/school-dashboard/finance/fees/transport-provisioning.ts":
    "auto-generated inside tx (after() unavailable): name is `${dictLabel} — ${route.name} — ${month}` on an isAutoGenerated row, not user text",
}
