...
program
  ...
  .option('-t, --tier <tier>', 'Subscription tier (free|basic|premium)', 'basic')
...
async function provisionSchool() {
  ...
  try {
    ...
    if (existing) {
      spinner.fail(chalk.red(`School with domain "${options.domain}" already exists`))
      process.exit(1)
    }
    ...
    console.log(`  URL:       ${chalk.blue(`https://${result.school.domain}.databayt.org`)}`)
    ...
    if (options.withDemoData) {
      ...
      console.log(chalk.gray(`  npx tsx scripts/seed-demo-data.ts --school ${result.school.id}`))
    }
