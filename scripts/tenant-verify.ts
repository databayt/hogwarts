...
function analyzeFile(filePath: string) {
  ...
  lines.forEach((line, index) => {
    ...
    const prismaMatch = line.match(/prisma\.(\w+)\.(findFirst|findMany|create|update|delete|updateMany|deleteMany|count|aggregate|findUnique)\(/g)
    ...
    if (prismaMatch) {
      prismaMatch.forEach(match => {
        ...
        const hasSchoolId = context.includes('schoolId') || context.includes('getTenantContext')
...
async function verifyIsolation() {
  const spinner = ora('Scanning codebase for multi-tenant violations...').start()
  ...
  try {
    ...
    console.log(chalk.red('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'))
    console.log(chalk.red(`Total: ${criticalCount} critical, ${warningCount} warnings`))
    console.log(chalk.red('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'))
    ...
    if (options.fix) {
      ...
      console.log(chalk.gray('   const { schoolId } = await getTenantContext()\n'))
      console.log(chalk.cyan('3. For server actions, get from session:'))
      console.log(chalk.gray('   const session = await auth()\n   const schoolId = session?.user?.schoolId\n'))
    }
