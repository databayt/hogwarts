...
program
  ...
  .option('--severity <level>', 'Minimum severity: low|moderate|high|critical', 'moderate')
  .parse()
...
function scanDirectory(dir: string) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      if (!item.startsWith('.') && !['node_modules', 'dist', '.next'].includes(item)) {
...
async function scanDependencies() {
  ...
  try {
    ...
    if (vulnCount > 0) {
      spinner.warn(chalk.yellow(`Found ${vulnCount} dependency vulnerabilities`))
    ...
    if (error.stdout) {
      try {
        ...
        console.log(`  Critical: ${chalk.red(metadata.vulnerabilities?.critical || 0)}`)
        console.log(`  High:     ${chalk.red(metadata.vulnerabilities?.high || 0)}`)
        console.log(`  Moderate: ${chalk.yellow(metadata.vulnerabilities?.moderate || 0)}`)
        console.log(`  Low:      ${chalk.gray(metadata.vulnerabilities?.low || 0)}`)
        ...
        return (metadata.vulnerabilities?.critical || 0) +
...
async function main() {
  ...
  console.log(`Dependency vulnerabilities: ${depVulns > 0 ? chalk.red(depVulns) : chalk.green('0')}`)
  console.log(`Code vulnerabilities: ${vulnerabilities.length > 0 ? chalk.red(vulnerabilities.length) : chalk.green('0')}`)
