    ...
    for (const r of rolesToCheck) {
      ...
      for (const [resource, actions] of Object.entries(perms)) {
        ...
        if (actionsArray.length === 0) {
          ...
          console.log(chalk.green(`  ✓ ${resource}: ${actionsArray.join(', ')}`))
        }
      }
      ...
      if (r === 'TEACHER') {
        ...
        if (hasPermission(perms, 'students') && perms.students.includes('delete')) {
          issues.push({
            role: r,
            resource: 'students',
      ...
      if (r === 'STUDENT') {
        if (hasPermission(perms, 'attendance') && perms.attendance.includes('write')) {
          issues.push({
            role: r,
            resource: 'attendance',
