  ...
  try {
    ...
    for (const row of data) {
      try {
        ...
        if (!yearLevelId) {
          errors.push(`Year level "${row.yearLevel}" not found for ${row.email}`)
          skipped++
          continue
        }
    ...
    spinner.succeed(chalk.green(`Import complete: ${imported} imported, ${skipped} skipped`))

    if (errors.length > 0) {
      console.log(chalk.yellow(`\n⚠️  ${errors.length} errors:\n`))
      errors.slice(0, 10).forEach(err => console.log(chalk.gray(`  • ${err}`)))
      if (errors.length > 10) {
...
async function importData() {
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log(chalk.gray(`Would import ${data.length} ${options.type} to ${school.name}`))
