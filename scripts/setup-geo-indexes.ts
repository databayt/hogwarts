...
async function setupIndexes() {
  ...
  try {
    console.log(
      "🔧 Setting up PostGIS spatial indexes for geofence performance..."
    )
    ...
    console.log(
      "   - idx_location_traces_point (GiST) for location spatial queries"
    )
    console.log(
      "   - idx_location_traces_timestamp_brin (BRIN) for time-series queries"
    )
    console.log(
      "   - idx_geo_events_timestamp_brin (BRIN) for event time-series"
    )
    ...
    result.rows.forEach(
      (row: { tablename: string; indexname: string; indexdef: string }) => {
