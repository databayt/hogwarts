...
| 7 | **people** | `pnpm db:seed:single people` | 25 teachers, 100 students, 200 guardians | school, academic, departments |
...
|-------|---------|--------|
| Name | Demo School | مدرسة دار بايت التجريبية |
| Domain | demo.databayt.org | - |
| Motto | Always Forward | إلى الأمام دائماً |
| Type | Full K-12 School | مدرسة متكاملة (روضة - ثانوي) |

### Location

| Field | English | Arabic |
|-------|---------|--------|
| Address | University Street, Khartoum | شارع الجامعة، الخرطوم |
| City | Khartoum | الخرطوم |
| State | Khartoum State | ولاية الخرطوم |
| Country | Sudan | السودان |
| Timezone | Africa/Khartoum | - |
| Phone | +249-183-123456 | +٢٤٩-١٨٣-١٢٣٤٥٦ |
...
|------|--------|------------------|------------------|
| DEVELOPER | All schools | Platform Developer - Full Access | مطور المنصة - صلاحيات كاملة |
| ADMIN | School-scoped | School Administrator | مدير المدرسة - صلاحيات إدارية |
| ACCOUNTANT | Finance modules | School Accountant | محاسب المدرسة - صلاحيات مالية |
| STAFF | Limited | School Staff | موظف المدرسة - صلاحيات محدودة |
...
|---------|-------------|-------------|----------|
| Kindergarten | KG 1, KG 2 | روضة أولى، روضة ثانية | 20 |
| Primary | Grade 1-6 | الصف الأول - السادس | 46 |
| Intermediate | Grade 7-9 | الصف السابع - التاسع | 18 |
| Secondary | Grade 10-12 | الصف العاشر - الثاني عشر | 16 |
...
|---------|--------|----------|
| Languages | اللغات | Arabic, English, French, Reading, Writing |
| Sciences | العلوم | Mathematics, Science, Physics, Chemistry, Biology |
| Humanities | العلوم الإنسانية | Social Studies, Geography, History, Civics |
| Religion | الدين | Islamic Studies, Quran |
| ICT | تقنية المعلومات | Computer Science |
| Arts & PE | الفنون والرياضة | Art, Physical Education, Music |
...
```
prisma/seeds/
├── index.ts           # Main orchestrator (full seed)
├── run-single.ts      # Individual phase runner
├── constants.ts       # Bilingual data constants
├── types.ts           # TypeScript types
├── cleanup.ts         # Database cleanup
│
├── school.ts          # Phase 2: School + branding
├── auth.ts            # Phase 3: Admin users
├── academic.ts        # Phase 4: Year, terms, levels
├── departments.ts     # Phase 5: Departments + subjects
├── classrooms.ts      # Phase 6: Physical rooms
├── people.ts          # Phase 7: Teachers, students, guardians
├── classes.ts         # Phase 8: Class sections
│
├── library.ts         # Phase 9: Books
├── announcements.ts   # Phase 10: Announcements
├── events.ts          # Phase 11: Events
├── fees.ts            # Phase 12: Fee structures
├── finance.ts         # Phase 13: Finance data
├── exams.ts           # Phase 14: Exam schedules
├── grades.ts          # Phase 15: Student grades
├── timetable.ts       # Phase 16: Class schedules
├── stream.ts          # Phase 17: LMS courses
├── lessons.ts         # Phase 18: Lesson plans
├── reports.ts         # Phase 19: Student reports
├── attendance.ts      # Phase 20: Attendance
└── admission.ts       # Phase 21: Applications
```
...
// People
export const TEACHER_DATA: TeacherData[];     // 25 teachers
...
export async function seedMyFeature(
  ...
  console.log("🎯 Creating My Feature (Bilingual AR/EN)...");
  ...
  console.log("   ✅ My Feature created");
}
```
