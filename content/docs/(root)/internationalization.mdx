...
export function AnnouncementCard({ announcement, lang }: Props) {
  const title = getLocalizedText(lang, announcement.titleEn, announcement.titleAr);
  ...
  return (
    <Tabs defaultValue="ar">
      ...
      <TabsContent value="ar" dir="rtl">
        <Input {...form.register("titleAr")} placeholder="العنوان" />
        <Textarea {...form.register("bodyAr")} placeholder="المحتوى" />
      </TabsContent>
...
```ts
import { getLocalizedText, getLocalizedFromObject, getLocalizedFromJSON } from "@/lib/i18n-content";
...
import { getBilingualStatus } from "@/lib/i18n-content";
...
const bilingual = convertToBilingual("مرحبا بالعالم"); // { en: "", ar: "مرحبا بالعالم" }
```
...
├── dictionaries.ts        # Dictionary loaders
├── use-locale.ts          # Client hooks
├── use-dictionary.ts      # Dictionary hook
├── language-switcher.tsx  # UI component
├── en.json                # General English
├── ar.json                # General Arabic
├── school-en.json         # School module (English)
├── school-ar.json         # School module (Arabic)
├── operator-en.json       # Operator module
├── operator-ar.json
└── dictionaries/
    ├── en/
    │   ├── finance.json
...
- **Arabic font**: Tajawal (system default)
- **English font**: Inter
