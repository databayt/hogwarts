...
### Custom Filter Dropdowns (Internationalized)

Add entity-specific filters via `additionalActions` with **translated labels**:

```typescript
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

// Filter options built inside component with dictionary translations
// Use simplified labels: "Scope" not "All Scopes", "Status" not "All Status"
const SCOPE_OPTIONS = useMemo(() => [
  { value: "all", label: t.scope },        // "Scope" / "النطاق"
  { value: "school", label: t.schoolWide }, // "School-wide" / "المدرسة"
  { value: "class", label: t.classSpecific },
  { value: "role", label: t.roleSpecific },
], [t]);

const STATUS_OPTIONS = useMemo(() => [
  { value: "all", label: t.status },       // "Status" / "الحالة"
...
// Filter dropdowns (h-9 py-1 to match Input styling exactly)
...
**Key Points:**
- Build filter options **inside the component** where dictionary is available
- Use `useMemo` to avoid recreating options on every render
- Use **simplified labels** ("Scope" not "All Scopes") for cleaner UI
- Pass translated `placeholder` to SelectValue
...
### Avoiding Duplicate Filters

When using custom toolbar filters, **disable column filters** to avoid duplicate UI:

```typescript
// columns.tsx - WRONG: creates duplicate filters
{
  accessorKey: "scope",
  meta: {
    variant: "select",
  ...
  enableColumnFilter: true,  // ❌ Creates dropdown in column header
}

// columns.tsx - CORRECT: toolbar handles filtering
{
  accessorKey: "scope",
  cell: ({ getValue }) => {
    const scope = getValue<string>();
    const labels = { school: t.schoolWide, class: t.classSpecific };
    return <span>{labels[scope] || scope}</span>;
  },
  meta: { label: t.scope, variant: "text" },
  enableColumnFilter: false,  // ✅ Toolbar has custom filter
}
```

**Rule:** If you have a custom filter dropdown in the toolbar, set `enableColumnFilter: false` on that column.
...
All toolbar elements must use `h-9` (36px) for visual consistency. **Important:** SelectTrigger uses `py-2` by default while Input uses `py-1`—add `py-1` to SelectTrigger for identical appearance:

```typescript
// ✅ CORRECT - consistent height AND padding
...
// ❌ WRONG - inconsistent heights or padding
...
Use dictionary translations for page-nav items:
