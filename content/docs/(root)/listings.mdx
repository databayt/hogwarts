...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 not-prose">
  <div className="border rounded-lg p-6">
    ...
    <p className="text-sm text-muted-foreground">Add, update, and remove items instantly with automatic rollback on errors.</p>
  </div>
  <div className="border rounded-lg p-6">
    <h4 className="font-semibold mb-2">URL-Persisted Views</h4>
    <p className="text-sm text-muted-foreground">Table/Grid view toggle persists via URL query params using nuqs.</p>
  </div>
  <div className="border rounded-lg p-6">
    <h4 className="font-semibold mb-2">Infinite Scroll</h4>
    <p className="text-sm text-muted-foreground">Load-more pagination for seamless data browsing without page reloads.</p>
  </div>
  <div className="border rounded-lg p-6">
    <h4 className="font-semibold mb-2">Centralized Queries</h4>
    <p className="text-sm text-muted-foreground">Type-safe query builders with role-based authorization per module.</p>
  </div>
</div>
...
├── table.tsx          # Main listing component (client)
├── form.tsx           # Create/Edit form (client)
├── columns.tsx        # Column definitions (client)
├── content.tsx        # Server component wrapper
├── actions.ts         # Server actions ("use server")
├── queries.ts         # Query builders & data fetchers (NEW)
├── authorization.ts   # Role-based access control (NEW)
...
```typescript
const handleTogglePublish = useCallback(async (item: RowType) => {
...
export async function getStudentsList(
  ...
  const [rows, count] = await Promise.all([
    db.student.findMany({ where, orderBy, skip, take, select: studentListSelect }),
...
export type ModuleAction = 'create' | 'read' | 'update' | 'delete' | 'export' | 'bulk_action';
...
import { studentSchema, getStudentsSchema } from "./validation";
import { getStudentsList, getStudentDetail } from "./queries";
import { getAuthContext, assertPermission } from "./authorization";
...
├──────────────────────────────────────────────────────────────────────┤
│  [━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━]                           │
│  Step 1 of 3: Basic Information            [Cancel] [Save] [Next]   │
└──────────────────────────────────────────────────────────────────────┘
```
            ...
            <GridContainer columns={3}>
              {data.map((student) => (
                <GridCard
                  ...
                  actions={[
                    { label: lang === 'ar' ? 'عرض' : 'View', onClick: () => handleView(student) },
                    { label: lang === 'ar' ? 'تعديل' : 'Edit', onClick: () => handleEdit(student) },
                    { label: lang === 'ar' ? 'حذف' : 'Delete', onClick: () => handleDelete(student), variant: "destructive" },
                  ]}
...
```typescript
const columns = useMemo(() => getColumns(dictionary, lang, callbacks), [dictionary, lang, callbacks]);
```
...
└─────────┘ └─────────┘        │ │ Ahmed (10A)             │ │
    ↓           ↓              │ │ 3 classes | 92% attend  │ │
 (nothing connects them)       │ │ [→ Grades] [→ Classes]  │ │
                               │ └─────────────────────────┘ │
                               └─────────────────────────────┘
```
...
<DropdownMenuItem asChild>
  <Link href={`/${locale}/attendance?studentId=${row.id}`}>
    ...
    {locale === 'ar' ? 'عرض الحضور' : 'View Attendance'}
  </Link>
</DropdownMenuItem>
```
