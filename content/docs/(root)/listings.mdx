...
// columns.tsx - Enable column filters with select options
{
  ...
  cell: ({ getValue }) => {
    ...
    const labels: Record<string, string> = {
      school: t.schoolWide,  // "School" / "مدرسة"
      class: t.classSpecific, // "Class" / "فصل"
      role: t.roleSpecific,   // "Role" / "دور"
    };
...
**Key Points:**
- Use **simplified labels** in dictionary: "School" not "School-wide", "مدرسة" not "على مستوى المدرسة"
...
// Arabic
{
  "schoolWide": "مدرسة",        // Not "على مستوى المدرسة"
  "classSpecific": "فصل",       // Not "فصل محدد"
  "roleSpecific": "دور"         // Not "دور محدد"
}
```
...
```typescript
// ✅ PREFERRED: Column filters (built into DataTable)
{
  accessorKey: "scope",
  meta: {
    variant: "select",
...
// ❌ AVOID: Custom toolbar dropdowns (duplicates column filters)
...
// ✅ CORRECT: "View", "Edit", "Delete"
// ❌ AVOID: "View Student", "Edit Student", "Delete Student"
```
...
export default async function Layout({ children, params }: Props) {
  ...
  // ✅ CORRECT: Useful production pages (not filter duplicates)
  ...
  // ❌ AVOID: Filter duplicates (use column filters instead)
