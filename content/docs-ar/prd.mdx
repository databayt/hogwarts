...
لا تستخدم Hogwarts أعمدة ثنائية اللغة. كل المحتوى يُخزَّن بلغة المدرسة المختارة مع حقل `lang`. القراءات بلغة أخرى تمرّ عبر Google Translate وتُخزَّن مؤقتاً في `Translation`. نصوص الواجهة في القواميس (`src/components/internationalization/dictionaries/`)، و`getText()` (`@/components/translation/display`) هو الوصول القانوني.
