---
title: الترجمة
description: i18n من طبقتين — JSON قاموس للواجهة الثابتة، وgetText للمحتوى الديناميكي. فجوات التغطية وأنماط التبني.
---

تعمل الترجمة على طبقتين: ملفات JSON القاموس للواجهة الثابتة (متزامنة بين `ar.json`/`en.json`) و`getText()` لمحتوى قاعدة البيانات الديناميكي عبر Google Translate + تخزين مؤقّت في DB. البنية التحتية متينة؛ التبنّي جزئي — آلاف السلاسل الإنجليزية المكتوبة يدوياً ما زالت موجودة عبر مئات ملفات لوحة المدرسة. شغّل `/i18n-check` للعدد الحيّ.
...
| 2 — محتوى ديناميكي | إعلانات، مواد، أقسام، فعاليات                 | تخزين بلغة واحدة → `getText()` → Google Translate مع كاش |
