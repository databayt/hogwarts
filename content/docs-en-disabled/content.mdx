...
225 content.tsx files audited across all route groups:
...
**Server content components (~111 files):** Async functions that fetch data via `db`/`queries.ts`, pass pre-computed data to client sub-components. No `"use client"` directive.

**Client content components (~114 files):** `"use client"` directive, use hooks and state, fetch data via server actions from `actions.ts`. Handle user interaction directly.
...
> **Streaming tip:** Server content components can use `<Suspense>` to stream independent data blocks. Wrap slow queries in their own async component and nest inside `<Suspense fallback={<Skeleton />}>` for progressive rendering.
...
> **React 19 tip:** Consider using `useActionState` (replaces `useFormState`) for form submissions and `useOptimistic` for optimistic UI updates instead of manual `useState` + `try/finally` patterns.
...
interface Props {
  dictionary?: Dictionary["school"] // TODO: type to Dictionary["onboarding"]
}
