...
description: Full K-12 school simulation with 17 seeding phases, single-language storage, and global catalog
---
...
> **CRITICAL:** Never run `pnpm db:seed` directly. Always use `pnpm db:seed:single <name>` to seed individual modules. Full seed takes 45-120 minutes and is reserved for manual-only complete resets. See [CLAUDE.md rule #8](/docs/claude-code).
...
| Catalog Subjects    | 62      | Platform-wide (no schoolId)             |
| Catalog Chapters    | 201     | Organized by subject                    |
| Catalog Lessons     | 986     | ClickView-sourced content               |
...
## Execution Phases (17 Total)

The seed runs in dependency order:

```
PHASE 0: GLOBAL CATALOG (Platform-Wide)
├── Catalog Subjects (62)
├── Catalog Chapters (201)
├── Catalog Lessons (986)
└── Catalog Images (S3/CloudFront)
...
├── catalog.ts            # Global catalog (CatalogSubject/Chapter/Lesson)
├── clickview-catalog.ts  # ClickView-sourced catalog data
├── clickview-images.ts   # ClickView banner images
├── clickview-lesson-images.ts # ClickView lesson images
├── catalog-images.ts     # Catalog image management
├── academic-structure-catalog.ts # Catalog bridge setup
...
└── single.ts             # Single-seed runner (pnpm db:seed:single)
```
...
```bash
# Seed a single module (recommended)
pnpm db:seed:single <name>
...
# Full reseed (45-120 min - manual only)
pnpm db:seed
```
