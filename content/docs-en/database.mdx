...
- **Single-language storage** — content stored in one language with a `lang` field; translation on-demand via Google Translate cached in `TranslationCache`.
...
| Academic structure                              |     13 | `academic-structure.prisma`, `subjects.prisma`, `classrooms.prisma`, `enrollment.prisma`                                                                                                               |
...
| Audit / security / misc                         |     10 | `audit.prisma`, `legal.prisma`, `webhooks.prisma`, `task.prisma`, `translation-cache.prisma`, `purge-tokens.prisma`                                                                                    |
...
- **Catalog globals** (22+): `Subject`, `Chapter`, `Lesson`, `Book`, `Textbook`, `CurriculumStandard`, etc. — platform-wide reference content; bridge models scope per-school selections.
...
| **Catalog** | Platform-wide (no `schoolId`) | `Subject`, `Chapter`, `Lesson`, `Material`, `Question`                                    | ClickView / curated    |
...
Content stored in one language with a `lang` field. Translation on-demand via Google Translate API, cached in `TranslationCache`.
