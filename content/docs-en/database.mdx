...
> **`prisma migrate deploy` is the wrong tool against production, and this page used to
> recommend it.** Verified 2026-08-18: the production database has **no `_prisma_migrations`
> table** — it is `db push`-managed — so `migrate deploy` would try to apply all 38 repo
> migrations from scratch onto a schema that already exists. Do not run it against prod.
...
Read what it prints, apply just the statements you intend, then re-run the diff to confirm it
comes back empty. Adding a nullable column with no default is a catalog-only change in
Postgres — instant, no table rewrite, existing rows untouched.
