...
- **Degradation observability.** Still only a throttled `console.error` (`reportTranslationDegraded`). Add a public signal — a getter or `/api/health` field — so a dashboard can _see_ when `/en` is silently falling back to source.
