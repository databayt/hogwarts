...
`localize` collapses what used to be **N×M `getText` lookups** into a **single**
...
| You have…                                  | Use                                                 | From              |
| ------------------------------------------ | --------------------------------------------------- | ----------------- |
| **A list/entity of a content model**       | **`localize` / `localizeOne`** _(batched, default)_ | **`localize.ts`** |
...
- **Adopt `localize()` everywhere.** ✅ The batched engine is shipped (`localize.ts` + `registry.ts`): one `findMany` per page instead of N×M `findUnique`. Announcements is migrated; rolling the rest of the ~24 features onto `localize` (and registering their models) is the remaining adoption work. Extend the audit ratchet to flag _any_ registered field rendered raw (not just person-names).
- **Cache lifecycle.** ✅ An in-memory LRU glossary for hot terms now fronts the DB cache (`memory-cache.ts`, 0 round-trip on hit). Still pending: TTL eviction for the `Translation` table (the `lastAccessedAt` index exists but is unused for cleanup) and exact-`sourceText` normalization.
- **Pre-warm on write.** ✅ `prewarm()` (`prewarm.ts`) translates the other language off the response path via `after()`, so reads are cache hits — no first-view latency, no source-language flash. Wired into announcements; rolling it into the remaining create/update paths is the rest of the work.
