...
92 total card files audited across the codebase (27 card.tsx + 71 \*-card.tsx, ~3,755 lines in card.tsx):
...
Reference: `atom/lab/*-card.tsx` (30+ variants in the flat `atom/lab/` directory)
    ...
    <Card className="transition-shadow hover:shadow-md">
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          <small className="text-muted-foreground">Requested</small>
          <div className="font-medium">{formatTimeSince(domain.createdAt)}</div>
        </div>
        {domain.notes && (
          <div>
            <small className="text-muted-foreground">Notes</small>
            <p className="text-muted-foreground line-clamp-2">{domain.notes}</p>
          </div>
        )}
      </CardContent>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        {/* Note: Use semantic HTML (<h2>, <h3>) instead of hardcoded text-* classes */}
...
> **Semantic HTML:** Per project convention, prefer semantic HTML elements (`<h2>`, `<h3>`, `<p>`) over hardcoded `text-2xl font-bold` classes. Import `typography` from `@/lib/typography` for consistent text styling.
...
Many cards are pure data display and can be **server components**. Only add `"use client"` when the card has:

- `onClick` handlers or interactive callbacks
- `showActions` with button event handlers
- Client state (hover state tracking, animation state)

### Server Component Cards (Preferred)

Cards that only display data don't need `"use client"`:

```typescript
// GOOD: Server component card (no "use client")
import { Badge } from "@/components/ui/badge"
import {
  Card,
  CardContent,
...
import { formatCurrency, getInvoiceStatusLabel } from "./util"

export function InvoiceCard({ invoice }: { invoice: InvoiceWithSchool }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>{invoice.stripeInvoiceId}</CardTitle>
        <CardDescription>{invoice.school.name}</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="font-medium tabular-nums">
        ...
        <Badge>{getInvoiceStatusLabel(invoice.status)}</Badge>
      </CardContent>
    </Card>
  )
}
...
### When Cards Need Client

Add `"use client"` only for interactive cards:

```typescript
// Client card -- has onClick callbacks
"use client"
...
### Split Pattern for Mixed Cards

When a card file has both display-only and interactive variants, consider splitting:

```
# Display cards as server components
card.tsx           # InvoiceCard, InvoiceStatsCard (server, display-only)
card-actions.tsx   # InvoiceActionCard (client, with onClick handlers)

# Or keep together with "use client" if all variants need interactivity
card.tsx           # "use client" -- all variants have callbacks
```
