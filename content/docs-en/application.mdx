...
| 4   | Academic    | applyingForClass — the school's own grades (the static KG1–12 list narrowed by `getSchoolGradeNumbers`; the full list only when the school has defined none)                                                   | `/{lang}/application/{id}/academic`    |
...
The guardian tab collects each parent's name, phone and **WhatsApp**; it collects no email. `Application` has only `fatherEmail` / `motherEmail` / `guardianEmail` to hold the third value, so for every online applicant those columns carry a phone number. The columns are not renamed (no DDL); the read side classifies by shape — `confirmEnrollment` hands a non-address value to `createOrLinkGuardian` as `whatsapp` (a second phone row), and the admin detail page labels it "WhatsApp". Before 2026-09-05 the number was written into `Guardian.emailAddress`, the enrollment mail tried to deliver to it, and a father and mother who shared a household number collapsed into one guardian row.
