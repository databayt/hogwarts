...
### User Flow Distinction (CRITICAL)

The login behavior differs based on **where** the user logs in from and **what action** they took:
...
### Login Context Implementation

The login form passes context parameters to determine post-login redirect:

```typescript
// Login URL from SaaS marketing
/login?context=saas

// Login URL from school marketing
/login?context=school&subdomain=demo
```

The login action uses these parameters:

- **No callbackUrl** → Stay on current marketing page (SaaS or school)
- **callbackUrl=/onboarding** → Go to onboarding (only from "Get Started")
- **callbackUrl=/dashboard** → Check membership, allow or deny

### Login Flow by User Type

| User Type    | Starting Point          | Action        | Post-Login Redirect               |
| ------------ | ----------------------- | ------------- | --------------------------------- |
| DEVELOPER    | `ed.databayt.org`       | Login button  | Stay on SaaS marketing            |
| DEVELOPER    | `ed.databayt.org`       | Get Started   | `/onboarding`                     |
| DEVELOPER    | Any                     | Platform link | Dashboard (always allowed)        |
| School User  | `ed.databayt.org`       | Login button  | Stay on SaaS marketing            |
| School User  | `{school}.databayt.org` | Login button  | Stay on school marketing          |
| School User  | `{school}.databayt.org` | Platform link | `{school}.databayt.org/dashboard` |
| Fresh User   | `ed.databayt.org`       | Login button  | Stay on SaaS marketing            |
| Fresh User   | `ed.databayt.org`       | Get Started   | `/onboarding` (create school)     |
| Fresh User   | `{school}.databayt.org` | Login button  | Stay on school marketing          |
| Fresh User   | `{school}.databayt.org` | Platform link | `/access-denied` (not a member)   |
| Wrong School | `{other}.databayt.org`  | Platform link | `/access-denied` (wrong school)   |
...
JWT created with: { id, email, role, schoolId }
        │
        ▼
Redirect callback (login/action.ts)
        │
        ├── Has callbackUrl=/onboarding? → /onboarding (Get Started flow)
        │
        ├── Has callbackUrl=/dashboard?
        │   │
        │   ├── On subdomain + user is member? → School dashboard ✓
        │   ├── On subdomain + user not member? → /access-denied ✗
        │   ├── On main domain + DEVELOPER? → SaaS dashboard ✓
        │   └── On main domain + not DEVELOPER? → /access-denied ✗
        │
...
These accounts have no `schoolId` - they operate at platform level:
...
> **Tenant Isolation**: If `admin@databayt.org` tries to access `other.localhost:3000`, access is denied because their `schoolId` doesn't match.

### Bulk Accounts

These accounts are tied to the demo school:
...
| Onboarding Flow     | `user@databayt.org`       | `localhost:3000`                | Click "Get Started" → onboarding   |
| RBAC - Admin Routes | `teacher@databayt.org`    | `demo.localhost:3000`           | Blocked from `/admin`, `/settings` |
| RBAC - Finance      | `accountant@databayt.org` | `demo.localhost:3000`           | Full access to finance routes      |
| Cross-subdomain SSO | Any school user           | Login on main, access subdomain | Auto-authenticated                 |
| Tenant Isolation    | `admin@databayt.org`      | `other.localhost:3000`          | Access denied (wrong school)       |

### Resetting Test User

The `user@databayt.org` account should remain fresh (no schoolId, USER role) for onboarding tests.

After testing modifies the account, reset it:
