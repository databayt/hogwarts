...
**Key Stats**: ~220 tenant-scoped models across 264 total, 132 enums, 8 user roles, 4 entry points, 58 error boundaries, 537-line rate limiting system.

## Comparison: Hogwarts vs Vercel Platforms

| Feature             | Vercel Platforms             | Hogwarts                                                           | Status      |
| ------------------- | ---------------------------- | ------------------------------------------------------------------ | ----------- |
| Subdomain Routing   | Middleware                   | `proxy.ts` (388 lines)                                             | Implemented |
| Preview URLs        | `tenant---branch.vercel.app` | Implemented                                                        | Match       |
| Tenant Data Store   | Redis (Upstash)              | PostgreSQL + In-Memory + Upstash Redis (custom domains)            | Implemented |
| Custom Domains      | Vercel Domains API           | DnsService (644 lines) + `resolveCustomDomain()` via Upstash Redis | Partial     |
...
| Rate Limiting       | Upstash Ratelimit            | In-memory (537 lines, 14 configs)                                  | Partial     |
| Error Monitoring    | Sentry/Datadog               | Sentry configured, DSN empty                                       | Gap         |
| Security Headers    | Manual                       | CSP + HSTS + XSS (173 lines)                                       | Implemented |
...
| `src/lib/tenant-context.ts`   | 189   | Tenant resolution — single source of truth with caching                    |
| `src/auth.ts`                 | 777   | Auth configuration — JWT, cookies, redirect callback (split from monolith) |
| `src/auth.config.ts`          | 151   | Auth configuration — providers, JWT strategy                               |
| `src/routes.ts`               | 410   | RBAC matrix — role-based route protection                                  |
| `src/lib/rate-limit.ts`       | 537   | Rate limiting — 14 configs, per-process storage                            |
| `src/lib/security-headers.ts` | 173   | Security headers — CSP, HSTS, XSS protection                               |
| `src/lib/dns-service.ts`      | 644   | DNS verification — multi-provider support                                  |
| `src/lib/catalog-setup.ts`    | 366   | Catalog bridge setup — platform-wide content                               |
...
| Category         | Models (representative)                                                      | Count | schoolId | Status                    |
| ---------------- | ---------------------------------------------------------------------------- | ----- | -------- | ------------------------- |
| **Core**         | School, User                                                                 | 2     | Present  | Compliant                 |
| **People**       | Student, Guardian, Teacher, Staff, StaffMember + child models                | ~30   | Present  | Compliant                 |
| **Academic**     | Class, Subject, Department, YearLevel, AcademicLevel, Grade, Stream          | ~15   | Present  | Compliant                 |
| **Catalog**      | CatalogSubject, CatalogChapter, CatalogLesson, CatalogExam + bridge models   | 9     | Missing  | By design (platform-wide) |
| **Attendance**   | 30+ models across basic + enhanced attendance                                | ~30   | Present  | Compliant                 |
| **Exams**        | QBank, certificates, marking, rubrics                                        | ~25   | Present  | Compliant                 |
| **Finance**      | ChartOfAccount, JournalEntry, Invoice, Payment, Fee, Budget (8 prisma files) | ~40   | Present  | Compliant                 |
| **Messaging**    | Conversation, Message, Participant, Attachment, etc.                         | 12    | Present  | Compliant                 |
| **Admission**    | Lead, Application, Campaign, AdmissionConfig, etc.                           | 9     | Present  | Compliant                 |
| **Stream (LMS)** | StreamCourse, StreamCategory, StreamEnrollment, etc.                         | 8     | Present  | Compliant                 |
| **Other**        | Notifications, Files, Events, Library, etc.                                  | ~20   | Present  | Compliant                 |
| **Auth Tokens**  | Account, VerificationToken, PasswordResetToken, TwoFactorToken               | 4     | Missing  | Acceptable                |

### Models WITHOUT schoolId (~44 models)

#### Auth Tokens (4 models)
...
#### Catalog Global Models (7+ models)

| Model               | Reasoning                                                   | Risk |
| ------------------- | ----------------------------------------------------------- | ---- |
| `CatalogSubject`    | Platform-wide educational content shared across all schools | None |
| `CatalogChapter`    | Platform-wide educational content shared across all schools | None |
| `CatalogLesson`     | Platform-wide educational content shared across all schools | None |
| `CatalogExam`       | Platform-wide educational content shared across all schools | None |
| `CatalogQuestion`   | Platform-wide educational content shared across all schools | None |
| `CatalogMaterial`   | Platform-wide educational content shared across all schools | None |
| `CatalogAssignment` | Platform-wide educational content shared across all schools | None |

Catalog models are intentionally global -- they represent platform-wide educational content shared across all schools. Schools access this content via bridge models (e.g., `CatalogBridge`) that are scoped by `schoolId`.

#### Subscription Tiers (1 model)

| Model              | Reasoning                                  | Risk |
| ------------------ | ------------------------------------------ | ---- |
| `SubscriptionTier` | Platform-wide pricing, not tenant-specific | None |
...
**Upstash Redis** is now partially used in production: `proxy.ts` uses `resolveCustomDomain()` to look up custom domain mappings via Upstash Redis at the edge. For general tenant resolution (subdomain-to-schoolId), caching remains in-memory only — see [Issue #4](/multi-tenant-issues.md#4-in-memory-tenant-cache-not-shared-across-serverless-instances).
...
pnpm db:seed:single users   # Re-seed users (or other module)
```
...
- [x] `schoolId` on ~220/264 models (~44 without, including auth tokens + global catalog)
...
- [x] Rate limiting (14 configs, 537 lines)
