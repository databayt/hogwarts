...
// Rule 2: Import row type from types.ts, not defined inline
...
> **Streaming tip:** Wrap server-rendered table content in `<Suspense fallback={<TableSkeleton />}>` for progressive loading. The table skeleton should match the table layout to prevent layout shift.
...
table.tsx and columns.tsx are always client components -- TanStack Table requires hooks, and column cell renderers use JSX. However, **data must always be fetched on the server**.

### Data Flow

```
page.tsx (server) → content.tsx (server, fetches data) → client.tsx ("use client") → table.tsx
```

Never fetch data inside table.tsx. Data comes from the server via props:

```typescript
// GOOD: Server-fetched data via props
...
export function TeachersTable({ initialData, total }: TeachersTableProps) {
  // Table receives pre-fetched data
  const { table } = useDataTable({ data: initialData, columns, pageCount })
  return <DataTable table={table} />
}

// BAD: Fetching inside table.tsx
...
The one acceptable case for server action calls in table.tsx is incremental loading (load-more pagination), where the initial data comes from the server and subsequent pages are loaded on demand:

```typescript
const handleLoadMore = useCallback(async () => {
  const result = await getItems({ page: nextPage }) // OK for pagination
  setData((prev) => [...prev, ...result.data])
}, [nextPage])
```
...
table.tsx should be imported by client.tsx, which manages modal state and callbacks:

```
content.tsx (server, fetches data)
    → client.tsx ("use client", modal state, callbacks)
        → table.tsx (client leaf, renders DataTable)
        → columns.tsx (client leaf, column definitions)
```
...
> **URL state:** This project uses [nuqs](https://nuqs.47ng.com/) for URL-based search params. Table filters, sorting, and pagination should sync to URL params via `createSearchParamsCache()` for shareable links and browser back/forward support.
