...
name, then the panel. The status row and the edit button go full width, section
headings step up a size, and the year rail becomes a dropdown on the graph
heading. The feed shows six entries with the rest behind **Show more activity**
— the server caps it at ten, so nothing is promised that is not already loaded.
