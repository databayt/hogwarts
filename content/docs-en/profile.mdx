...
> **Deploy note:** `profile_badges`, `organizations`, and
> `organization_memberships` are **deploy-pending**. Apply the additive
> migration-of-record
> `prisma/migrations/20260615000000_add_profile_badges_organizations/` to the
> prod default branch (idempotent `CREATE TABLE IF NOT EXISTS` + guarded enums,
> validated on a Neon branch), then run `pnpm db:seed:single profile-extras`.
