...
| Area                                               | Status | Detail                                                              |
| -------------------------------------------------- | ------ | ------------------------------------------------------------------- |
| Standard props `{ data, error, dictionary, lang }` | PASS   | 10/12 files follow the standard prop pattern                        |
...
Reference: `saas-dashboard/tenants/detail.tsx`, `saas-dashboard/catalog/detail.tsx`
...
> **Note:** Prefer `<Link href="...">` for navigation over `router.back()` for better predictability and server-side rendering compatibility.
...
detail.tsx can often be a **server component**. Most detail views render data without interactivity. Only add `"use client"` when you need:

- `useRouter()` for back navigation (prefer `<Link href={parentRoute}>` instead)
- Click handlers on action buttons
- Client state (tabs with URL params can use server-side rendering)

### Prefer Server Components

Before adding `"use client"` to a detail.tsx, check:

```typescript
// GOOD: Server component detail (no "use client" needed)
import Link from "next/link"
import { ArrowLeft } from "lucide-react"

import { Button } from "@/components/ui/button"

export function AnnouncementDetail({ data, error, lang }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pure data display -- no hooks needed */}
    </div>
  )
}

// BAD: Client component just for router.back()
"use client"
import { useRouter } from "next/navigation"

export function AnnouncementDetail({ data }: Props) {
  const router = useRouter()
  return (
    <Button onClick={() => router.back()}>Back</Button> // Use <Link> instead
  )
}
```

### When Client Detail Is Necessary

Use `"use client"` detail only when the component needs:

| Need                          | Solution                                      |
| ----------------------------- | --------------------------------------------- |
| Back navigation               | `<Link>` (server) or `router.back()` (client) |
| Action buttons (edit, delete) | Callbacks from client.tsx                     |
| Tab state                     | URL-based tabs (server) or useState (client)  |
| Interactive content (video)   | Extract to client sub-component               |

### Boundary Placement

When detail needs interactivity, import it from client.tsx:

```
content.tsx (server, fetches entity)
    → client.tsx ("use client", action handlers)
        → detail.tsx (client leaf, renders view)
```

When detail is pure display, content.tsx can render it directly:

```
content.tsx (server, fetches entity) → detail.tsx (server, renders view)
```

## Quick Reference

> **Loading:** Use `loading.tsx` in the route segment for page-level loading states. For component-level loading within detail views, use `<Suspense fallback={<DetailSkeleton />}>`.
