...
1. **Install.** On Android Chrome open `https://<school>.balqalam.com`, sign in, and a welcome sheet slides up offering the app — tap **Continue**: Android opens the install sheet; iPhone opens the share sheet, where you pick *Add to Home Screen* (a two-step guide appears underneath). The icon carries the school's name in Arabic, opens right-to-left in the school's colour, and has no browser chrome.
2. **Mark attendance offline.** Open *Attendance → Quick* for a section, put the phone in airplane mode, mark the absentees and save. The panel says the marks were saved on the device and the sync banner shows one pending item. Turn the network back on: the banner drains within seconds and the marks appear on the server; save the same section again and the server keeps whichever mark is newer. Guardians get the absence notice when the replay lands, not before.
3. **Turn on push.** *Notifications → Preferences → Push* asks the browser's permission on the tap and subscribes this device. From then on an absence, exam or live-class notice also arrives as a system notification; tapping it opens that page in the installed app. On iPhone the toggle only works from the installed app (Safari cannot receive Web Push).
...
### Shipped 2026-09-13 — explorable offline, and fast on thin networks

A live measurement of `kingfahd` on 4G explained the slow menu: every dashboard open shipped 1.38 MB of HTML (935 KB of it the dictionary), 63 script chunks, 1 MB of CSS from marketing-site clones, 36 prefetch requests and **two 1.1 MB Server Action responses** — because the proxy wrote the locale cookie on every response, and Next answers any Server Action whose request carries a proxy-set cookie with a full re-render of the page. And no static file was cached at Cloudflare's edge: each chunk round-tripped to the single container, ~0.5 s apiece from Europe. All of it is fixed: the cookie is written only when it changes, the Worker serves hashed chunks, fonts and optimised images from the edge, the clone stylesheets load only on the pages that use them, every sidebar route has a loading boundary so its link is prefetched and a tap draws the skeleton at once, and pages visited in the last 30 s come back from memory.

**Offline is no longer a dead end.** Service worker v7 keeps every screen a signed-in user opens — the HTML of a full load and the payload of a client-side navigation — in a cache named by a per-user session key the server sends with each response. A different key, or none, empties the previous person's pages, and the sign-out button and the sign-in and join pages tell the worker to forget everything before they move on, so a shared device can only ever replay the last signed-in person's own screens — even on a network too slow to have answered yet. With no connection, an opened page renders as last seen and the blue strip says so; an unopened page shows the offline page. When the network is merely slow (no answer within four seconds), the saved copy appears with a "slow connection — saved copy" strip and a Retry. Every dashboard page opened on a good connection is also kept as a full page, once a day, so it opens offline from any other page. The installed app also saves the whole sidebar in the background — once a day, on 4G, with data saver off, after the screen is idle — so the menu can be explored at a bus stop.

#### How to experience it

1. Open the installed app on a good connection, sign in, and leave the dashboard on screen for a minute.
2. Airplane mode. Tap through the sidebar: the pages you opened — and, from the installed app, the rest of the menu — render as they last were, under the strip "You're offline. Pages you've opened before still work…". Anything you mark is queued as before.
3. Throttle instead of cutting (a weak-signal room): a page that takes longer than four seconds to answer shows its saved copy at once with the slow-connection strip; Retry asks the server again.
4. Sign out and sign in as another account on the same phone: the first account's saved pages are gone before the second dashboard renders.
