...
**Offline is no longer a dead end.** Service worker v6 keeps every screen a signed-in user opens — the HTML of a full load and the payload of a client-side navigation — in a cache named by a per-user session key the server sends with each response. A different key, or none, empties the previous person's pages, so a shared device can only ever replay the last signed-in person's own screens. With no connection, an opened page renders as last seen and the blue strip says so; an unopened page shows the offline page. When the network is merely slow (no answer within four seconds), the saved copy appears with a "slow connection — saved copy" strip and a Retry. The installed app also saves the whole sidebar in the background — once a day, on 4G, with data saver off, after the screen is idle — so the menu can be explored at a bus stop.
