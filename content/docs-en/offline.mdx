...
- **Network first.** A page with a saved copy shows it when the network fails, or has not answered within four seconds, and the request keeps running to refresh the copy. The strip says which: "You're offline…" or "Slow connection — showing the last saved copy of this page", with Retry. A toast says the same wherever the page is scrolled: "You're offline" stays until the connection returns, then "You're back online"; a slow connection shows "Your connection is slow" for five seconds.
...
| Offline: dashboard → a page opened before           | Saved page in 283 ms, under the offline strip         |
| Offline: a page never opened                        | "This page isn't on this device yet"; Back returns    |
| Slow: reload a saved page                           | Saved copy at 4.0 s, slow-connection strip with Retry |
...
3. On a weak signal rather than none, a saved page that takes longer than four seconds shows its saved copy under the slow-connection strip; **Retry** asks the server again.
...
| `src/components/offline/sync-banner.tsx`        | The offline and slow-connection strip                                                      |
