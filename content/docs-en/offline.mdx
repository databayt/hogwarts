...
### Shipped 2026-09-12 — installable, and attendance in the outbox

The PWA layer was repaired after a live audit of `demo` and `kingfahd`: the manifest is now per-tenant (school name, Arabic RTL by default, the school's brand colour), the missing icons exist, and service worker v3 precaches `/ar/offline` and `/en/offline` instead of two redirects — so the offline fallback works for the first time — and no longer caches authenticated pages or API bodies. **Quick attendance is the first non-lesson outbox kind**: a mark made offline is saved on the phone and replayed through the same ownership check and `markAttendance` path when the connection returns; a newer mark already on the server wins. The "Teacher marking attendance offline" row above is now ✓. Verify any host with kun's `pwa-audit` script.

Two more pieces landed the same day: **Web Push** (a browser or the installed app subscribes from the notification preferences page; absence, exam and live-class notices arrive as system notifications and a tap opens the school's own route) and an **install card** on phone-sized dashboards that offers "Add to Home Screen" (Android shows the real install prompt; iPhone gets the Share → Add to Home Screen hint).

#### How to experience it (any school, today)

1. **Install.** On Android Chrome open `https://<school>.balqalam.com`, sign in, and a welcome sheet slides up offering the app — tap **Continue**: Android opens the install sheet; iPhone opens the share sheet, where you pick _Add to Home Screen_ (a two-step guide appears underneath). The icon carries the school's name in Arabic, opens right-to-left in the school's colour, and has no browser chrome.
2. **Mark attendance offline.** Open _Attendance → Quick_ for a section, put the phone in airplane mode, mark the absentees and save. The panel says the marks were saved on the device and the sync banner shows one pending item. Turn the network back on: the banner drains within seconds and the marks appear on the server; save the same section again and the server keeps whichever mark is newer. Guardians get the absence notice when the replay lands, not before.
3. **Turn on push.** _Notifications → Preferences → Push_ asks the browser's permission on the tap and subscribes this device. From then on an absence, exam or live-class notice also arrives as a system notification; tapping it opens that page in the installed app. On iPhone the toggle only works from the installed app (Safari cannot receive Web Push).
4. **Lose the connection anywhere.** In airplane mode, opening any app route shows the school's offline page in the right language — with the outbox view listing what is queued — instead of the browser's error screen.

What is verified by tooling: the install audit passes on `demo` and `kingfahd` (0 FAIL), the worker controls the page after one reload, the outbox replay returns `applied` then `duplicate`, and the push cron reports the channel configured. What only a phone can verify: that a push actually shows on a device and that the offline page appears under a real disconnect — both tracked in [hogwarts#408](https://github.com/databayt/hogwarts/issues/408).

### Shipped 2026-09-13/14 — explorable offline, and fast on thin networks

A live measurement of `kingfahd` on 4G explained the slow menu. Every dashboard open shipped 1.38 MB of HTML, 935 KB of it the dictionary, plus 63 script chunks, 1 MB of CSS from marketing-site clones, 36 prefetch requests and **two 1.1 MB Server Action responses**. The proxy wrote the locale cookie on every response, and Next answers any Server Action whose request carries a proxy-set cookie with a full re-render of the page. No static file was cached at Cloudflare's edge either: each chunk round-tripped to the single container, about 0.5 s apiece from Europe.

What changed for speed:

- **The locale cookie is written only when it changes**, so a Server Action answers with its own data again.
- **The Worker serves hashed chunks, fonts, icons and optimised images from Cloudflare's edge.** It asks the container for uncompressed bytes and lets the edge compress for each visitor, keys optimised images by the format the browser accepts, and stamps every static response `x-edge-cache: hit | miss | bypass`.
- **The clone stylesheets load only on the pages that use them.**
- **Every sidebar route has a loading boundary**, so its link is prefetched and a tap draws the skeleton at once. Pages visited in the last 30 s come back from memory.
- **The header's messages link prefetches on hover, touch or focus**, not on sight: its route group carried a 684 KB dictionary.
- **The notification bell backs off for 15 s after a failed request.** With the server unreachable it had been sending bursts of about 120 requests.

**How offline works now (service worker v7):**

- **Saved per person.** Every screen a signed-in user opens is kept in a cache named by a per-user session key the server sends with each response. A response carrying a different key, or none, empties the previous person's pages. The sign-out button, and the sign-in and join pages as they open, tell the worker to forget everything first: on a slow network a saved copy can appear before any response reveals who is signed in.
- **Saved as whole pages.** A client-side navigation leaves no HTML behind, so each dashboard page opened on a good connection (4G-class, data saver off) is also saved as a full page, once a day. The installed app also saves the whole sidebar once a day. A navigation's payload is saved too, but it only answers the exact navigation it came from.
- **Network first.** A page with a saved copy shows it when the network fails, or has not answered within four seconds, and the request keeps running to refresh the copy. It does so silently: no strip, no toast, no "offline", "slow" or "back online" notice (#416). When the connection comes back, a page showing its saved copy refreshes itself, and queued work drains on its own. The only strip left is work the server rejected — "N items need attention", linking to `/offline`.
- **Unopened pages say so.** A page with no saved copy shows "This page isn't on this device yet" with Back and Retry; an installed iPhone app has no back button of its own. The offline page and the files it needs live in their own cache, refreshed daily, so the fallback survives deploys.
- **Lessons open offline; their videos do not.** A saved lesson page holds text, quiz questions (never the answers) and a reference to the video — never the video, a recording or a material. Those stay behind `/api/lumos/*`, which the worker never caches, and the video address refuses anything but the in-app player. This is the 2026-08-30 school policy (videos and materials are watched in the app, never copied to a device), and it is why a PRIVATE video cannot outlive its owner's choice on someone's phone.
- **The installed app cannot block screenshots.** An installed PWA is still the browser: no web API stops or detects a screen capture. Every video, recording and the live room carry the viewer's code in a 1% watermark that survives a capture, plus a visible mark on capture signals — see [Lumos → protection](/docs/lms). Real blocking needs a native shell ([hogwarts#411](https://github.com/databayt/hogwarts/issues/411)).

**Corrected on 2026-09-14.** The first version of this section passed a test that replayed navigations the same tab had just made. With the server actually stopped, three things failed, and a fourth gap showed on a slow network:

| Found                                                                     | Cause                                                                                                                    | Fix                                                                                 |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| A sidebar click did nothing offline                                       | Next's experimental `useOffline` was on; its scheduler sends no requests while offline, the likely cause                 | The flag is off, so every click reaches the worker                                  |
| The dashboard stayed on screen under the Teachers address                 | A payload saved on students → teachers was replayed on dashboard → teachers                                              | Saved payloads are keyed with Next's `_rsc`, which names the router tree they patch |
| Pages opened by a click had no offline copy                               | An offline click never repeats an online `_rsc`: a prefetched route sends a partial tree, an offline click the whole one | Each opened page is saved as HTML                                                   |
| On a slow network the next person could see the previous one's saved page | The worker learned about a new session only from a response                                                              | Sign-out and the sign-in and join pages clear saved pages first                     |

**Measured** on a local production build — offline checks with the server stopped, slow checks behind a proxy that holds every response for 5 s:

| Check                                               | Result                                                |
| --------------------------------------------------- | ----------------------------------------------------- |
| Dashboard open: each of two Server Action responses | 1.1 MB before, about 1 KB after                       |
| Dashboard CSS                                       | 1,078 KB before, 589 KB after                         |
| Tap Students → skeleton on screen                   | 58 ms                                                 |
| Offline: dashboard → a page opened before           | Saved page in 283 ms (strip removed since, #416)      |
| Offline: a page never opened                        | "This page isn't on this device yet"; Back returns    |
| Slow: reload a saved page                           | Saved copy at 4.0 s (strip removed since, #416)       |
| Slow: sign out                                      | Saved pages gone before the sign-out request returned |
| Slow: the next account's first dashboard            | Waited 5.1 s for its own page                         |
| One saved dashboard page on the device              | 1,367 KB                                              |

#### How to experience it

1. Open the app on a good connection, sign in, and open a few sidebar pages.
2. Airplane mode. Tap through the pages you opened: they render as last seen under "You're offline. Pages you've opened before still work…". A page you never opened shows "This page isn't on this device yet", and **Back** returns to where you were. Anything you mark is queued as before.
3. On a weak signal rather than none, a saved page that takes longer than four seconds shows its saved copy, silently; it refreshes itself when the connection returns.
4. Sign out and sign in as another account on the same phone: nothing the first account saved appears, even before the network answers.

To test from a computer, stop the server, or use a phone in airplane mode. The browser developer tools' Offline setting does not reach the service worker's own requests, so "offline" pages quietly load from the server.

#### Limits

- **Storage.** A saved dashboard page is about 1.4 MB, mostly the dictionary. The cap of 80 saved pages per person allows about 110 MB, far past the 25 MB average in _What success looks like_. Moving the dictionary out of the HTML fixes size and speed together.
- **Server load.** Saving opened pages costs one extra server render per page, per person, per day.
- **Session expiry** lands on the sign-in page, which also clears that same person's saved pages.
- **Script eviction.** Past 400 files the worker's static cache drops its oldest entries first, which can include scripts every page uses; a saved page whose scripts were dropped shows without them offline.
- **Delete account** signs out without clearing saved pages first; the session-key check clears them as soon as the network answers.

#### Files

| File                                            | Role                                                                                       |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `public/service-worker.js`                      | v7: saved pages per session key, offline shell, offline/slow reason, outbox wake, Web Push |
| `src/components/offline/warmup.tsx`             | Asks the worker to save the opened page and, from the installed app, the whole sidebar     |
| `src/components/offline/forget-saved-pages.tsx` | Clears saved pages at sign-out, sign-in and join                                           |
| `src/components/offline/sync-banner.tsx`        | Outbox drain triggers, silent refresh on reconnect, the "needs attention" strip             |
| `src/components/offline/content.tsx`            | The offline page, and the "not on this device yet" card                                    |
| `src/proxy.ts`                                  | Sends `x-session-key`; writes the locale cookie only when it changes                       |
| `cf/worker.js`                                  | Edge cache for static files                                                                |
...
| ----------------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Service worker          | `public/service-worker.js`                                                | v7: static assets cache-first; signed-in pages saved per session key; offline shell refreshed daily; `/api/` network-only; wakes a tab to drain the outbox |
| SW registration         | `src/components/providers/service-worker-provider.tsx`                    | Production only (`NEXT_PUBLIC_SW_DEV=1` for a local `next start`); registers at once when the page has already loaded                                      |
| Offline fallback page   | `src/components/offline/content.tsx`                                      | Outbox view; "not on this device yet" card with Back and Retry when served in place of an unsaved page                                                     |
...
- [ ] Dashboard offline: a page opened while online renders from its saved copy under the offline strip; a page never opened shows "This page isn't on this device yet" and Back returns. Test with airplane mode or the server stopped — the developer tools' Offline setting does not reach the worker's own requests
- [ ] Slow network, responses held longer than 4 s: a saved page shows its copy under the slow-connection strip, and Retry refreshes it
- [ ] Shared device: sign out, sign in as another account — no page saved by the first account appears, on a slow network too
...
| `public/service-worker.js`                                                | v7 shipped; the `syncOfflineForms()` stub is gone — the worker wakes a tab to drain the outbox |
