...
> **Note:** `src/lib/action-response.ts` has a `"use server"` directive, so client components must use `import type` (not value import) to reference `ActionResponse`.

- `ExtendedUser` -- canonical at `src/components/auth/user.ts`
- `PaginationParams` -- should be consolidated (currently defined independently in 10+ queries.ts files)
...
export type StudentWithRelations = Student & {
  guardian?: Pick<Guardian, "id" | "firstName" | "lastName" | "phone">
...
| PaginationParams                       | Single shared location (to be created)    | queries.ts               |
