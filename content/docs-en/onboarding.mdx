...
**Edge Proxy** (`src/proxy.ts`) extracts the subdomain and rewrites to tenant-scoped routes.
...
> **Note**: `HostValidationContext` is now an alias for `WizardValidationContext` from the Form Block.
...
```typescript
// host-validation-context.tsx (now re-exports from form block)
...
// Usage in layout
;<HostValidationProvider>{children}</HostValidationProvider>
...
- [Server Actions](/docs/actions) - Next.js 16 server actions
