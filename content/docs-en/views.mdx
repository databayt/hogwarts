...
> **Clarification:** Most views need `"use client"` for interactivity (tabs, filters, state). Pure display views that only compose server-rendered cards/sections can omit the directive.
...
> **React Compiler note:** When React Compiler is enabled, manual `useMemo` calls are unnecessary as the compiler auto-memoizes. Until then, use `useMemo` for expensive column/row computations.
...
> **URL state:** Prefer nuqs `useQueryState()` over `useState` for filter/tab state. This syncs state to URL params for shareable links and browser back/forward support.
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Quick overview of recent and important invoices</p>
        </div>
      </div>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
...
View components (all.tsx, featured.tsx) are client components because they manage filter/sort state. However, prefer server-side filtering when possible.

### Server-Side vs Client-Side Filtering

| Dataset Size      | Approach                                  | Pattern                                       |
| ----------------- | ----------------------------------------- | --------------------------------------------- |
| Large (100+ rows) | Server-side (preferred)                   | table.tsx with URL-based pagination + filters |
| Small (under 100) | Client-side (acceptable)                  | all.tsx with `useMemo` filtering              |
| Mixed             | Server for initial, client for refinement | Hybrid approach                               |

For large datasets, move filtering to the server and use table.tsx with server-side pagination instead of all.tsx with client-side filtering:

```typescript
// GOOD: Large dataset -- use server-side filtering via table.tsx
// content.tsx (server)
...
return <TeachersTable data={rows} total={count} />

// BAD: Large dataset with client-side filtering
// all.tsx (client) -- filtering 500+ items in the browser
const filtered = useMemo(
...
View components are leaf components imported by client.tsx:

```
content.tsx (server, fetches data)
    → client.tsx ("use client", tab state)
        → all.tsx (client leaf, filter/sort UI)
        → featured.tsx (client leaf, curated views)
```
