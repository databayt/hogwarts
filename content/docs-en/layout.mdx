...
55 layout.tsx files audited across 8+ route groups:

| Area                                 | Status | Detail                                                                                    |
| ------------------------------------ | ------ | ----------------------------------------------------------------------------------------- |
| Promise-based params                 | PASS   | All 46 async layouts correctly use `Promise<>`                                            |
| Provider hierarchy                   | PASS   | Clean nesting: root → [lang] → route group. No duplicates                                 |
| `export default function` convention | PASS   | 53/55 use function declarations                                                           |
...
> Note: The actual locale layout wraps 8 providers (DirectionProvider, SessionProvider, NuqsAdapter, ThemeProvider, UserThemeProvider, AnalyticsProvider, ServiceWorkerProvider, Toaster). This template shows the essential 5 for clarity.
  ...
  return (
    <SchoolProvider school={result.data}>
      <SidebarProvider>
        <ModalProvider>
          <PageHeadingProvider>
            {/* Header uses session for user display, Sidebar, children */}
          </PageHeadingProvider>
        </ModalProvider>
      </SidebarProvider>
    </SchoolProvider>
...
- 6 layouts in codebase follow this pattern: (listings), application, subjects, etc.
...
In Next.js 16, layouts persist across soft navigation and do NOT re-execute. However, they DO re-execute on hard navigation (full page reload). A DB query here means a database round-trip on initial load and hard navigations. Move the count to the specific page that displays it to keep layouts lean and side-effect-free.
...
| Shape            | Type                                  | Used by                                               |
| ---------------- | ------------------------------------- | ----------------------------------------------------- |
| None             | `{ children: React.ReactNode }`       | Root, passthrough, auth, onboarding, application      |
...
- React Compiler support is available (`reactCompiler: true` in next.config) but not currently enabled in this project
