...
Reference: `school-marketing/application/personal/actions.ts` (20 lines)
...
> **Tip:** Use `revalidateTag(tag)` for targeted cache invalidation when paths are insufficient. Tag-based revalidation is more granular than path-based.
...
> **Note:** `redirect()` is valid in server actions for navigation after mutations (e.g., after creating a resource, redirect to its detail page). It throws internally, so it must be called outside try/catch blocks.
...
### Consuming Actions in Client Components (React 19)

Use `useActionState` (replaces `useFormState`) to consume server actions in forms:
...
Use `useFormStatus` for nested submit button components:
