...
> **Note:** The templates below exceed the ~200 line guideline because they show complete real-world utilities for reference. In practice, split large util.ts files by concern (e.g., `formatters.ts`, `mappers.ts`, `validators.ts`).
...
> **Note:** `formatTimeSince` calls `new Date()` internally, making it non-deterministic. For testability, consider accepting a `now` parameter with `new Date()` as the default.
...
util.ts is naturally server-compatible -- pure functions with no React dependencies run on both server and client. This is the ideal pattern for maximum reusability.

### Server-Only Utilities

For utilities that must never be bundled to the client (e.g., they import server-only modules), use the `server-only` package:
...
util.ts files are imported by both server and client components. This makes them the ideal location for logic shared across the boundary:

```
content.tsx (server) → imports formatCurrency from util.ts
card.tsx (client)    → imports formatCurrency from util.ts
detail.tsx (server)  → imports formatCurrency from util.ts
all.tsx (client)     → imports sortItems from util.ts
```

Since util.ts has no React dependencies, it works identically on server and client with zero overhead.
...
> **Clarification:** Validation schemas belong in `validation.ts` (Zod schemas for input validation). Only pure formatting/mapping helpers belong in `util.ts`. See the architecture doc for the distinction.
