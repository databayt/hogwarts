...
> **Note:** `src/lib/validation.ts` doesn't exist yet — this is the target location for shared validation primitives. Currently, each feature defines its own email/phone/name schemas independently.
...
This project uses Zod 4.0.14 (major upgrade from Zod 3). Key differences:

- `z.object()` is stricter by default — unknown keys are stripped (same as `.strip()` in Zod 3)
- `z.string().email()` validates more strictly
- `.parse()` error format changed — use `error.issues` (not `error.errors`) for iteration
- Import is still `import { z } from "zod"` — no breaking import changes
- `z.infer<>` works identically
- `.refine()` and `.transform()` APIs unchanged
