...
| `push`   |   yes   | Web Push   | VAPID (`web-push`); subscribe on `/notifications/preferences`; iOS needs the installed app |
...
A browser — or the installed PWA — becomes a recipient from the preferences page: the toggle asks permission on a tap, subscribes with the public VAPID key, and stores the subscription tenant-scoped (`PushSubscription`). `process-push-notifications` drains `channels has push` rows through `src/lib/notifications/push-web.ts`; the worker (`public/service-worker.js`) shows `{title, body}` and a tap opens the absolute `url` on the school's own host. Subscriptions the push service reports gone (404/410) are deleted. On iPhone, Web Push reaches only an app added to the Home Screen, so the toggle says "install first" in Safari. See [Offline](/docs/offline) for the PWA shell.

Try it: on a phone (Android Chrome, or the installed app on iPhone) open *Notifications → Preferences*, tap the **Push** toggle and allow the browser's permission prompt — the row switches to "on" and a subscription row appears for this device. Have a teacher mark a child absent in quick attendance: within the next cron tick (every 15 minutes) the guardian's phone shows the absence as a system notification, and tapping it opens the notification in the school's app. Exam and live-class notices use the same channel.
...
Hooks expose the data — dual-channel (Socket.IO when available, 30 s polling fallback), with optimistic updates. An initial fetch always runs regardless of transport (a live socket only pushes _new_ events), and polled snapshots merge through `poll-merge.ts` — forward-only read-state sync, so a notification read in another tab clears here without fighting optimistic updates.

Every hook in a tab shares one bell request. A success is reused for 5 s. A failed or non-OK request starts a 15 s cooldown for all of them, and the shared fetch resolves `null` instead of rejecting, so a first load while offline no longer throws an unhandled rejection. Before the cooldown (2026-09-13), an unreachable server drew bursts of about 120 bell requests: a failure is instant, so each remount, focus and poll became its own request.
