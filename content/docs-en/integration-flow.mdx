...
| Classroom setup not in onboarding                | 2 --> 6   | Partial | Capacity step saves `maxClasses` count but creates no `Classroom` records. Post-onboarding, the Configure tab auto-provisions rooms and sections per grade.                                                                                     |
...
| `maxClasses` naming confusion                    | 2 --> 6   | Open    | Form says "Classrooms" but saves to `maxClasses` (limits `Class` records, not `Classroom` rooms). Less confusing now that the UI route is `/classrooms`.                                                                                        |
...
| No Academic Year Setup Wizard                    | 2 --> 7   | Open    | Schools must manually create new academic years and terms. No guided wizard for year rollover.                                                                                                                                                  |
| Stream-specific subjects not auto-assigned       | 4         | Open    | `SchoolSubjectSelection.streamId` is always null during auto-assignment. Stream-specific subjects (e.g., Physics for Science stream only) must be configured manually.                                                                          |
| `generateSections()` placeholder teacher         | 5a        | Open    | Auto-generated class sections use a placeholder teacher reference. Real teacher assignment is a manual step.                                                                                                                                    |
