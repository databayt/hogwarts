...
The capacity step's form labels say **"Students"**, **"Teachers"**, **"Classrooms"**, and **"Facilities"**. What actually gets persisted:
...
| **Facilities** | _(not persisted)_    | Collected by the form but never saved to any field |
...
After publishing, `setupCatalogForSchool()` should be called to provision the academic structure and subject selections. This function reads the school's profile (`country`, `schoolLevel`) and auto-creates everything in Layer 3 and Layer 4.
...
### Grade-to-Classroom Bridge

There is **no direct `gradeId` or `yearLevelId` on the `Class` model**. The grade connection is implicit. The `Class` model is the bridge between academics and physical space:

```
AcademicGrade (e.g., Grade 7)
    |
    | (implicit -- via class name and StudentYearLevel/StudentClass joins)
    |
    +--> Class
    |      name: "Math - Grade 7 A"
...
- **`Class` is the bridge** -- it holds `subjectId`, `teacherId`, `classroomId`, and `termId`
- **No `gradeId` on Class** -- the grade connection is implicit (via the class name convention and `StudentYearLevel`/`StudentClass` join tables)
...
| `country` format mismatch             | 1 --> 4   | Open   | Catalog uses ISO codes (`"US"`, `"SD"`). Seeds use mixed formats: `"SD"`, `"Sudan"`, `"السودان"`. No validation enforces ISO. |
| Head of Department not in schema      | 5a        | Open   | `TeacherDepartment` has `isPrimary` but no explicit `isDepartmentHead` flag.                                                  |
| No auto-placement after ADMITTED      | 5b --> 7  | Open   | After `ADMITTED` status, no automated `Class` assignment. `applyingForClass` is a grade string, not a FK.                     |
| Classroom setup not in onboarding     | 2 --> 6   | Open   | Capacity step saves `maxClasses` count but creates no `Classroom` records.                                                    |
| Onboarding `facilities` not persisted | 2 --> 6   | Open   | Capacity form collects "facilities" count but never saves it.                                                                 |
| `allowedSubjectTypes` unused          | 6 --> 7   | Open   | `RoomConstraint.allowedSubjectTypes` exists but timetable algorithm ignores it.                                               |
| Dashboard mock room data              | 7         | Open   | `UpcomingClassCard` shows hardcoded room strings instead of querying timetable.                                               |
...
| No `gradeId` on Class model           | 3 --> 5a  | Open   | Grade-to-class connection is implicit (via name and joins). No direct FK to `AcademicGrade`.                                  |
| ~~No classroom CRUD UI~~              | 6         | Fixed  | Full 6-file CRUD pattern exists at `school-dashboard/listings/classrooms/`.                                                   |
| Capacity cross-validation missing     | 5a --> 6  | Open   | `Class.maxCapacity` not validated against `Classroom.capacity`.                                                               |
