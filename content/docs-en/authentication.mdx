...
On the `demo` subdomain (any root — `demo.balqalam.com`, `demo.databayt.org`,
`demo.localhost:3000`) the login page shows a **role dropdown** instead of the
credential fields: Administrator (preselected), Teacher, Student, Parent, Accountant,
Staff, plus the two platform accounts User and Applicant. Press Login. A "sign in with
email instead" link reveals the normal form.

This is a shortcut, not a second auth lane. `demoRoleLogin()`
(`src/components/auth/login/demo-action.ts`) resolves the role to its seeded
`@balqalam.com` account and calls the same `login()` action, so the session, cookies,
audit log and redirect are identical to typing those credentials. The subdomain is
re-checked inside the action — Server Actions are publicly callable, so every other host
