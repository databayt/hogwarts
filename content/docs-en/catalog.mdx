...
| ------------------- | ---------------------------------------- | ---------------------------- |
| US K-12 (demo)      | `country: "US"`, `curriculum: "us-k12"`  | ~220 grade-specific subjects |
| International (UAE) | `country: "AE"`, `curriculum: "ib"`      | IB curriculum subjects       |
| British school      | `country: "GB"`, `curriculum: "british"` | UK National Curriculum       |

The curriculum defaults to `"us-k12"` (inferred from `country + schoolType` when not explicitly set). International schools always get `"us-k12"`, known countries get `"national"`, and unknown countries fall back to `"us-k12"`. Schools can also **browse** the full catalog and add subjects from other curricula.
...
All subjects use `country: "US"`, `curriculum: "us-k12"`. Each subject has `grades: [N]` (single grade number) instead of spanning multiple grades.
...
| PUBLIC  | Anyone (free)                                   |
| PAID    | Anyone who pays (requires `price` + `currency`) |
...
| prerequisites  | String?       | Prerequisite topics/skills                                    |
...
| curriculum     | String        | Curriculum identifier (default "national")                    |
| schoolTypes    | String[]      | Applicable school types (empty = all)                         |
| concept        | String?       | Image lookup key (e.g., "math", "science")                    |
...
| price          | Decimal?      | Subject price for PAID visibility                             |
...
| questions           | Json?             | Legacy: use CatalogExamQuestion junction instead |
| distribution        | Json?             | Question distribution (e.g., type x difficulty)  |
| bloomDistribution   | Json?             | Bloom's taxonomy distribution                    |
| gradeRange          | String?           | Grade applicability                              |
| levels              | SchoolLevel[]     | Level applicability                              |
...
| variantOf           | String?           | Parent CatalogExam id (for exam variants)        |
| variantLabel        | String?           | Variant name (e.g., "Version A", "Easy")         |
| price               | Decimal?          | Price for PAID visibility                        |
| currency            | String?           | Payment currency (default "USD")                 |
| usageCount          | Int               | Engagement metric                                |
| averageScore        | Float             | Average score across attempts                    |
| qualityScore        | Float             | Quality metric                                   |
| ratingCount         | Int               | Number of ratings                                |
...
#### CatalogExamTemplate (Reusable Exam Blueprints)

Subject-level reusable exam blueprints that define question type distributions, Bloom's taxonomy weights, and exam metadata. Schools can adopt these templates to generate exams.
...
| totalMarks          | Decimal           | Total marks                                  |
| distribution        | Json              | Question distribution (type x difficulty)    |
| bloomDistribution   | Json?             | Bloom's taxonomy weights                     |
| gradeRange          | String?           | Grade applicability                          |
| levels              | SchoolLevel[]     | Level applicability                          |
| contributedBy       | String?           | userId who created it                        |
| contributedSchoolId | String?           | School that contributed                      |
| approvalStatus      | ApprovalStatus    | PENDING, APPROVED, REJECTED                  |
| visibility          | ContentVisibility | PRIVATE, SCHOOL, PUBLIC, PAID                |
| usageCount          | Int               | Times adopted by schools                     |
| status              | ContentStatus     | Publication status                           |

#### CatalogExamBlueprint (Subject-Category Blueprints)

Persists 8 subject-category blueprints (STEM_MATH, LANGUAGE_ARTS, etc.) with question type weights, Bloom weights, difficulty curves, and exam type configurations.

| Field               | Type    | Description                                                         |
| ------------------- | ------- | ------------------------------------------------------------------- |
| id                  | String  | Primary key (cuid)                                                  |
| category            | String  | Unique category key (e.g., STEM_MATH, LANGUAGE_ARTS)                |
| name                | String  | Human-readable name                                                 |
| description         | String? | Category description                                                |
| questionTypeWeights | Json    | `{"MULTIPLE_CHOICE": 0.25, "SHORT_ANSWER": 0.20, ...}`              |
| bloomWeights        | Json    | `{"REMEMBER": 0.15, "UNDERSTAND": 0.20, ...}`                       |
| difficultyCurves    | Json    | `{"ELEMENTARY": {"EASY": 0.50, "MEDIUM": 0.35, "HARD": 0.15}, ...}` |
| examTypeConfigs     | Json    | `{"final": {"duration": [90, 150], "marks": [80, 100], ...}, ...}`  |
| questionTemplates   | Json?   | Template objects for auto-generation                                |
| mcqOptionGenerators | Json?   | MCQ distractor generation rules                                     |
| classificationRules | Json?   | Regex patterns to match subjects to this category                   |
| isActive            | Boolean | Whether this blueprint is active                                    |

#### CatalogPaperTemplate (Exam Paper Templates)

Platform-wide exam paper templates with composable building blocks for header, footer, student info, instructions, and answer sheets.
...
| name                  | String            | Template name                                  |
| lang                  | String            | Content language (default "ar")                |
| template              | String            | Base template: CLASSIC, MODERN, FORMAL, CUSTOM |
| layout                | String            | SINGLE_COLUMN, TWO_COLUMN, BOOKLET             |
| answerSheetType       | String            | NONE, SEPARATE, BUBBLE                         |
| showSchoolLogo        | Boolean           | Include school logo in header                  |
| showExamTitle         | Boolean           | Include exam title                             |
| showInstructions      | Boolean           | Include instructions section                   |
| showStudentInfo       | Boolean           | Include student info section                   |
| showQuestionNumbers   | Boolean           | Number questions                               |
| showPointsPerQuestion | Boolean           | Show point values                              |
| shuffleQuestions      | Boolean           | Randomize question order                       |
| shuffleOptions        | Boolean           | Randomize MCQ option order                     |
| blockConfig           | Json?             | Composable block configuration array           |
| contributedBy         | String?           | userId who created it                          |
| contributedSchoolId   | String?           | School that contributed                        |
| approvalStatus        | ApprovalStatus    | PENDING, APPROVED, REJECTED                    |
| visibility            | ContentVisibility | PRIVATE, SCHOOL, PUBLIC, PAID                  |
...
#### CatalogProposal (School-Initiated Curriculum Proposals)

Schools can propose new subjects, chapters, or lessons to be added to the catalog. Reviewed by SaaS admins.

| Field           | Type           | Description                                                |
| --------------- | -------------- | ---------------------------------------------------------- |
| id              | String         | Primary key (cuid)                                         |
| schoolId        | String         | FK to School (has schoolId -- school-scoped)               |
| proposedBy      | String         | userId who proposed                                        |
| type            | ProposalType   | SUBJECT, CHAPTER, LESSON                                   |
| status          | ProposalStatus | DRAFT, SUBMITTED, IN_REVIEW, APPROVED, REJECTED, PUBLISHED |
| data            | Json           | Proposal payload (varies by type)                          |
| parentSubjectId | String?        | CatalogSubject ID (for chapter proposals)                  |
| parentChapterId | String?        | CatalogChapter ID (for lesson proposals)                   |
| reviewedBy      | String?        | Who reviewed                                               |
| reviewedAt      | DateTime?      | When reviewed                                              |
| reviewNotes     | String?        | Reviewer notes                                             |
| rejectionReason | String?        | Why rejected                                               |
| catalogEntityId | String?        | Created catalog entity ID (after approval)                 |
...
| `getCatalogDashboardData`      | `get-dashboard-data.ts`       | Dashboard analytics shape     |
| `getCatalogLessonContent`      | `get-lesson-content.ts`       | Lesson content with materials |
...
3. CatalogSubjects (~220 records, country: "US", curriculum: "us-k12")
