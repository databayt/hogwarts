...
302 page.tsx files audited across 5 route groups:

| Area                                  | Status  | Detail                                                                  |
| ------------------------------------- | ------- | ----------------------------------------------------------------------- |
| Promise-based params/searchParams     | PASS    | All 302 pages correctly use `Promise<>`                                 |
...
Extract data fetching to co-located `queries.ts` if exceeded.

```tsx
// BAD: 200 lines of Prisma queries in page.tsx

// GOOD: Extract to queries.ts
import { getTeachersWithPerformance } from "./queries"
...
`loading.tsx` automatically creates a Suspense boundary. Use `loading.tsx` for full-page loading states. Use inline `<Suspense>` only for granular streaming of independent data blocks within a page.
...
import { getTeacherDetail } from "./queries"
...
- Data fetching via co-located `queries.ts` (not inline Prisma queries)
...
```tsx
// GOOD: Extract to co-located queries.ts
// queries.ts
import "server-only"
...
- `useActionState` hook (React 19) replaces `useFormState` for consuming server actions in forms
- `useFormStatus` hook (React 19) provides pending state for submit buttons
- `use()` hook (React 19) can unwrap promises and context in render
- Partial Prerendering (PPR) enables static shell with streaming dynamic content
