...
> **CRITICAL:** Never run `pnpm db:seed` directly. Always use `pnpm db:seed:single <name>` to seed individual modules. Full seed takes 45-120 minutes and is reserved for manual-only complete resets. See `CLAUDE.md` rule #8 for details.
