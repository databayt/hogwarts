 ...
 * @example
 * ```ts
 * import { createMockSchool, createMockUser } from '@/tests/factories'
 *
...
export function createMockSubject(overrides?: Partial<Subject>): Subject {
  ...
  return {
    ...
    subjectGroupId: overrides?.subjectGroupId ?? null,
...
export function createSchoolWithEntities(options?: {
  ...
  const teachers = createBatch(createMockTeacher, options?.teacherCount ?? 3, {
    schoolId: school.id,
  })
  const students = createBatch(createMockStudent, options?.studentCount ?? 10, {
    schoolId: school.id,
  })
