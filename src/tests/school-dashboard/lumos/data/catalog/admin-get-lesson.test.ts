...
import { adminGetLesson } from "@/components/lumos/data/catalog/admin-get-lesson"
...
describe("adminGetLesson — auth gate", () => {
  it("rejects anonymous callers (no session) without touching the DB", async () => {
    ...
    await expect(adminGetLesson("lesson-1")).rejects.toThrow("NEXT_NOT_FOUND")
  ...
  it("allows ADMIN and returns the lesson shape", async () => {
    ...
    const result = await adminGetLesson("lesson-1")
...
describe("adminGetLesson — behavior", () => {
  ...
  it("notFound() when the lesson does not exist", async () => {
    ...
    await expect(adminGetLesson("missing")).rejects.toThrow("NEXT_NOT_FOUND")
  })

  it("scopes the video query to the school OR public when schoolId given", async () => {
    await adminGetLesson("lesson-1")
  ...
  it("does not constrain by school for a school-less tenant", async () => {
