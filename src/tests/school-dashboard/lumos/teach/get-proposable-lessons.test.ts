...
const mockVideoAggregate = db.video.aggregate as ReturnType<typeof vi.fn>
...
beforeEach(() => {
  ...
  mockVideoAggregate.mockResolvedValue({
    _avg: { price: null },
    _count: { _all: 0 },
  })
...
describe("getProposableCatalog", () => {
  ...
  it("buckets subjects under the grade the school selected them for", async () => {
    ...
    // Ordered by grade number; the lumos duplicate is deduped; the
  ...
  it("filters out subjects whose grades array does not match the selection grade", async () => {
    mockAuth.mockResolvedValueOnce({ user: { id: "t-1", role: "TEACHER" } })
    mockSelectionFindMany.mockResolvedValueOnce([
      {
        catalogSubjectId: "physics-g11",
        customName: null,
        grade: { id: "grade-1", gradeNumber: 1 },
      },
      {
        catalogSubjectId: "physics-g11",
        customName: null,
        grade: { id: "grade-11", gradeNumber: 11 },
      },
    ])
    mockSubjectFindMany.mockResolvedValueOnce([
      { id: "physics-g11", name: "Physics", slug: "physics-11", grades: [11] },
    ])
    mockChapterFindMany.mockResolvedValueOnce([
      { subjectId: "physics-g11", _count: { lessons: 5 } },
    ])

    const result = await getProposableCatalog()
    // Physics Grade 11 is only placed under Grade 11, not Grade 1
    expect(result).toEqual([
      {
        id: "grade-11",
        gradeNumber: 11,
...
describe("getSuggestedPrice", () => {
  /** The `where` the aggregate handed to Prisma. */
  function aggregateWhere() {
    return mockVideoAggregate.mock.calls[0][0].where as {
      price: { gt: number }
      currency: string
      approvalStatus: string
      lesson: { chapter: { subjectId: string; subject: unknown } }
    }
  }

  it("returns nothing for an unauthenticated caller", async () => {
    mockAuth.mockResolvedValueOnce(null)
    expect(await getSuggestedPrice("subj-1", "USD")).toBeNull()
    expect(mockVideoAggregate).not.toHaveBeenCalled()
  })

  it.each(["STUDENT", "GUARDIAN", "ACCOUNTANT", "STAFF", "USER"] as const)(
    "denies role %s — a school's pricing is not public",
    async (role) => {
      mockAuth.mockResolvedValueOnce({ user: { id: "u-1", role } })
      expect(await getSuggestedPrice("subj-1", "USD")).toBeNull()
      expect(mockVideoAggregate).not.toHaveBeenCalled()
  ...
  it("rejects a currency that is not a 3-letter code", async () => {
    mockAuth.mockResolvedValueOnce({ user: { id: "t-1", role: "TEACHER" } })
    expect(await getSuggestedPrice("subj-1", "DOLLARS")).toBeNull()
    expect(mockVideoAggregate).not.toHaveBeenCalled()
  })

  it("averages only positive, APPROVED prices in the SAME currency, inside the caller's scope", async () => {
    mockAuth.mockResolvedValueOnce({ user: { id: "t-1", role: "TEACHER" } })
    mockVideoAggregate.mockResolvedValueOnce({
      _avg: { price: 12.5 },
      _count: { _all: 4 },
    })
    const result = await getSuggestedPrice("subj-1", "sdg")
    expect(result).toEqual({ price: 12.5, sampleSize: 4, currency: "SDG" })

    const where = aggregateWhere()
    expect(where.price).toEqual({ gt: 0 })
    // Lowercase in, ISO code out — a "sdg" row would match nothing.
    expect(where.currency).toBe("SDG")
    expect(where.approvalStatus).toBe("APPROVED")
    expect(where.lesson.chapter.subjectId).toBe("subj-1")
    // The tenant gate travels with the aggregate, exactly as it does for the
    // picker — an out-of-scope subject can never average another school's
    // pricing.
    expect(where.lesson.chapter.subject).toBeDefined()
  })

  it("rounds to 2dp — an average is never a currency amount by itself", async () => {
    mockAuth.mockResolvedValueOnce({ user: { id: "t-1", role: "TEACHER" } })
    mockVideoAggregate.mockResolvedValueOnce({
      _avg: { price: 9.996666666 },
      _count: { _all: 3 },
    })
    expect((await getSuggestedPrice("subj-1", "USD"))?.price).toBe(10)
  })

  it("returns null for a course with no paid videos — no fabricated comparable", async () => {
    mockAuth.mockResolvedValueOnce({ user: { id: "t-1", role: "TEACHER" } })
    expect(await getSuggestedPrice("subj-1", "USD")).toBeNull()
  })
})
