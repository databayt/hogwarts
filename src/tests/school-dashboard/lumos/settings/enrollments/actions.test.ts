...
} from "@/components/lumos/settings/enrollments/actions"
...
describe("bulkEnrollStudents — flow", () => {
  ...
  it("revalidates the settings page (the route that renders the list)", async () => {
    await bulkEnrollStudents(input)
    // The `"page"` argument is not decoration: the path carries `[lang]` and
    // `[subdomain]`, and Next ignores a bracketed path when `type` is absent.
    // Asserting it here is what keeps the call from silently reverting to a
    // no-op — which is exactly what this assertion used to permit.
