/**
 * Epic 9: Lumos - Lesson Page Flow
 *
 * Tests the lumos dashboard → course → lesson page flow,
 * including the lesson hero, sibling lessons row, and video player.
 * Tag: @lumos @lesson @catalog
 */
...
// Lumos pages are heavy — give extra time for login + SSR
...
async function goToFirstLesson(page: import("@playwright/test").Page) {
  ...
  await page
    .waitForURL(/\/lumos\/dashboard\/elementary-arts\/\w+/, {
...
test.describe("Lumos Dashboard @lumos", () => {
  ...
  test("ST-001: Can access lumos dashboard", async ({ page }) => {
  ...
  test("ST-002: Lumos dashboard shows enrolled courses", async ({ page }) => {
    await page.goto(buildSchoolUrl("demo", "/lumos/dashboard", "en", env))
    await page.waitForLoadState("domcontentloaded")
...
test.describe("Lumos Course Detail @lumos", () => {
  test.beforeEach(async ({ page }) => {
    await clearAuthState(page)
    await loginAsAdmin(page)
  ...
  test("ST-003: Course slug redirects to first lesson", async ({ page }) => {
    ...
    await page.waitForURL(/\/lumos\/dashboard\/elementary-arts\/\w+/, {
      timeout: 20_000,
    })

    expect(page.url()).toMatch(
      /\/lumos\/dashboard\/elementary-arts\/[a-zA-Z0-9]+/
    )
    await assertNoSSE(page)
  })
})

test.describe("Lumos Lesson Page @lumos @lesson", () => {
  test.beforeEach(async ({ page }) => {
    await clearAuthState(page)
    await loginAsAdmin(page)
  ...
  test("ST-004: Lesson page loads with hero", async ({ page }) => {
    ...
    expect(page.url()).toMatch(/\/lumos\/dashboard\/elementary-arts\//)
    await assertNoSSE(page)
