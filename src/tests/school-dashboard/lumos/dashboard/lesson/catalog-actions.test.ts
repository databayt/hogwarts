...
import { markLessonComplete } from "@/components/lumos/dashboard/lesson/catalog-actions"

vi.mock("@/auth", () => ({ auth: vi.fn() }))
vi.mock("next/cache", () => ({ revalidatePath: vi.fn() }))
vi.mock("next/headers", () => ({
  cookies: vi.fn().mockResolvedValue({ get: () => ({ value: "en" }) }),
  // lumosTenantUrl reads host/x-subdomain/x-locale; null values exercise its
