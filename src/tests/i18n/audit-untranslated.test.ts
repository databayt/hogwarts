...
//
// 2026-09-20: FEATURE_BASELINE 1→0. The one gap was never real —
// transportation/routes translates every row through
// `../shared/translate-display`'s `translateRoutes` (which wraps `getLabels`),
// and the detector only looked for helper calls INSIDE the feature dir. It now
// follows relative imports one level, and the count is honestly 0.
//
// PREWARM stays at 1 and is met exactly. It read 4: `finance/fees/actions.ts`
// was a genuine gap on a user-entered `name` and now prewarms on create and on
// the update that writes it; the other two were verified non-user paths and
// joined PREWARM_EXEMPT (`live/demo-clock.ts` also runs from the seed, where
// after() has no request scope; `fees/transport-provisioning.ts` composes an
// isAutoGenerated name inside a $transaction). The remaining 1 is
// transportation O-9, deferred P3 in that block's own ISSUE.md.
const PERSON_BASELINE = 0
const FEATURE_BASELINE = 0
