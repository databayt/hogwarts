...
export function usePerformance(componentName: string) {
  ...
  const trackUserAction = useCallback(
    (action: string, element?: string, value?: any) => {
  ...
  const endTimer = useCallback(
    (timerName: string, context?: Record<string, any>) => {
...
export function useFormPerformance(formName: string) {
  ...
  const { trackUserAction, trackAsyncOperation } = usePerformance(
    `form_${formName}`
  )
...
export function useTablePerformance(tableName: string) {
  const { trackUserAction, trackAsyncOperation, startTimer, endTimer } =
    usePerformance(`table_${tableName}`)

  const trackTableAction = useCallback(
    (action: string, rowCount?: number, filters?: any) => {
