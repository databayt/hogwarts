...
import { confirmDelete, showCrudToast } from "@/lib/toast-utils"
  ...
  const handleFormSubmit = useCallback(
    async (formData: TFormValues) => {
      ...
      if (mode === "create" && config.actions.create) {
        ...
        crudModal.state.data
      ) {
        return await config.actions.update(
          crudModal.state.data.id,
          processedData
        )
      }
