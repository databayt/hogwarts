import type { SearchItem, RecentItem, SearchContext, Role } from "./types";
...
export function filterByQuery(items: SearchItem[], query: string): SearchItem[] {
  ...
  return items.filter((item) => {
    ...
    // Match breadcrumb
    if (item.breadcrumb?.some((b) => b.toLowerCase().includes(lowerQuery))) return true;
 ...
 */
export function recentItemsToSearchItems(recentItems: RecentItem[]): SearchItem[] {
