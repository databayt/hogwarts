...
import { RESERVED_SUBDOMAINS, SUBDOMAIN_CONSTANTS } from "./config"
  ...
  return z.object({
    domain: z
      ...
      .min(SUBDOMAIN_CONSTANTS.MIN_LENGTH, {
        message: v.get("subdomainMinLength", {
          min: SUBDOMAIN_CONSTANTS.MIN_LENGTH,
        }),
      })
      .max(SUBDOMAIN_CONSTANTS.MAX_LENGTH, {
        message: v.get("subdomainMaxLength", {
          max: SUBDOMAIN_CONSTANTS.MAX_LENGTH,
        }),
      })
      .toLowerCase()
      ...
      .refine((val) => !RESERVED_SUBDOMAINS.includes(val as any), {
...
export const subdomainValidation = z.object({
  domain: z
    .string()
    .min(
      SUBDOMAIN_CONSTANTS.MIN_LENGTH,
      `Subdomain must be at least ${SUBDOMAIN_CONSTANTS.MIN_LENGTH} characters`
    )
    .max(
      SUBDOMAIN_CONSTANTS.MAX_LENGTH,
      `Subdomain must be less than ${SUBDOMAIN_CONSTANTS.MAX_LENGTH} characters`
    )
    .toLowerCase()
...
export function validateSubdomain(
  ...
  if (!/^[a-z0-9]/.test(subdomain)) {
    errors.push(
      v?.get("subdomainMustStartWithLetterOrNumber") ||
        "Must start with a letter or number"
    )
  }
  if (!/[a-z0-9]$/.test(subdomain)) {
    errors.push(
      v?.get("subdomainMustEndWithLetterOrNumber") ||
  ...
  if (subdomain.includes("--")) {
    errors.push(
      v?.get("subdomainNoConsecutiveHyphens") ||
...
export function isValidSubdomainLength(subdomain: string): boolean {
  return (
    subdomain.length >= SUBDOMAIN_CONSTANTS.MIN_LENGTH &&
