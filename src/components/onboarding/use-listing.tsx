...
export function ListingProvider({
  ...
  const loadListing = useCallback(async (id: string, retryCount = 0) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (result.error) {
          if (typeof result.error === "string") {
            ...
            if (
              "code" in errorObj &&
              errorObj.code === "CROSS_TENANT_ACCESS_DENIED"
            ) {
              isAccessDenied = true
            }
          }
        ...
        if (isAccessDenied && retryCount < 3) {
        ...
        if (retryCount >= 3 && isAccessDenied) {
          setError(
            "Unable to access this school. This might be because you're trying to access a different school than the one you created. Please go back to the overview and try again."
          )
        } else {
      ...
      // Handle access denied error during onboarding with exponential backoff
      if (
        errorMessage.includes("Access denied to this school") &&
        retryCount < 2
      ) {
