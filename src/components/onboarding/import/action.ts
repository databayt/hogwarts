...
async function processCsvImport(
  ...
  try {
    ...
    const fileContent =
      typeof data.file === "string" ? data.file : await data.file.text()
...
async function processExcelImport(
  ...
  try {
    ...
    if (typeof data.file === "string") {
      ...
      buffer =
        tempBuffer instanceof ArrayBuffer
          ? tempBuffer
          : (tempBuffer as ArrayBuffer)
    }
        ...
        case "classes":
          for (const item of data) {
            try {
              ...
              logger.warn(
                "Class import not yet implemented - requires subject/teacher/term/classroom setup",
                {
                  row: item.row,
...
export async function skipDataImport(
  schoolId: string
): Promise<ActionResponse> {
  try {
    await requireSchoolOwnershipLazy(schoolId)
