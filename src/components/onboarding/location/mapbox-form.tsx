...
export function MapboxForm({ initialData, onLocationChange }: MapboxFormProps) {
  ...
  useEffect(() => {
    if (marker.current && selectedLocation.latitude && selectedLocation.longitude) {
  ...
  return (
    <div className="relative rounded-lg overflow-hidden border h-[400px]">
      ...
      {/* Search Input Overlay - Inside the map */}
      <div className="absolute top-4 left-4 right-4 max-w-md">
        <div ref={searchRef} className="relative">
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder="Search for your school's location..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-10 pr-10 bg-white/95 backdrop-blur-sm shadow-lg border-gray-200"
            />
            {isSearching && (
          ...
          {showResults && searchResults.length > 0 && (
            <Card className="absolute top-full mt-2 w-full z-50 max-h-60 overflow-auto shadow-xl bg-white/95 backdrop-blur-sm">
              <div className="p-1">
