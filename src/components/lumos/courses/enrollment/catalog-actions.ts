...
import {
  ...
  getAuthContext,
} from "@/components/lumos/authorization"
import { sendEnrollmentEmail } from "@/components/lumos/shared/email-service"
  ...
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (!isPaid) {
        ...
        if (session.user.email) {
          sendEnrollmentEmail({
            ...
            courseUrl: await lumosTenantUrl(
      ...
      const checkoutSession = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url: await lumosTenantUrl(
          `/lumos/courses/${subject.slug}?enrolled=true`,
          locale
        ),
        cancel_url: await lumosTenantUrl("/lumos/courses", locale),
        metadata: {
          userId: session.user.id,
          catalogSubjectId: subject.id,
