...
export async function deleteOwnVideo(videoId: string): Promise<ActionResponse> {
  ...
  try {
    ...
    if (video.schoolId && video.fileSize && video.fileSize > 0) {
      try {
        const { decrementSchoolVideoUsage } =
          await import("@/components/lumos/lib/quota")
    ...
    // Drop the bytes, then the cached copy of them. Until 2026-08-14 this only
    // invalidated the CDN: the quota counter said the space was freed while the
    // object sat in the bucket forever, so the school's real storage bill grew
    // with every "deleted" video and drifted further from the counter each time.
    if (video.storageKey) {
      // Two guards before an irreversible delete. The prefix check is the same
      // one `uploadVideo` and `DELETE /api/blob/presign` make — rows written
      // before that assert existed could name anything. The reference check
      // covers a key shared by more than one row (a replace flow that reused
      // it); the object belongs to whoever is left.
      const key = video.storageKey
      const ownPrefix = video.schoolId
        ? `stream/${video.schoolId}/video/`
        : "stream/"
      const stillReferenced = await db.video.findFirst({
      ...
      if (
        ...
        try {
          ...
          // Non-critical — the row is already gone; a sweep can reclaim this.
          console.error("Failed to delete video object from storage:", err)
        }
      }
      try {
