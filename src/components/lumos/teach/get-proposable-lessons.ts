...
async function resolveProposableScope(): Promise<ProposableScope | null> {
  ...
  // Auto-provision if no selections exist (mirrors courses / subjects page behavior)
...
export async function getProposableCatalog(
        ...
        _count: {
          select: {
            chapters: {
              where: {
                id: { notIn: scope.hiddenChapterIds },
                lessons: { some: { id: { notIn: scope.hiddenLessonIds } } },
              },
            },
          },
        },
  ...
  if (scope.kind === "school") {
    for (const selection of scope.selections) {
      ...
      // If the subject specifies grades, ensure this selection's grade matches.
      if (
        subject.grades.length > 0 &&
        !subject.grades.includes(selection.grade.gradeNumber)
      ) {
        continue
      }
      ...
      push(
        ...
        },
        {
          ...
          lessonCount,
          chapterCount,
          imageUrl: getCatalogImageUrl(subject.thumbnail, "sm") ?? null,
    ...
    .map((c) => ({
      ...
      ...(c.thumbnail
        ? {
            ...
            imageUrl: getCatalogImageUrl(c.thumbnail, "sm") ?? null,
          }
        : {}),
 ...
 * `GET /api/lumos/proposable-lessons`, so ids belonging to another school's
  ...
  const page = lessons.slice(0, pageSize).map((l) => ({
    ...
    ...(l.thumbnail
      ? {
          ...
          imageUrl: getCatalogImageUrl(l.thumbnail, "original") ?? null,
        }
      : {}),
    ...(l.color || l.chapter.color || l.chapter.subject.color
      ? { color: l.color ?? l.chapter.color ?? l.chapter.subject.color }
      : {}),
    ...(l.durationMinutes != null
      ? { durationMinutes: l.durationMinutes }
      : {}),
    ...(l.videoCount != null ? { videoCount: l.videoCount } : {}),
  }))
...
export type SuggestedPrice = {
  /** Average price of the paid videos already on this course, 2dp. */
  price: number
  /** How many videos that average is drawn from — a 1-video "average" is noise. */
  sampleSize: number
  currency: string
}

/**
 * What comparable videos on this course already charge.
 *
 * A blank price field asks a contributor to invent a number, and the honest
 * answer to "what should I charge?" is whatever the course's other paid videos
 * charge. Only APPROVED, positive-priced videos in the SAME currency count —
 * mixing currencies would average SDG into USD and produce a confident wrong
 * number.
 *
 * Deliberately subject-scoped with no wider fallback: a course with no paid
 * videos yet has no comparable, and inventing one from an unrelated subject is
 * worse than showing nothing. Returns null there, and the caller renders no
 * suggestion.
 *
 * Goes through `resolveProposableScope` like every other entry point here, so
 * a subject the caller may not contribute to yields nothing rather than
 * leaking another school's pricing.
 */
export async function getSuggestedPrice(
  ...
  if (!scope || !subjectId || code.length !== 3) return null

  const { _avg, _count } = await db.video.aggregate({
    where: {
      price: { gt: 0 },
      currency: code,
      approvalStatus: "APPROVED",
      lesson: { chapter: { subjectId, subject: scope.subjectWhere } },
    },
    _avg: { price: true },
    _count: { _all: true },
  })

  const average = _avg.price
  if (average === null || _count._all === 0) return null
  return {
    price: Math.round(average * 100) / 100,
    sampleSize: _count._all,
    currency: code,
  }
}
