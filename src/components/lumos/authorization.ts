...
/**
 * Authorization middleware for Lumos LMS
 * Implements role-based access control (RBAC) with ownership checks
 */

import { UserRole } from "@prisma/client"

export type LumosAction = "create" | "read" | "update" | "delete" | "enroll"
...
/**
 * Check if a user has permission to perform an action on a Lumos course
 *
 * Permission matrix:
...
export function assertStreamPermission(
  ...
  if (!checkStreamPermission(auth, action, course)) {
    throw new Error(
      `Unauthorized: ${auth.role} cannot perform ${action} on lumos course${
        course?.id ? ` ${course.id}` : ""
      }`
    )
  }
