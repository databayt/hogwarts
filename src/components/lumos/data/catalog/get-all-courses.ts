 ...
 *
 * Server-only, NOT `"use server"`: this used to be an action so the courses
 * grid could call it from the browser for "See More", but every client-invoked
 * action response ships a full RSC re-render of the current page. The browser
 * now reads `GET /api/lumos/course-search` instead, which wraps this — so the
 * POST RPC stub (and the attack surface that came with it) is gone. No
 * `cache()` wrapper: the sole argument is an object literal, which React's
...
export async function getAllCatalogCourses(
  ...
  if (searchTerm) {
    where.OR = await buildBilingualNameMatch(searchTerm, schoolId, displayLang)
  }
 ...
 *
 * The cache read is scoped to `targetLanguage: displayLang` — the term was
 * typed in the language the reader is looking at, so rows translated INTO any
 * other language can never match it. Without that predicate the query scanned
 * every cached translation the school owns (the whole `translation_cache`
 * partition, on an unindexed `contains`) on every keystroke.
 */
