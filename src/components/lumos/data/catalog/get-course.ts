...
import { getInstructorPolicy } from "@/components/lumos/lib/instructor-policy"
    ...
    (allLessonIds.length > 0
      ? db.video
          .groupBy({
            ...
            // Several, not one: the top contributor may be an instructor this
            // school disabled, and a card must never advertise a name whose
            // videos it will not serve.
            take: 5,
          })
          .then(async (groups) => {
            const brandName = lang === "ar" ? "بالقلم" : "balqalam"
            const policy = await getInstructorPolicy(schoolId)
            const topCreatorSchoolId =
              groups.find(
                (g) => g.schoolId && !policy.blocked.has(`school:${g.schoolId}`)
              )?.schoolId ?? null
      ...
      : Promise.resolve(lang === "ar" ? "بالقلم" : "balqalam")
    ).catch(() => (lang === "ar" ? "بالقلم" : "balqalam")),
  ])
