# Lumos (LMS) Module - Issues & Roadmap
...
**Fixed + tested this pass (committed on `fix/lumos-production-ready`):**
...
- **P1 — lesson-player i18n:** fixed the `.lumos.lumos` double-nesting so ~45
...
- **Cleanup:** deleted ~1,800 lines of dead legacy code (`queries.ts`,
  ...
  enrollment `button.tsx`, `/api/lumos/courses/[courseId]`) and 3 stale
  "Marshal LMS" docs.
...
- **i18n (consumption layer) — partially done:** the lesson player (~45 strings,
  ...
  codes + `ErrorHelper` per `.claude/rules/translation.md` — a lumos-isolated but
  ...
  the ROOT dictionary, not the lumos subtree — high collision risk while the
...
(lumos), 250/250 lumos unit tests green.
...
- **Fixed** `bulkEnrollStudents` `revalidatePath` (`/lumos/admin/enrollments`
...
- ✅ **The admin "Review" tab was unwired — now re-wired.** `settings/page.tsx`
  ...
  and lumos routes, and `Video @@index([schoolId, approvalStatus])` was added
...
259 lumos unit tests green (+9 new).
...
- Run with: `pnpm test src/components/lumos/__tests__/ --run`
...
- Created comprehensive `LumosDictionary` interface in `types.ts`
- 200+ typed dictionary keys for full i18n support
- `LumosContentProps` now uses typed dictionary
...
| Dictionary Types are `any`  | ✅ FIXED | Full `LumosDictionary` interface    |
...
- [x] Catalog-based architecture migration (replaces legacy lumos\_ tables)
...
```bash
# Add tests file
src/components/lumos/__tests__/[name].tests.ts

# Run tests
pnpm tests src/components/lumos/__tests__/ --run
```
