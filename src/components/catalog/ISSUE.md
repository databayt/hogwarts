...
- [x] **SD grade 12 — Markdown twins (2026-09-05)**: every `g12/<subject>/textbook.pdf` has a MarkItDown-derived `textbook.md` beside it and on BOTH buckets (`catalog/textbooks/<slug>/textbook.md`, `text/markdown`); the uploader gained the asset + `--assets` filter. Honest yield: 10/25 usable (A: agriculture, engineering, French; B: art, geography, history, family sciences; C: biology, computer science, Islamic elective), **15/25 EMPTY** (5 scans + 10 fonts without a Unicode map) — those need OCR (MarkItDown + Azure Document Intelligence, or tesseract-ara). Front matter carries the grade. No Subject field points at the .md (deterministic key); adding one is a schema decision.
