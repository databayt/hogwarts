...
// Ported verbatim from zenda (home/testimonials). Renders under the `.zenda-clone`
...
// Active-slide zoom — drive each slide's scale from its Swiper progress (0 at the
// centre, ±1 one slide away) so the centred card is full size *exactly* when it
// reaches the centre. Tying the scale to position rather than the `.is-active`
// class (which Swiper's loopFix drops mid-loop) means the incoming card grows
// *while* it slides in and is already big on arrival.
...
// Keep the scale transition in lock-step with the slide movement: Swiper passes
// the active transition duration (the 480ms slide speed, or 0 while dragging).
...
export function Testimonials() {
  ...
  // cards just sit there at their persistent 0.9 scale (set in CSS).
  const reveal = (on: boolean, name: string): Record<string, string> =>
    on ? { [name]: "" } : {}

  // testimonialIntro — header + active-card text + nav rise in, the active photo
  // scales up, and the telescope pops. The neighbour cards are NOT animated here:
  // their smaller size + the gaps come from the persistent CSS scale(.9), so they
  // always read immediately (animating them was what hid the peek before).
  useEffect(() => {
