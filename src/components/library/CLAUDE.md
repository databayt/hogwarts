...
- **The detail page localizes, and it must do so AFTER its related-book queries** -- `book-detail/content.tsx` runs `localizeOne("Book", catalogBook)` plus `localize()` over both shelves, then renders from `shown` rather than `catalogBook`. The queries above it match `moreByAuthor` on `catalogBook.author` and `similarBooks` on `catalogBook.genre`, and a translated author matches no row -- the same trap `FEATURED_BOOK_TITLE` documents on the library home. The lazy `schoolBook.create` above deliberately keeps the RAW values: the school-scoped row is storage, not display.
- **The featured book's blurb is fetched on its own, and `description` stays out of the list `select`** -- `content.tsx` reads it with a second `findUnique` + `localizeOne`. `description` is a registered translatable field, so putting it in the list select would have `localize()` translate a paragraph for every book on the page when the section shows one. One extra row read is the cheaper half.

- **Borrow/return confirm in `book-detail/borrow-alert.tsx`, never a toast** -- ONE alert rendered outside both branches of `borrow-book.tsx`, because `router.refresh()` flips `hasBorrowedBook` while it is open and an alert inside a branch unmounts with it. It has NO button: it dismisses itself after 5 s, on a click outside, or on Escape — so it is a Radix `Dialog` composed from primitives, not an `AlertDialog`, which refuses outside clicks by design (and `DialogContent` hardwires a 50% black overlay the reference doesn't have). The tick inside the glyph is flipped back in RTL -- the layout mirrors, a tick reversed reads as a chevron.
