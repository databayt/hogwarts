...
- **Templates live IN the block; `/documents` is gone (2026-07-20)** — the standalone
  `/documents` page and its sidebar entry were **deleted**. The docx-template UI is now
  hosted by the blocks that consume it: **`/exams/templates`** (categories `EXAM_PAPER` +
  `CERTIFICATE`, new `templates` tab in `getExamTabsForRole`, staff-only —
  DEVELOPER/ADMIN/TEACHER) and **`/grades/templates`** (category `REPORT_CARD`, replacing
  the old redirect-to-`/documents`). The shared `DocumentsManager` now takes a
  `categories` prop so each host renders only its own sections, and reads
  `dictionary.school.documents.*` instead of the hardcoded bilingual `L` object it used
  before. The engine itself (`documents/actions.ts`, `generate.ts`, `resolvers/`,
  `src/lib/docx-fill/`) is unchanged and still lives under `school-dashboard/documents/`.
- **Exam papers are generated by COUPLING a layout to exam data (2026-07-20)** — an
  `EXAM_PAPER` template card opens `UseExamTemplateDialog` instead of the old
  paste-an-entity-id box. Two modes, both ending in a filled `.docx`:
  _existing_ (pick a `GeneratedExam` that already has questions) and _blueprint_
  (pick a `SchoolExamTemplate` + class + title + date → `generateExamPaperFromTemplate`
  creates `SchoolExam` → `GeneratedExam` in ONE `$transaction`, then runs
  `autoGenerateExamQuestions` to select questions from the bank, then fills). The dialog
  shows per-tag coverage by intersecting `DocumentTemplate.mergeFields` (detected at
  upload) with `FIELD_VOCAB[category]`, so a school sees which of its own tags will be
