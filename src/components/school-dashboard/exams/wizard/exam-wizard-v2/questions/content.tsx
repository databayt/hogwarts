...
export default function QuestionsContent() {
  ...
  return (
    <WizardStep
      ...
      nextStep={`/exams/generate/add/${generatedExamId}/paper-config`}
      isValid={isValid}
      formRef={formRef}
      isLoading={isLoading}
    >
      <FormLayout>
        <FormHeading
          title={getStepLabel("questions", "title", locale)}
          description={getStepLabel("questions", "description", locale)}
        />
        <QuestionsForm
          ref={formRef}
          generatedExamId={generatedExamId}
          subjectId={data?.templateSubjectId || data?.examSubjectId}
          initialQuestionIds={data?.selectedQuestionIds}
