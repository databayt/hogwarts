...
import { updateTemplateFooterLayout } from "./actions"

const SLOT: SlotName = "footer"

interface FooterLayoutFormProps {
  templateId: string
  initialVariant?: string
  onValidChange?: (isValid: boolean) => void
}

export const FooterLayoutForm = forwardRef<
  WizardFormRef,
  FooterLayoutFormProps
    ...
    saveAndNext: () =>
      new Promise<void>((resolve, reject) => {
        startTransition(async () => {
          try {
            const result = await updateTemplateFooterLayout(
              templateId,
              selected
            )
            if (!result.success) {
...
FooterLayoutForm.displayName = "FooterLayoutForm"
