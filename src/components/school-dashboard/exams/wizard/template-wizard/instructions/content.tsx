...
export default function InstructionsContent({ dictionary }: Props) {
  ...
  return (
    <WizardStep
      entityId={templateId}
      nextStep={`/exams/template/add/${templateId}/footer-layout`}
      isValid={isValid}
      formRef={formRef}
      isLoading={isLoading}
    >
      <FormLayout>
        <FormHeading
          title={
            dictionary?.title ?? getStepLabel("instructions", "title", locale)
          }
          description={
            dictionary?.description ??
            getStepLabel("instructions", "description", locale)
          }
        />
        <InstructionsForm
          ref={formRef}
          templateId={templateId}
          initialVariant={data?.instructionsVariant ?? "standard"}
