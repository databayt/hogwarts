...
import { balancedDifficulty, type QuestionTypeConfig } from "../../types"
import {
  commonLabels,
  QUESTION_TYPE_LABELS,
  questionTypesLabels,
} from "../labels"
import { updateTemplateQuestions } from "./actions"
...
export const QuestionsForm = forwardRef<WizardFormRef, QuestionsFormProps>(
  ({ templateId, initialData, onValidChange }, ref) => {
      ...
      if (checked) {
        setQuestionTypes((curr) => [
          ...curr,
          {
            ...
            count: DEFAULT_COUNT,
            difficulty: balancedDifficulty(DEFAULT_COUNT),
          },
        ])
      } else {
    ...
    const handleCountChange = useCallback(
      (typeValue: string, delta: number) => {
        setQuestionTypes((curr) =>
          curr.map((qt) => {
            if (qt.type !== typeValue) return qt
            const newCount = Math.max(1, qt.count + delta)
            // Keep the difficulty split coherent with the total so the
            // difficulty step loads pre-filled and balanced.
            return {
              ...qt,
              count: newCount,
              difficulty: balancedDifficulty(newCount),
            }
          })
        )
      },
        ...
        new Promise<void>((resolve, reject) => {
          startTransition(async () => {
            try {
              ...
              const result = await updateTemplateQuestions(templateId, {
                questionTypes,
              })
              if (!result.success) {
