...
- **Auto-generate an exam into the school's own `.docx` (2026-08-14)** — the paper path is now usable by a school that has never seen docxtemplater syntax. The engine lives in the [documents block](../documents/ISSUE.md) (full detail there); what changed for exams: (1) **Starter `.docx`** per category, in the school's language, offered next to Upload — the entry problem was that an untagged upload stores fine and fills as a blank copy of itself. (2) **The upload dialog was teaching broken loop syntax** — it showed `{#tag}`, but the engine runs `{{ }}` delimiters, under which a single-brace loop prints _literally_ and drops its body while `detectMergeFields` still reports the inner tags, so the coverage badges looked healthy. Now `{{#tag}}`, with a regression guard. (3) The **`EXAM_PAPER` resolver** gained a `{{#sections}}` loop (questions grouped by type with per-section mark totals) beside the flat `{{#questions}}` one, plus `typeLabel` / `hasOptions` / `isMcq` / `answerLines` (blank ruled space sized by question type) and `startTime` / `endTime` / `instructions` / counts — a school paper reads in sections, which one flat list cannot express. `FIELD_VOCAB` grew in lockstep or the coverage badge would call the new tags unsupported. (4) **A short paper now says so**: `generateExamQuestions` has always degraded rather than failed (the P1 in `generate/ISSUE.md` claiming a hard failure was stale and is corrected there), but `generateExamPaperFromTemplate` discarded `distributionMet` / `missingCategories`, so an under-stocked bank handed a teacher a silently short exam. Also fixed: the resolver passed a Prisma `Decimal` straight into the template as a question's marks. tsc 0 · build green · exams+documents 308/308 (+22).
