...
describe("createLiveClass (external provider)", () => {
  const externalInput = {
    ...goodInput,
  ...
  it("external → single create with meetingUrl, no SFU room update", async () => {
    mockAdmin()
    const result = await createLiveClass(externalInput)
    expect("success" in result && result.success).toBe(true)
    expect(db.liveClassSession.create).toHaveBeenCalledWith(
      expect.objectContaining({
...
describe("startLiveClass / endLiveClass (external provider)", () => {
  it("start external → flips to live WITHOUT provisioning an SFU room", async () => {
    mockTeacher()
  ...
  it("end external → flips to ended WITHOUT tearing down an SFU room", async () => {
    mockTeacher()
    vi.mocked(db.liveClassSession.findFirst).mockResolvedValue({
      id: SESSION_ID,
      status: "live",
