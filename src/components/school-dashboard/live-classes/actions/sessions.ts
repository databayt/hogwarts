...
async function createLiveClassWithCtx(
  ...
  try {
    ...
    if (provider === "external") {
      // External provider: store the pasted meeting URL, no SFU room, no recording.
      session = await db.liveClassSession.create({
        data: {
          ...baseData,
      ...
      // LiveKit: two-step create so we can write the final roomName containing the cuid.
      ...
      session = await db.liveClassSession.update({
        where: { id: created.id },
    ...
    // "Set once & reuse" — persist the link as the section+subject default so
    // every weekly recurrence of this class surfaces the same Join button.
    if (
      provider === "external" &&
      data.saveAsDefault &&
      data.sectionId &&
      ...
      try {
        const { term } = await resolveActiveTerm(ctx.schoolId)
        if (term) {
        ...
        // best-effort — default-link persistence must not fail the schedule.
      }
    }
...
export async function startLiveClass(input: IdOnly) {
  ...
  try {
    ...
    // External sessions have no SFU room — "start" just flips status to live so
    // students see the Join button. Only LiveKit provisions an SFU room.
...
export async function endLiveClass(input: IdOnly) {
  ...
  try {
    ...
    // Only LiveKit sessions have an SFU room to tear down.
