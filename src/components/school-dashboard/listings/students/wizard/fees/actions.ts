...
export async function applyFeeAdjustments(
  ...
  try {
    ...
    // Permission: admin must have fees:approve to apply scholarship/override
    ...
    // Apply to each assignment inside a transaction so state is consistent
    await db.$transaction(async (tx) => {
      for (const assignment of assignments) {
        ...
        // Strip previously-applied ADMIN_OVERRIDE + SCHOLARSHIP entries so we
        // don't stack adjustments when admin re-saves
        ...
        // Sync linked invoice totals (best-effort)
...
/**
 * Expose whether the current admin can apply fee adjustments. Used by the
 * wizard fees step to decide whether to render the admin controls panel.
 */
