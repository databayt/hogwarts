...
export function createEnrollmentSchema(v?: ValidationHelper) {
  const req = v?.required() || "Required"
  return z.object({
    studentId: z
      .string()
      .min(1, v?.get("studentIdRequired") || "Student is required"),
    batchId: z.string().min(1, req),
    courseId: z.string().optional(),
    sectionId: z.string().optional(),

    // Academic Details
    academicYearId: z.string().min(1, req),
    ...
    mandatorySubjects: z
      ...
      .min(
        1,
        v?.get("atLeastOne") || "At least one mandatory subject is required"
      ),
...
export function createBatchTransferSchema(v?: ValidationHelper) {
  const req = v?.required() || "Required"
  return z.object({
    studentId: z
      .string()
      .min(1, v?.get("studentIdRequired") || "Student is required"),
    fromBatchId: z.string().min(1, req),
    toBatchId: z.string().min(1, req),
    reason: z
      .string()
      .min(10, v?.minLength(10) || "Reason must be at least 10 characters")
      .max(500),
    effectiveDate: z.date(),
  })
}

export const batchTransferSchema = createBatchTransferSchema()

export function createCourseSchema(v?: ValidationHelper) {
  const req = v?.required() || "Required"
  return z.object({
    name: z.string().min(1, req).max(100),
    code: z.string().min(1, req).max(20),
    description: z.string().max(500).optional(),
    yearLevelId: z.string().min(1, req),
...
export function createSubjectSchema(v?: ValidationHelper) {
  const req = v?.required() || "Required"
  return z.object({
    name: z.string().min(1, req).max(100),
    code: z.string().min(1, req).max(20),
...
export function createSectionSchema(v?: ValidationHelper) {
  const req = v?.required() || "Required"
  return z.object({
    batchId: z.string().min(1, req),
    name: z.string().min(1, req).max(10),
...
export const sectionSchema = createSectionSchema()
