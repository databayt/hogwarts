...
- [x] Three dashboards stopped presenting invented numbers as real (2026-09-12) — **accountant:** five of six months in "Revenue vs Expenses", all five days of "Weekly Collections", every "Financial Calendar" deadline, a fixed seven-of-twelve fiscal year, two trend percentages, and two named students with debts shown whenever the real defaulters query came back empty. **admin:** the entire "Attendance Overview" — 96/94/92/88/91/85 per grade, 91% overall, 196 students, 178 present — identical for every school. Both removed rather than wired, since no query backs them; labels and chart components kept. **staff:** the reverse — real data under `mock`-prefixed names, renamed
- [x] `DashboardSkeleton` rewritten against measurements, not shapes (2026-09-12) — one shared body for every role now that the order is unified, plus three tails (none / learner pair / four-tile metric row). `NeutralBodySkeleton`, which drew three sections in the wrong order for guardian, staff and accountant, is gone, as are the hero, Quick Look and attendance placeholders. Invoice table drew a 96px empty band for everyone and was short by up to 394px; five rows now. Re-measured at 1440 against every demo role
- [ ] `staffDashboard`, `accountantDashboard` and `adminDashboard` dictionary blocks — every string in `staff.tsx`, `accountant.tsx` and `admin-client.tsx` is hardcoded English on an Arabic-first product. ~30 keys each, both JSONs
- [ ] Accountant money formatting — `$` hardcoded with a hand-rolled `/1000` "K" (a school on SDG sees `$10593K`). Should be `formatCurrency` from `lib/i18n-format` against `School.currency`; the collection rate is also read from two different sources in the same page and disagrees with itself (10% in the gauge, 13% in the progress card)
- [ ] `principal.tsx` — 800 lines of dead code behind an unreachable switch case, including ~49 mock references. Delete the file and the case, or add PRINCIPAL to `UserRole`
