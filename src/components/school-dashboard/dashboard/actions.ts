...
import { getActiveAlertsForSchool, loadUpcomingData } from "./upcoming-queries"
...
export async function getFeeCollectionMetrics() {
  ...
  return loadFeeCollectionMetrics(schoolId)
}
...
export async function getActiveAlerts(): Promise<EmergencyAlert[]> {
  const { schoolId } = await getTenantContext()
  if (!schoolId) throw new Error("Missing school context")
  return getActiveAlertsForSchool(schoolId)
}
...
export async function getUpcomingDataByRole(role: string) {
  ...
  if (role.toUpperCase() === "PRINCIPAL") {
    return getPrincipalUpcomingData(schoolId)
  }
  return loadUpcomingData(userId, schoolId, role)
}
 ...
 *
 * A thin session wrapper over `loadResourceUsage` in `queries.ts` — the same
 * rows the mobile `/api/mobile/dashboard/sections` route serves, so the phone
 * web dashboard and the Android dashboard cannot drift.
 */
export async function getResourceUsageByRole(role: string) {
  ...
  const session = await auth()
  return loadResourceUsage(role, { schoolId, userId: session?.user?.id })
}

/**
 * Get invoice history based on user role.
 *
 * Reads the shared `loadInvoices` and formats for the web's `InvoiceHistory`:
 * an Arabic-formatted date and a `$`-prefixed amount.
 */
export async function getInvoicesByRole(role: string) {
  ...
  const rows = await loadInvoices(role, { schoolId, userId })
  return rows.map((row) => ({
    id: row.id,
    date: formatDate(row.date, "ar"),
