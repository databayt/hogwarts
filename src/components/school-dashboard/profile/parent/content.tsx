...
import type { ConnectionStatus, ParentProfile } from "../types"
...
const generateMockParentProfile = (): ParentProfile =>
  ({
    ...
    coverImage:
      "https://images.unsplash.com/photo-1516627145497-ae6968895b74?w=1200&h=400&fit=crop",
...
export function ParentProfileContent({
  ...
  const { data: contributionData } = useServerContributions(parentId)
  const { data: pinnedItems } = useServerPinnedItems(parentId)
  const { data: recentActivity } = useServerActivity(parentId, 10)
