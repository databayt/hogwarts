...
import { Building2, Clock, ThumbsUp } from "lucide-react"
...
const createAppendixContent = (userData: UserSkillsData | null) => {
  return {
    resume: {
      title: "السيرة",
    },
...
export default function Contribute() {
  ...
  const getCurrentDocumentTitle = (): string => {
    ...
    return (
      appendixContent[activeAppendix as keyof typeof appendixContent]?.title ||
      "المستند"
    )
  }
            ...
            <div>
              ...
              (userData?.interests && userData.interests.length > 0) ? (
                ...
                <p className="text-muted-foreground py-2 text-center">
                  لا توجد مهارات أو اهتمامات متاحة
                </p>
              )}
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              {userData?.partyMember && (
                <div className="">
                  <p className="text-md">
                    عضو في{" "}
                    {userData.partyName
                      ? `حزب ${userData.partyName}`
                      : "حزب سياسي"}
                    {(userData?.partyStartDate || userData?.partyEndDate) &&
                      " في الفترة من "}
                    {userData.partyStartDate
                      ? formatDateWithArabicMonth(
                          new Date(userData.partyStartDate)
                        )
                      : ""}
                    {userData.partyEndDate
                      ? ` إلى ${formatDateWithArabicMonth(new Date(userData.partyEndDate))}`
                      : userData.partyStartDate
                        ? " حتى الآن"
                        : ""}
                  </p>
              ...
              {userData?.unionMember && (
                <div className="">
                  <p className="text-md">
                    عضو في{" "}
                    {userData.unionName
                      ? `نقابة ${userData.unionName}`
                      : "نقابة"}
                    {(userData?.unionStartDate || userData?.unionEndDate) &&
                      " في الفترة من "}
                    {userData.unionStartDate
                      ? formatDateWithArabicMonth(
                          new Date(userData.unionStartDate)
                        )
                      : ""}
                    {userData.unionEndDate
                      ? ` إلى ${formatDateWithArabicMonth(new Date(userData.unionEndDate))}`
                      : userData.unionStartDate
                        ? " حتى الآن"
                        : ""}
                  </p>
              ...
              {userData?.clubMember && (
                <div className="">
                  <p className="text-md">
                    عضو في{" "}
                    {userData.clubName ? ` ${userData.clubName}` : "نادي"}
        ...
        <CardContent className="space-y-3 pt-0">
          <div className="flex items-start gap-2">
            <p className="text-muted-foreground text-sm leading-normal">
              أتمتة الاعمال · تطوير الويب · الانظمة المدمجة
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <g fill="none">
                ...
                <path
                  fill="currentColor"
                  d="M8 1C4.136 1 1 4.136 1 8s3.136 7 7 7s7-3.136 7-7s-3.136-7-7-7m1 11H7V7.5h2zm0-6H7V4h2z"
                />
              </g>
            </svg>
        ...
        <CardContent className="pt-0">
          <p className="text-muted-foreground mb-4 text-sm">
            يمكن الاطلاع على المستندات والمرفقات التالية للحصول على مزيد من
            المعلومات.
          </p>
