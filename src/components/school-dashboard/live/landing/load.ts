...
import { resolveLandingViewer } from "./viewer"
...
/**
 * Everything the /live landing shows, for one reader.
 *
 * The web page and `/api/mobile/live/landing` both call this, so a phone and a
 * browser signed in as the same person read the same classes in the same
 * phases, ranked the same way. It used to live inline in the page, and the
 * mobile route had its own list query beside it — no viewer rules, no phase,
 * no catch-up or recording ranking — which is how the two drifted.
 *
 * It takes the reader as plain arguments rather than reading a session, since
 * the web's comes from `auth()` and the phone's from a bearer token. The
 * admin-only readiness band stays in the page: its coverage read is a server
 * action that authorises against the web session.
 */
export async function loadLiveLanding(opts: {
  schoolId: string
  ...
  const { schoolId, userId, role, lang, isDemo } = opts
  ...
  try {
    ...
    const [scope, attendeeUserIds] = await Promise.all([
      resolveViewerSectionScope(schoolId, userId, role),
      resolveCatchUpAttendees(schoolId, userId, role),
    ])
  ...
  return { viewer, policy, live, upcoming, catchUp, recordings }
}
