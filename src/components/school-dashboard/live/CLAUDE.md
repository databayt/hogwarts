...
> **Superseded the same day** by the section below: the row of five is gone
> and the transport is copied after all.
...
## The in-call chrome IS File.png (2026-09-13, later)

Abdout, asked directly whether to keep the row or match `public/apple-tv/File.png`
fully: **match it fully**. This supersedes the 09-03 "row of five" choice and
the section above. The room and the lumos player now draw the same frame.

| File.png                     | the room                                                |
| ---------------------------- | ------------------------------------------------------- |
| ✕                            | leave (`room.disconnect()`)                             |
| pill: PiP · AirPlay · Share  | pill: people · fill/fit · share the class page          |
| speaker circle               | speaker: mutes the class ON THIS DEVICE                 |
| ⟲10 · ▶ · ⟳10 (64 · 92 · 64) | camera · MICROPHONE · hand (host: screen share)         |
| title + ⋯                    | `● live · subtitle` / subject + ⋯ (drill-in card)       |
| clock · track · −clock       | `ClassProgress`, phone clocks `0:07:22`, 8px bare track |
| Info · InSight · Continue    | Discussion (with count) · Raised hands (host, if any)   |

- **`control-bar.tsx` exports three pieces**, not a bar: `ClassTransport`,
  `ClassCapsules`, `ClassMoreMenu`. The shell places them. Measured at 390px:
  the discs centre on 95 / 195 / 295 at y 422, the reference's own centres.
- **The speaker is `RoomAudioRenderer muted`**, which now lives INSIDE
  `RoomShell` (moved from `room.tsx`) so the shell owns the state. `muted` also
  tells the SFU to stop sending audio — it is a device-local silence, not a
  moderation tool.
- **The connection moved into the ⋯ card** as its first drill-in row (the
  film's "Playback Speed ›"), its symbol tinted by `QUALITY_TONE`, its current
  label as the row's value. `QualityMenuButton` is gone.
- **The ⋯ card is the lumos card**: `phoneMenuCard` / `phoneMenuRow` moved into
  `glass.ts` and both surfaces use them. A drill-in REPLACES the rows with a
  back row plus the list (quality tiers, microphones, cameras, slides); device
  choice is a checkmark list now, not a `<select>`.
- **Every glyph is an SF Symbol traced from macOS** —
  `lumos/shared/video-player/sf-symbols.tsx`, generated by
  `scripts/sf-icons/extract.py` from `manifest.json`. `<SfSymbol glyph pt>`
  sizes by POINT SIZE, not a box, so symbols of different shapes keep iOS's
  rhythm. Point sizes and weights were matched against the captures (top row
  20pt regular, skips 31pt medium, play 44 / pause 51 medium, ⋯ 18pt, menu
  rows 17pt, chevrons 12pt semibold). Never hand-edit the generated file.
- **A stage tap beside the discs still toggles the chrome**: the centre layer
  is `pointer-events-none` and only the trio takes pointer events. A tap ON the
