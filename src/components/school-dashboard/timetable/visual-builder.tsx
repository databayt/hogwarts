...
function DraggableSubject({ subject }: { subject: Subject }) {
  ...
  return (
    <div
      ...
      {...listeners}
      className={cn(
        "cursor-move rounded-lg p-3 transition-colors",
        "hover:shadow-md",
        subject.color,
        "text-white"
      )}
...
function TimetableCell({
  ...
  const isBreak =
    timeSlot.label.toLowerCase().includes("break") ||
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "relative min-h-[80px] rounded-lg border p-2 transition-colors",
    ...
    >
      {entry ? (
        <div
          className={cn(
            "h-full rounded p-2 text-sm text-white",
...
export function TimetableBuilder({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (
      active.data.current?.type === "subject" &&
    ...
    >
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {timeSlots.map((timeSlot) => (
                        <tr key={timeSlot.id}>
                          <td className="bg-muted border p-2 text-xs font-medium">
      ...
      <DragOverlay>
        {activeSubject && (
          <div
            className={cn(
              "cursor-move rounded-lg p-3 shadow-lg",
              activeSubject.color,
              "text-white opacity-90"
            )}
          >
            <div className="text-sm font-medium">{activeSubject.name}</div>
