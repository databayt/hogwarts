...
import { useTimetableStore } from "@/components/school-dashboard/timetable/timetable"

import type {
  ClassesApiResponse,
  ConflictsApiResponse,
  TeachersApiResponse,
  TermsApiResponse,
} from "./types"
...
export function TimetableHeader({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (!selectedTeacher && nextTeachers[0])
        setSelectedTeacher(nextTeachers[0].id)
    })()
  }, [termId])
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const params: {
        ...
        classId?: string
        teacherId?: string
      } = { termId, weekOffset: isNextWeek ? 1 : 0 }
      ...
      const cd = await safeFetchJson<ConflictsApiResponse>(
        `/api/timetable/conflicts?termId=${termId}`
      )
    ...
    <div className="mb-2 print:mb-8">
      <div className="mb-4 flex flex-wrap items-center gap-2 print:hidden">
        ...
        <Button
          ...
          className={cn(
            "rounded-xl border-neutral-200 bg-neutral-50 hover:bg-neutral-100 dark:border-neutral-700 dark:bg-neutral-800 dark:hover:bg-neutral-900",
            "px-3 transition-all duration-200",
            isWeekChangeLoading && "cursor-not-allowed opacity-50"
          )}
        >
          <ChevronLeft
            className={cn(
        ...
        {conflicts != null && (
          <button
            className={cn(
              "muted underline-offset-2",
              conflicts > 0 ? "text-destructive underline" : ""
            )}
      ...
      <ScheduleSettingsDialog
        ...
        termId={termId}
        onSaved={() => {
          /* no-op, grid will reflect on next load */
        }}
      />
      <SlotEditor
        open={openSlot}
        ...
        termId={termId}
        initialDayOfWeek={applyTarget.dayOfWeek}
        initialPeriodId={applyTarget.periodId}
      />
      <ConflictsDrawer
        termId={termId}
        open={openConflicts}
