...
interface AttendanceContextType {
  ...
  addStudentIdentifier: (
    identifier: Omit<StudentIdentifier, "id">
  ) => Promise<void>
...
export function AttendanceProvider({
  ...
  const [currentMethod, setCurrentMethod] =
    useState<AttendanceMethod>(initialMethod)
  ...
  const [permissions] = useState<AttendancePermissions>(
    userPermissions || {
      canMarkManual: true,
      canUseGeofence: true,
      canScanQR: true,
          ...
          await markSingleAttendance({
            ...
            date:
              typeof record.date === "string"
                ? record.date
                : record.date?.toISOString?.() || selectedDate,
  ...
  const markBulkAttendance = useCallback(
    async (records: Partial<AttendanceRecord>[]) => {
      setLoading(true)
      setError(null)
      try {
        ...
        if (records.length > 0 && records[0].classId) {
          ...
          const date =
            typeof recordDate === "string"
              ? recordDate
              : recordDate?.toISOString?.() || selectedDate
  ...
  const fetchStudentIdentifiers = useCallback(async (studentId?: string) => {
    ...
    try {
      ...
      const identifiers: StudentIdentifier[] = result.identifiers.map(
        (i: any) => ({
          id: i.id,
          schoolId: "", // Will be filled by server context
  ...
  const addStudentIdentifier = useCallback(
    async (identifier: Omit<StudentIdentifier, "id">) => {
      setLoading(true)
      setError(null)
      try {
  ...
  const checkMethodSupport = useCallback(
    (method: AttendanceMethod): boolean => {
