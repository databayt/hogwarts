 ...
 *
 * Theme customization settings for the school-dashboard.
...
import { Download, Eye, Palette, Settings } from "lucide-react"
...
const PresetGallery = React.lazy(() =>
  import("@/components/theme/preset-gallery").then((m) => ({
    default: m.PresetGallery,
  }))
)
const ThemeImportExport = React.lazy(() =>
  import("@/components/theme/import-export").then((m) => ({
    default: m.ThemeImportExport,
  }))
)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <h3>
          {dictionary?.school?.settings?.appearanceTitle ||
            "Appearance Settings"}
        </h3>
        <p className="muted">
          {dictionary?.school?.settings?.appearanceDescription ||
            "Customize your school-dashboard appearance with beautiful preset themes or create your own."}
        </p>
      </div>
      ...
      >
        ...
        <TabsContent value="presets" className="space-y-6">
          <div className="space-y-4">
            <div className="space-y-2">
              <h4>
                {dictionary?.school?.settings?.presetThemes || "Preset Themes"}
              </h4>
              <p className="muted">
        ...
        <TabsContent value="custom" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                {dictionary?.school?.settings?.customThemeBuilder ||
                  "Custom Theme Builder"}
              </CardTitle>
              <CardDescription>
            ...
            <CardContent>
              <div className="rounded-lg border border-dashed p-12 text-center">
                ...
                <h4 className="mt-4">
                  {dictionary?.school?.settings?.customThemeBuilderTitle ||
                    "Custom Theme Builder"}
                </h4>
  ...
  return (
    <div className="flex flex-wrap gap-3">
      {Array.from({ length: 16 }).map((_, i) => (
        <div
          key={i}
          className="bg-muted h-[88px] w-[140px] animate-pulse rounded-lg"
        />
      ))}
    </div>
  )
