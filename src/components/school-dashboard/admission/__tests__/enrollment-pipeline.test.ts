...
// canonical fee-invoice-sync helper (dynamically imported inside
// ensureStudentFeeAssignments). Mock it so we can assert per-assignment sync.
...
describe("confirmEnrollment - full pipeline", () => {
  ...
  it("creates fee assignments from active fee structures", async () => {
    // Fee assignment is delegated to the canonical ensureStudentFeeAssignments
    // helper, which runs against this tx client. It matches active structures
    // for the student's grade + academic year and inserts the missing rows via
    // feeAssignment.create (diffing against existing rather than blind upsert).
    // Give the student a resolved academicGradeId so the helper does real work:
    // first student.findUnique = cross-school check (null → create),
    // second = academicGradeId lookup feeding the helper.
    vi.mocked(db.student.findUnique)
      .mockResolvedValueOnce(null)
      .mockResolvedValue({ academicGradeId: "grade-1" } as any)
    vi.mocked(db.feeStructure.findMany).mockResolvedValue([
      {
        id: "fs-1",
        totalAmount: 5000,
    ...
    vi.mocked(db.feeAssignment.create)
      .mockResolvedValueOnce({ id: "fa-1" } as any)
      .mockResolvedValueOnce({ id: "fa-2" } as any)

    const result = await confirmEnrollment({ id: "app-1" })

    expect(result.success).toBe(true)
    expect(db.feeAssignment.create).toHaveBeenCalledTimes(2)
    expect(db.feeAssignment.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        schoolId: SCHOOL_ID,
        studentId: "student-new",
        feeStructureId: "fs-1",
  ...
  it("syncs an invoice for each newly created fee assignment", async () => {
    // Invoice creation is no longer inlined in confirmEnrollment; the canonical
    // fee helper fans out one invoice per newly created assignment via
    // ensureInvoicesForAssignment(schoolId, assignmentId, tx).
    vi.mocked(db.student.findUnique)
      .mockResolvedValueOnce(null)
      .mockResolvedValue({ academicGradeId: "grade-1" } as any)
    vi.mocked(db.feeStructure.findMany).mockResolvedValue([
    ...
    vi.mocked(db.feeAssignment.findMany).mockResolvedValue([])
    vi.mocked(db.feeAssignment.create).mockResolvedValue({ id: "fa-1" } as any)

    const result = await confirmEnrollment({ id: "app-1" })

    expect(result.success).toBe(true)
    expect(ensureInvoicesForAssignment).toHaveBeenCalledWith(
      SCHOOL_ID,
      "fa-1",
  ...
  it("succeeds even when invoice generation fails", async () => {
    ...
    vi.mocked(db.feeAssignment.findMany).mockResolvedValue([])
    vi.mocked(db.feeAssignment.create).mockResolvedValue({ id: "fa-1" } as any)
    vi.mocked(ensureInvoicesForAssignment).mockRejectedValue(
  ...
  it("creates guest user for applications without userId", async () => {
    ...
    expect(result.success).toBe(true)
    // Guest user gets the per-school generated student code as username
    // (YYGGNNNN), produced by generateStudentUsername inside the tx — not the
    // applicant's display name.
    expect(db.user.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        email: "ahmed@test.com",
        username: expect.stringMatching(/^\d+$/),
        role: "STUDENT",
        schoolId: SCHOOL_ID,
