...
import { DEFAULT_SETTINGS, FILE_UPLOAD_CONFIG } from "./config"
...
export const createConversationSchema = z
  ...
  .superRefine((val, ctx) => {
    ...
    if (
      ["group", "class", "department", "announcement"].includes(val.type) &&
      !val.title
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const createConversationInviteSchema = z
  .object({
    conversationId: z.string().min(1, "Conversation ID is required"),
...
export const getMessagesListSchema = z
  .object({
    conversationId: z.string().min(1, "Conversation ID is required"),
