...
export const createPayrollRunSchema = (v: ValidationHelper) =>
  z
    .object({
      runNumber: z.string().min(1, v.required()).max(50),
      periodStart: z.date(),
      periodEnd: z.date(),
      paymentDate: z.date(),
    ...
    .refine((data) => data.periodEnd >= data.periodStart, {
      message: "Period end date must be after start date", // TODO: add custom validation key
      path: ["periodEnd"],
    })
    .refine((data) => data.paymentDate >= data.periodEnd, {
      message: "Payment date must be after period end date", // TODO: add custom validation key
      path: ["paymentDate"],
    })

export const createSalarySlipSchema = (v: ValidationHelper) =>
  z.object({
    payrollRunId: z.string().min(1, v.required()),
    teacherId: z.string().min(1, v.required()),
    slipNumber: z.string().min(1, v.required()).max(50),
    periodStart: z.date(),
    periodEnd: z.date(),
...
export const createTaxCalculationSchema = (v: ValidationHelper) =>
  z.object({
    grossSalary: z.number().min(0, v.min(0)),
    taxExemptions: z.number().min(0).default(0),
    dependents: z.number().min(0).default(0),
    additionalDeductions: z.number().min(0).default(0),
  })

export const createPayrollApprovalSchema = (v: ValidationHelper) =>
  z
    .object({
      runId: z.string().min(1, v.required()),
      ...
      (data) => {
        if (data.action === "REJECT") {
          return (
            ...
            data.rejectionReason.length > 0
          )
        }
        return true
      },
      {
        message: "Rejection reason is required when rejecting", // TODO: add custom validation key
        path: ["rejectionReason"],
      }
    )

export const createPayrollDisbursementSchema = (v: ValidationHelper) =>
  z.object({
    runId: z.string().min(1, v.required()),
    slipIds: z.array(z.string()).optional(),
...
export const createPayrollAdjustmentSchema = (v: ValidationHelper) =>
  z.object({
    slipId: z.string().min(1, v.required()),
