...
7. **One gate per page, not one per action**: pages resolve permissions through `guard.ts` `resolveFinanceAccess(module, actions)`, which reads session + tenant once and evaluates every action concurrently. All 13 gated overview pages now use it; a chain of `checkCurrentUserPermission` calls is the anti-pattern it replaced (one session read + one user lookup serialized per action)
