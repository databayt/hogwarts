...
- **A blueprint the bank cannot fill was a dead end (2026-08-28)** — a PARTIAL shortfall named the unfilled slots; a TOTAL failure returned a bare "no matching questions". The per-slot breakdown was already computed on that path — `autoGenerateExamQuestions` discarded it, and `generateExamPaperFromTemplate` dropped `details` again on the way out. Both carry it now and the dialog lists the slots under the error. **Closed 2026-08-29:** `adoptExamTemplate` now runs `validateDistribution` (previously exported and called from nowhere) before copying a catalog distribution, and refuses with `DISTRIBUTION_INVALID` instead of the old blind `as any`. Note the path is currently unreachable from the UI — `adopt-exam-dialog` calls `adoptExam`, which adopts a catalog EXAM and writes no distribution — so this is protection for whoever wires it up, not a live fix.
