...
import { formatCurrency, getDaysUntilDue, isInvoiceOverdue } from "./util"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="muted">
            Quick overview of recent and important invoices
          </p>
        </div>
      </div>
            ...
            <div className="text-end">
              ...
              <small className="muted">
                {new Intl.RelativeTimeFormat("en", { numeric: "auto" }).format(
                  Math.ceil(
                    (new Date(invoice.createdAt).getTime() - Date.now()) /
                      (1000 * 60 * 60 * 24)
                  ),
                  "day"
                )}
      ...
      <div className="space-y-3">
        {overdueInvoices.map((invoice) => {
          ...
          return (
            <div
              key={invoice.id}
              className="flex items-start justify-between rounded-lg border border-red-200 bg-red-50/50 p-4 dark:bg-red-950/10"
            >
 ...
 */
export function InvoicesDueSoon({
  invoices,
  maxItems = 5,
}: {
  invoices: InvoiceWithSchool[]
      ...
      <div className="space-y-2">
        {dueSoonInvoices.map((invoice) => {
          ...
          return (
            <div
              key={invoice.id}
              className="flex items-center justify-between rounded-lg border border-amber-200 bg-amber-50/50 p-4 dark:bg-amber-950/10"
            >
              <div className="flex items-center gap-3">
