 ...
 */
export async function domainApprove(input: {
  id: string
  notes?: string
  ...
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error : new Error("Failed to approve domain"),
    }
  }
}
 ...
 */
export async function domainReject(input: {
  id: string
  notes?: string
}): Promise<ActionResult<DomainRequest>> {
  try {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error : new Error("Failed to reject domain"),
    }
  }
}
 ...
 */
export async function domainVerify(input: {
  id: string
  ...
  try {
    ...
    if (isVerified) {
      ...
      return {
        success: true,
        data: { verified: true, domainRequest: updatedRequest },
      }
    } else {
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error : new Error("Failed to verify domain"),
    }
  }
}
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error
          : new Error("Failed to create domain request"),
    }
  }
}
    ...
    return {
      ...
      error: {
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch domain requests",
      },
      data: [],
      total: 0,
    }
