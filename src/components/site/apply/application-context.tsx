...
import { AUTO_SAVE_INTERVAL, APPLY_STEPS, type ApplyStep } from './config.client';
import type { ApplySessionState, PublicCampaign, ApplicationFormData } from './types';
import { saveApplicationSession, resumeApplicationSession } from '@/components/site/admission/actions';
...
export const ApplicationProvider: React.FC<ApplicationProviderProps> = ({
  ...
  const initSession = useCallback(async (campaignId: string, subdomainValue: string) => {
    setSession(prev => ({ ...prev, isLoading: true, error: null }));
    setSubdomain(subdomainValue);

    try {
      // Create a new session or restore from localStorage
      const storedSession = localStorage.getItem(`${STORAGE_KEY}_${campaignId}`);
      if (storedSession) {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setSession(prev => ({
          ...
          currentStep: (data.formData as { currentStep?: ApplyStep })?.currentStep || 'personal',
          isLoading: false
        }));
        return true;
      } else {
  ...
  // Update step data
  const updateStepData = useCallback(<T extends keyof ApplySessionState['formData']>(
    step: T,
    data: ApplySessionState['formData'][T]
  ) => {
    setSession(prev => ({
  ...
  // Get step data
  const getStepData = useCallback(<T extends keyof ApplySessionState['formData']>(
    step: T
  ): ApplySessionState['formData'][T] | undefined => {
  ...
  const saveSession = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        localStorage.setItem(`${STORAGE_KEY}_${session.campaignId}`, JSON.stringify({
  ...
  }, [subdomain, session.campaignId, session.formData, session.currentStep, session.sessionToken]);
