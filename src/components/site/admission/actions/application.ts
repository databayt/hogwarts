...
import type { ActionResult, ApplicationFormData, PublicCampaign, SubmitApplicationResult } from "../types";
 ...
 */
export async function getActiveCampaigns(subdomain: string): Promise<ActionResult<PublicCampaign[]>> {
  try {
        ...
        _count: {
          select: {
            applications: {
              where: {
                status: {
                  in: ["SUBMITTED", "UNDER_REVIEW", "SHORTLISTED", "SELECTED", "ADMITTED"]
                }
              }
            }
          }
    ...
    const publicCampaigns: PublicCampaign[] = campaigns.map(campaign => ({
      ...
      requiredDocuments: campaign.requiredDocuments as unknown as PublicCampaign["requiredDocuments"],
      eligibilityCriteria: campaign.eligibilityCriteria as unknown as PublicCampaign["eligibilityCriteria"],
    }));
 ...
 */
export async function getCampaignById(subdomain: string, campaignId: string): Promise<ActionResult<PublicCampaign>> {
  try {
    const schoolResult = await getSchoolBySubdomain(subdomain);
    if (!schoolResult.success || !schoolResult.data) {
      return { success: false, error: "School not found" };
    ...
    if (!campaign) {
      return { success: false, error: "Campaign not found or not accepting applications" };
    }
...
export async function resumeApplicationSession(
  ...
  try {
    ...
    if (session.expiresAt < new Date()) {
      ...
      return { success: false, error: "Session has expired. Please start a new application." };
    }
...
export async function submitApplication(
  ...
  try {
    ...
    if (!campaign) {
      return { success: false, error: "This admission campaign is no longer accepting applications" };
    }
    ...
    if (existingApplication) {
      return {
        success: false,
        error: "An application with this email already exists for this campaign"
      };
    }
    ...
    if (attempts >= 10) {
      return { success: false, error: "Failed to generate application number. Please try again." };
    }
    ...
    const application = await db.application.create({
      data: {
        ...
        previousMarks: validated.previousMarks ? parseFloat(validated.previousMarks) : null,
        previousPercentage: validated.previousPercentage ? parseFloat(validated.previousPercentage) : null,
        ...
        documents: validated.documents ? (validated.documents as unknown as object[]) : undefined,
        // Status
        status: "SUBMITTED",
    ...
    const requiresPayment = campaign.applicationFee && Number(campaign.applicationFee) > 0;
    ...
    if (error instanceof Error && error.message.includes("Unique constraint")) {
      return { success: false, error: "An application with this information already exists" };
    }
