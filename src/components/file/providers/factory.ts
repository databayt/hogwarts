...
import type { StorageProvider, FileCategory, StorageTier } from "../types";
 ...
 */
export function selectProvider(criteria: ProviderSelectionCriteria): StorageProvider {
  ...
  if (category === "image" && (purpose === "library" || purpose === "avatar" || purpose === "logo")) {
  ...
  if (tier === "cold") {
    if (process.env.CLOUDFLARE_R2_ACCESS_KEY_ID && process.env.CLOUDFLARE_R2_BUCKET) {
      return "cloudflare_r2";
    }
  }

  // Warm tier uses S3 if available
  if (tier === "warm" && process.env.AWS_ACCESS_KEY_ID && process.env.AWS_S3_BUCKET) {
    return "aws_s3";
  }
...
export function detectProviderFromUrl(url: string): StorageProvider {
  ...
  if (urlLower.includes("r2.cloudflarestorage.com") || urlLower.includes("r2.dev")) {
    return "cloudflare_r2";
  }
...
export function getAvailableProviders(): StorageProvider[] {
  const allProviders: StorageProvider[] = ["vercel_blob", "aws_s3", "cloudflare_r2", "imagekit"];
