...
export function isReservedDomain(domain: string): boolean {
  ...
  if (RESERVED_DOMAINS.some((reserved) => lowerDomain === reserved || lowerDomain.endsWith(`.${reserved}`))) {
    return true;
  }

  // Check if any part of the domain contains reserved keywords
  const parts = lowerDomain.split(".");
  return parts.some((part) => RESERVED_KEYWORDS.includes(part as typeof RESERVED_KEYWORDS[number]));
}
...
export function validateDomain(domain: string): DomainValidationResult {
  ...
  labels.forEach((label, index) => {
    if (label.length > VALIDATION_RULES.LABEL_MAX_LENGTH) {
      errors.push(`Label "${label}" exceeds maximum length of ${VALIDATION_RULES.LABEL_MAX_LENGTH} characters`);
    }
  ...
  // Warnings
  if (labels.length === 2) {
    warnings.push("Consider using a subdomain (e.g., school.yourdomain.com) for better organization");
  }
 ...
 */
export function generateVerificationToken(schoolId: string, domain: string): string {
 ...
 */
export function formatDomainStatus(status: DomainStatus, verifiedAt: Date | string | null): string {
 ...
 */
export function canTransitionStatus(currentStatus: DomainStatus, newStatus: DomainStatus): boolean {
 ...
 */
export function getAllowedTransitions(currentStatus: DomainStatus): DomainStatus[] {
...
export function sortDomains<
  T extends { domain?: string; schoolName?: string; status?: string; createdAt?: Date | string; verifiedAt?: Date | string | null }
>(
  domains: T[],
 ...
 */
export function getDomainHealth(status: DomainStatus): "healthy" | "warning" | "critical" {
