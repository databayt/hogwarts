...
import type { BillingPeriod, InvoiceStatus, ReceiptStatus } from "./types"
...
export function isValidReceiptFileType(file: File): boolean {
  return ALLOWED_RECEIPT_FILE_TYPES.includes(
    file.type as (typeof ALLOWED_RECEIPT_FILE_TYPES)[number]
  )
}
...
export function formatInvoiceStatus(
  ...
  if (amountPaid > 0)
    return `Partially paid (${formatCurrency(outstanding)} due)`
    ...
    if (field === "createdAt") {
      const aTime =
        typeof aVal === "string"
          ...
          : (aVal as Date)?.getTime() || 0
      const bTime =
        typeof bVal === "string"
          ? new Date(bVal).getTime()
          : (bVal as Date)?.getTime() || 0
