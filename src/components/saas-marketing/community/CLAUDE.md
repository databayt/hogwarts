...
- **No lang gating on the hub grid** (2026-06-16): `getCommunitySubjects` does NOT filter `where.lang`. Catalog subjects are single-language (US/GB/IB-DP → `en`; the national curricula → `ar`), so gating on the page locale hid 7 of the 12 curricula on `/en` and left the `/ar` default (US) completely empty. The hub is a cross-curriculum directory — every curriculum's subject cards render regardless of locale; names show in source language until catalog translation is wired. (The detail/sub-route queries never gated on lang.)
- **Public sub-routes + serializable href props** (2026-06-16): the reused school components (`CatalogHero`, `CatalogContentSections`) take optional **string** href props that default to the school-dashboard paths and are overridden here to public `/community/[slug]/...` paths. They are `"use client"`, so the props MUST be plain strings — never functions (a function prop silently breaks SSR across the server/client boundary; that's what made the grade pills vanish before the fix). `CatalogHero` → `gradeBasePath`; `CatalogContentSections` → `materialsHref` / `qbankHref` / `examsHref` (`""` to hide) / `videosHref` / `videoTileBasePath` (`""` → tiles fall back to `videosHref`). Sub-routes live at `/community/[slug]/{chapters,materials,qbank}` and inherit the hero from `[slug]/layout.tsx`.
...
- The reused `CatalogContentSections` / `CatalogHero` / `CatalogDetailContent` components default their deep links to the school-dashboard routes (`/${lang}/subjects/...`, `/${lang}/exams/...`, `/${lang}/stream/dashboard/...`) which 404 / redirect-to-login on the public domain. Every one is now overridden here to a public `/community/[slug]/...` path via the string href props (see Key Decisions). Exams have no public destination yet → `examsHref=""` suppresses that "see all" link. Video tiles have no public player → they fall back to the chapters page. **When adding a new deep link to these shared components, give it a prop default + a community override — never hardcode a school path.**
