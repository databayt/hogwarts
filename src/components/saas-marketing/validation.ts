...
export const newsletterSchema = z.object({
  ...
  consent: z.boolean().refine((val) => val === true, {
    message: "You must agree to receive saas-marketing emails",
  }),
})
...
export const demoRequestSchema = z.object({
  ...
  numberOfStudents: z
    .number()
    .int()
    .positive("Number of students must be positive")
    .max(100000, "Number exceeds maximum"),
  numberOfTeachers: z
    .number()
    .int()
    .positive("Number of teachers must be positive")
    .max(10000, "Number exceeds maximum"),
  preferredDate: z
    .date()
...
export const waitlistSchema = z.object({
  email: z.string().email("Please enter a valid email address"),
  schoolType: z
    .enum(["primary", "secondary", "university", "other"])
    .optional(),
