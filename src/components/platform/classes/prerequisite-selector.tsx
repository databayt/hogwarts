...
export function PrerequisiteSelector({ form, disabled = false, currentClassId }: PrerequisiteSelectorProps) {
  const [availableClasses, setAvailableClasses] = useState<Array<{ id: string; name: string; subjectName: string }>>([]);
  ...
  useEffect(() => {
    const loadClasses = async () => {
      try {
        ...
        setAvailableClasses([
          { id: "pre_001", name: "Introduction to Mathematics", subjectName: "Mathematics" },
          { id: "pre_002", name: "Basic Programming", subjectName: "Computer Science" },
      ...
      render={({ field }) => (
        <FormItem>
          ...
          >
            <FormControl>
              <SelectTrigger>
                <SelectValue placeholder={loading ? "Loading..." : "No prerequisite required"} />
              </SelectTrigger>
            </FormControl>
            <SelectContent>
