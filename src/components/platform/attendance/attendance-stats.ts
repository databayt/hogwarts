 ...
 */
export async function calculateAttendancePercentage(input: z.infer<typeof attendancePercentageSchema>) {
  ...
  const percentage = effectiveDays > 0
...
export async function getBulkAttendanceStats(input: {
  ...
  const studentMap = new Map(
    students.map(s => [s.id, `${s.givenName || ''} ${s.surname || ''}`.trim()])
  );
...
export async function getAttendanceTrends(input: {
  ...
  // Group by date
  const trendsByDate = records.reduce((acc, record) => {
...
function calculateStreak(records: Array<{ date: Date; status: string }>) {
  ...
  for (const record of sorted) {
    ...
    if (record.status === 'ABSENT' || record.status === 'EXCUSED' || record.status === 'SICK') {
      break;
    }
