      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={350}>
          <ComposedChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line type="monotone" dataKey="rate" stroke={COLORS.primary} strokeWidth={2} name="Attendance Rate %" />
          </ComposedChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
...
export function MethodUsagePieChart({ data, className }: MethodUsagePieChartProps) {
  const chartColors = [
    COLORS.primary,
    COLORS.secondary,
    COLORS.success,
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <RadarChart data={data}>
            ...
            <PolarAngleAxis dataKey="day" stroke="#64748b" style={{ fontSize: 12 }} />
            <PolarRadiusAxis angle={90} domain={[0, 100]} stroke="#64748b" style={{ fontSize: 10 }} />
            <Radar
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={data} layout="horizontal" margin={{ top: 5, right: 30, left: 80, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
            <XAxis type="number" domain={[0, 100]} stroke="#64748b" style={{ fontSize: 12 }} />
            <YAxis type="category" dataKey="class" stroke="#64748b" style={{ fontSize: 12 }} width={70} />
            <Tooltip
              contentStyle={{
                backgroundColor: 'rgba(255, 255, 255, 0.95)',
                border: '1px solid #e2e8f0',
...
export function StudentAttendanceHeatmap({ data, className }: StudentAttendanceHeatmapProps) {
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>Weekly attendance patterns per student</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          ...
          {data.slice(0, 10).map((student) => (
            <div key={student.student} className="grid grid-cols-6 gap-2">
              <div className="text-xs truncate">{student.student}</div>
              {days.map(day => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      value === 100 ? "bg-green-500 text-white" :
                      value >= 80 ? "bg-green-200" :
                      value >= 60 ? "bg-yellow-200" :
                      value >= 40 ? "bg-orange-200" :
                      "bg-red-200"
                    )}
                  >
                    {value}%
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>Current year vs previous year attendance</CardDescription>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <YAxis stroke="#64748b" style={{ fontSize: 12 }} domain={[80, 100]} />
            <Tooltip
              contentStyle={{
                backgroundColor: 'rgba(255, 255, 255, 0.95)',
                border: '1px solid #e2e8f0',
...
export function AbsenceReasonsChart({ data, className }: AbsenceReasonsChartProps) {
  return (
    <Card className={className}>
      <CardHeader>
