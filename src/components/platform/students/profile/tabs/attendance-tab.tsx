...
export function AttendanceTab({ student }: AttendanceTabProps) {
  ...
  const attendanceRecords = daysInMonth.map((date) => {
    ...
    const realRecord = realAttendances.find(
      (a: any) => a.date && isSameDay(new Date(a.date), date)
    )
  ...
  const allSchoolDays = attendanceRecords.filter(
  ...
  const presentRecords = realAttendances.filter(
    (a: any) =>
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "excused":
        return <CircleCheck className="h-4 w-4 text-blue-600" />
      default:
        return null
    }
  }
  ...
  const hasAttendanceData =
    realAttendances.length > 0 || stats.totalSchoolDays > 0
              ...
              return (
                <>
                  ...
                  <div
                    ...
                    className={`relative rounded-lg border p-2 text-center ${record.status === "weekend" ? "bg-muted" : ""} ${record.status === "present" ? "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/30" : ""} ${record.status === "absent" ? "border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30" : ""} ${record.status === "late" ? "border-yellow-200 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-950/30" : ""} ${record.status === "excused" ? "border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/30" : ""} ${record.status === "future" ? "bg-muted opacity-50" : ""} `}
                  >
      ...
      <Card>
        ...
        <CardContent>
          {attendanceRecords.filter(
            (r) => r.status !== "future" && r.status !== "weekend"
          ).length > 0 ? (
