...
import { lessonColumns, type LessonRow } from "./columns";
...
export function LessonsTable({ initialData, total, perPage = 20 }: LessonsTableProps) {
  const columns = useMemo(() => lessonColumns, []);

  // State for incremental loading
  const [data, setData] = useState<LessonRow[]>(initialData);
  ...
  const handleLoadMore = useCallback(async () => {
    ...
    try {
      ...
      const result = await getLessons({ page: nextPage, perPage });
