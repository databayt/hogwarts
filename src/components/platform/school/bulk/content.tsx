...
import { Calendar, Users, Building, ClipboardList, Clock, FileText, BookOpen } from 'lucide-react'
...
export default function BulkContent({ dictionary, lang }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{isArabic ? 'العمليات الجماعية' : 'Bulk Operations'}</CardTitle>
          <CardDescription>
            {isArabic
      ...
      <Tabs defaultValue="academic" className="space-y-4">
        <TabsList className="grid w-full grid-cols-7">
          <TabsTrigger value="academic" className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">{isArabic ? 'أكاديمي' : 'Academic'}</span>
          </TabsTrigger>
          <TabsTrigger value="structure" className="flex items-center gap-2">
            <Building className="h-4 w-4" />
            <span className="hidden sm:inline">{isArabic ? 'الهيكل' : 'Structure'}</span>
          </TabsTrigger>
          <TabsTrigger value="people" className="flex items-center gap-2">
            <Users className="h-4 w-4" />
            <span className="hidden sm:inline">{isArabic ? 'الأشخاص' : 'People'}</span>
          </TabsTrigger>
          <TabsTrigger value="attendance" className="flex items-center gap-2">
            <ClipboardList className="h-4 w-4" />
            <span className="hidden sm:inline">{isArabic ? 'الحضور' : 'Attendance'}</span>
          </TabsTrigger>
          <TabsTrigger value="timetable" className="flex items-center gap-2">
            <Clock className="h-4 w-4" />
            <span className="hidden sm:inline">{isArabic ? 'الجدول' : 'Timetable'}</span>
          </TabsTrigger>
          <TabsTrigger value="exams" className="flex items-center gap-2">
            <FileText className="h-4 w-4" />
            <span className="hidden sm:inline">{isArabic ? 'الامتحانات' : 'Exams'}</span>
          </TabsTrigger>
          <TabsTrigger value="materials" className="flex items-center gap-2">
            <BookOpen className="h-4 w-4" />
            <span className="hidden sm:inline">{isArabic ? 'المواد' : 'Materials'}</span>
          </TabsTrigger>
        </TabsList>
