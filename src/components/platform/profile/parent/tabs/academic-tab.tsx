...
const mockReportCards: ReportCard[] = [
  {
    ...
    year: 2023,
    subjects: [
      { name: 'Mathematics', grade: 'A', score: 92, credits: 4, teacher: 'Mr. Smith', remarks: 'Excellent problem-solving skills' },
      { name: 'Science', grade: 'A-', score: 88, credits: 4, teacher: 'Ms. Johnson', remarks: 'Shows great curiosity' },
      { name: 'English', grade: 'B+', score: 85, credits: 4, teacher: 'Mrs. Davis', remarks: 'Improving writing skills' },
      { name: 'History', grade: 'A', score: 90, credits: 3, teacher: 'Mr. Wilson' },
      { name: 'Art', grade: 'A+', score: 95, credits: 2, teacher: 'Ms. Chen', remarks: 'Very creative' }
    ],
    gpa: 3.8,
    rank: 5,
    totalStudents: 120
  },
  {
    ...
    year: 2023,
    subjects: [
      { name: 'Mathematics', grade: 'A', score: 95, credits: 4, teacher: 'Mr. Brown', remarks: 'Outstanding performance' },
      { name: 'Science', grade: 'A', score: 94, credits: 4, teacher: 'Dr. Lee', remarks: 'Natural scientist' },
      { name: 'English', grade: 'A', score: 91, credits: 4, teacher: 'Ms. Taylor', remarks: 'Excellent essays' },
      { name: 'History', grade: 'B+', score: 86, credits: 3, teacher: 'Mr. Anderson' },
      { name: 'Music', grade: 'A+', score: 98, credits: 2, teacher: 'Ms. Rodriguez', remarks: 'Gifted musician' }
    ],
    gpa: 3.9,
    rank: 3,
    totalStudents: 100
...
export function AcademicTab({
  ...
  const filteredAssignments = mockAssignments.filter(assignment => {
    if (selectedChild !== 'all' && assignment.childId !== selectedChild) return false
  ...
  const averageGrade = mockAssignments
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <Tabs defaultValue="assignments" className="space-y-4">
                      ...
                      {assignment.score !== undefined && (
                        <div className="flex items-center gap-4 mt-2">
                          ...
                          <span className="text-sm">
                            {assignment.score}/{assignment.maxScore} ({(assignment.score / assignment.maxScore * 100).toFixed(0)}%)
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-semibold text-sm">
                          {exam.subject} - {exam.type.charAt(0).toUpperCase() + exam.type.slice(1)}
                        </h4>
                      ...
                      {exam.score !== undefined ? (
                        <div className="flex items-center gap-4 mt-2">
                          <span className="text-sm">
                            Score: {exam.score}/{exam.maxScore} ({(exam.score / exam.maxScore * 100).toFixed(0)}%)
                          </span>
                          {exam.percentile && (
                            <Badge variant="secondary" className="text-xs">
        ...
        <TabsContent value="grades" className="space-y-4">
          {/* Report Cards */}
          {mockReportCards
            .filter(card => selectedChild === 'all' || card.childId === selectedChild)
            .map((card) => (
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {(children || []).map((child) => {
                  const childAttendance = mockAttendance.filter(a => a.childId === child.id)
                  ...
                  return (
                    <div key={child.id} className="p-4 border rounded-lg">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <p className="font-medium text-sm">{child.givenName} {child.surname}</p>
              ...
              <div className="space-y-2">
                ...
                {mockAttendance
                  .filter(a => selectedChild === 'all' || a.childId === selectedChild)
