...
export function ScheduleTab({
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'office': return <Clock className="h-3 w-3" />
      case 'meeting': return <Users className="h-3 w-3" />
      case 'exam': return <Calendar className="h-3 w-3" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold">{weeklyTeachingHours.toFixed(1)}</p>
          ...
          <div className="relative overflow-x-auto">
            ...
            <div className="flex">
              ...
              <div className="w-20 flex-shrink-0">
                ...
                {timeSlots.map(time => (
                  <div key={time} className="h-[60px] border-b text-xs text-muted-foreground p-2">
                    {time}
                  </div>
                ))}
              </div>
              ...
              <div className="flex-1 grid grid-cols-7 min-w-[700px]">
                {days.map((day, dayIndex) => {
                  ...
                  const isToday = isSameDay(addDays(weekStart, dayIndex), new Date())
                  ...
                  return (
                    <div key={day} className="border-l">
                      ...
                      <div className={cn(
                        ...
                        <p className="text-xs font-medium">{daysShort[dayIndex]}</p>
                      ...
                      <div className="relative">
                        ...
                        {daySchedule.map(slot => (
                          <div
                            key={slot.id}
                            className={cn(
                              'absolute left-0 right-0 mx-1 rounded-md p-1 text-white overflow-hidden',
                              slot.color
                            )}
                          ...
                          >
                            <div className="text-xs">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium">{slot.courseCode || slot.title}</span>
                              </div>
                              {slot.courseCode && (
                                <p className="text-[10px] opacity-90 truncate">{slot.title}</p>
                              )}
          ...
          <div className="space-y-3">
            {upcomingEvents.map(event => (
              <div key={event.id} className="flex items-start gap-4 p-3 border rounded-lg">
                <div className={cn('p-2 rounded-lg', getEventTypeColor(event.type))}>
