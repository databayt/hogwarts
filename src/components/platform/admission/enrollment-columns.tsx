...
const getOfferBadge = (
  ...
  if (row.admissionConfirmed) {
    return { label: t?.enrollment?.accepted || "Accepted", variant: "default" as const, icon: Check };
  }
  if (row.admissionOffered) {
    const isExpired = row.offerExpiryDate && new Date(row.offerExpiryDate) < new Date();
    if (isExpired) {
      return { label: t?.enrollment?.expired || "Expired", variant: "destructive" as const, icon: X };
    }
    return { label: t?.enrollment?.pending || "Pending", variant: "outline" as const, icon: Clock };
  }
  return { label: t?.enrollment?.pendingOffer || "Not Offered", variant: "secondary" as const, icon: null };
};
  ...
  return [
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title={t?.columns?.meritRank || "Rank"} />
      ),
      cell: ({ getValue }) => {
    ...
    {
      id: "offerStatus",
      accessorFn: (row) => row.admissionConfirmed ? "accepted" : row.admissionOffered ? "pending" : "not_offered",
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title={t?.columns?.offerStatus || "Offer"} />
      ),
      cell: ({ row }) => {
      ...
      meta: {
        ...
        options: [
          ...
          { label: t?.enrollment?.pendingOffer || "Not Offered", value: "not_offered" },
        ],
      },
      enableColumnFilter: true,
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title={t?.columns?.fees || "Fees"} />
      ),
      cell: ({ getValue }) => {
        const paid = getValue<boolean>();
        return (
          <Badge variant={paid ? "default" : "outline"}>
            {paid ? t?.enrollment?.paid || "Paid" : t?.enrollment?.unpaid || "Unpaid"}
          </Badge>
        );
      },
      meta: {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title={t?.columns?.documents || "Documents"} />
      ),
      cell: ({ getValue }) => {
        const has = getValue<boolean>();
        return (
          <Badge variant={has ? "default" : "outline"}>
            {has ? t?.enrollment?.verified || "Verified" : t?.enrollment?.inProgress || "Pending"}
          </Badge>
        );
      },
    },
