...
describe('tenant isolation and schedule resolution', () => {
  ...
  it('scopes weekly timetable query by schoolId', async () => {
    schoolWeekConfigFindFirst.mockResolvedValue({ workingDays: [0,1,2,3,4], defaultLunchAfterPeriod: 2 })
  ...
  it('scopes upsertTimetableSlot by schoolId and enforces role', async () => {
    ...
    const res = await upsertTimetableSlot({ termId: 't1', dayOfWeek: 0, periodId: 'p1', classId: 'c1', teacherId: 'tch1', classroomId: 'r1' })
    expect(timetableUpsert).toHaveBeenCalled()
    const call = timetableUpsert.mock.calls[0]?.[0]
    expect(call?.where?.schoolId_termId_dayOfWeek_periodId_classId_weekOffset?.schoolId).toBe('s1')

    // deny non-admin
    getTenantContext.mockResolvedValueOnce({ schoolId: 's1', role: 'TEACHER' })
    const denied = await upsertTimetableSlot({ termId: 't1', dayOfWeek: 0, periodId: 'p1', classId: 'c1', teacherId: 'tch1', classroomId: 'r1' })
    expect(denied instanceof Response).toBe(true)
  })

  it('resolves schedule config days and lunch correctly', async () => {
    schoolWeekConfigFindFirst.mockResolvedValue({ workingDays: [0,2,4], defaultLunchAfterPeriod: 3 })
    termFindFirst.mockResolvedValue({ yearId: 'y1' })
    periodFindMany.mockResolvedValue([
      { id: 'p1', name: 'P1', startTime: new Date('1970-01-01T07:00:00Z'), endTime: new Date('1970-01-01T07:45:00Z') },
      { id: 'p2', name: 'P2', startTime: new Date('1970-01-01T07:50:00Z'), endTime: new Date('1970-01-01T08:35:00Z') },
    ])
    timetableFindMany.mockResolvedValue([])
