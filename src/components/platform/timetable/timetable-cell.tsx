...
export function TimetableCell({
  ...
  if (isEmpty) {
    return (
      <div className={cn(
        "py-1 px-2 flex flex-col items-center justify-center transition-all duration-200 border-t border-border",
        index < 4 ? 'border-r' : '',
        "bg-muted",
        "print:hover:bg-white print:py-4",
        onSubjectChange ? "cursor-pointer hover:bg-muted/80" : ""
      )}
      onDoubleClick={handleDoubleClick}
      title={onSubjectChange ? "Double-click to edit subject" : ""}
      >
        <h6 className="h-14 xs:h-18 sm:h-23 text-foreground text-center print:text-base print:font-semibold">
          {subject}
        </h6>
      </div>
    )
        ...
        >
          <div className="w-full h-14 xs:h-18 sm:h-23 flex flex-col items-center justify-center overflow-hidden">
            ...
            {isReplaced && originalSubject && (
              <p className="text-destructive -mt-1 line-clamp-1">
                <small>(Replaced: {originalSubject})</small>
              </p>
            )}

            {info && (
              <p className="text-muted-foreground print:text-base print:mt-0 line-clamp-1">
