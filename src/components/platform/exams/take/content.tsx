...
export function ExamTakingContent({
  ...
  const handleAnswerChange = useCallback(
    (questionId: string, value: { answerText?: string; selectedOptionIds?: string[] }) => {
      setAnswers((prev) => {
        ...
        >
          <div className="space-y-3">
            {options.map((option, idx) => (
              ...
              >
                <RadioGroupItem value={option.id || idx.toString()} id={`option-${idx}`} />
    ...
    return (
      <div className="space-y-3">
        {options.map((option, idx) => {
          ...
          const isSelected = answer?.selectedOptionIds?.includes(optionId) || false;
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>{(answer?.answerText || "").split(/\s+/).filter(Boolean).length} words</span>
                    ...
                    {questions.map((q, idx) => {
                      ...
                      return (
                        <Button
                          ...
                          className={cn(
                            "h-10 w-10 p-0",
                            answered && !isCurrent && "bg-green-100 hover:bg-green-200 dark:bg-green-900"
                          )}
      ...
      <AlertDialog open={showTimeWarning} onOpenChange={setShowTimeWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You have 5 minutes left to complete and submit your exam. Make sure to
              review your answers before the time runs out.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
