...
export type StorageProvider = "vercel_blob" | "aws_s3" | "cloudflare_r2" | "imagekit";
...
export type FileCategory = "image" | "video" | "document" | "audio" | "archive" | "other";
...
export interface ImportConfig<T, TCreate = T> {
  ...
  updateRecord?: (id: string, data: Partial<TCreate>, schoolId: string) => Promise<void>;
  duplicateDetection?: {
    fields: (keyof T)[];
    findExisting: (criteria: Partial<T>, schoolId: string) => Promise<{ id: string } | null>;
  };
...
export interface UseExportReturn<T> {
  exportData: (data: T[], format: ExportFormat, config: ExportConfig<T>) => Promise<ExportResult>;
  isExporting: boolean;
  progress: number;
  error: string | null;
}
...
export interface UseImportReturn {
  importFile: (file: File, options?: Partial<ImportOptions>) => Promise<ImportResult>;
  previewFile: (file: File, format: ImportFormat) => Promise<ImportPreview | null>;
...
export interface UseGenerateReturn<TData> {
  generate: (data: TData, options?: Partial<GenerationOptions & TemplateOptions>) => Promise<GenerationResult>;
  preview: (data: TData, options?: Partial<TemplateOptions>) => Promise<string | null>;
