 ...
 */
export async function exportToExcel<T>(config: ExportConfig<T>): Promise<ExportResult> {
  try {
    const {
      filename,
      sheetName = "Data",
    ...
    const headerRowIndex = title || schoolName ? wsData.length - data.length - 1 : 0;
...
export async function exportToExcelMultiSheet<T>(
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Multi-sheet Excel export failed",
    };
  }
}
