...
import { Info, AlertTriangle, AlertCircle, Bell, BellRing } from "lucide-react";
...
export function ScopeStep({ form, isView, dictionary, lang }: ScopeStepProps) {
  ...
  const priorityOptions = [
    ...
    },
    {
      ...
      description: isRTL ? 'إشعار فوري للجميع' : 'Immediate notification to all',
      icon: BellRing,
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <FormField
        ...
        name="priority"
        render={({ field }) => {
          const selectedPriority = priorityOptions.find(p => p.value === field.value) || priorityOptions[1];
          ...
          return (
            <FormItem>
                        ...
                        {priorityOptions.map((option) => {
                          ...
                          return (
                            <div key={option.value} className="flex items-start gap-2">
                              <Badge variant="secondary" className={`${option.color} shrink-0`}>
              ...
              <Select onValueChange={field.onChange} value={field.value || 'normal'} disabled={isView}>
                <FormControl>
                  <SelectTrigger className="w-full">
                    <SelectValue>
                      <div className="flex items-center gap-2">
