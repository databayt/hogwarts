...
import { createLead, updateLead, getLeadById } from "@/components/sales/actions";
...
export function LeadForm({ dictionary, onSuccess }: LeadFormProps) {
  ...
  // Translations
  const isRTL = false; // TODO: Get from context
  const t = {
    title: isEditing
      ? isRTL
    ...
    title_field: isRTL ? "المسمى الوظيفي" : "Job Title",
    website: isRTL ? "الموقع الإلكتروني" : "Website",
    linkedin: isRTL ? "لينكدإن" : "LinkedIn",
    leadType: isRTL ? "نوع العميل" : "Lead Type",
    ...
    WEBSITE: isRTL ? "موقع ويب" : "Website",
    REFERRAL: isRTL ? "إحالة" : "Referral",
    SOCIAL_MEDIA: isRTL ? "وسائل التواصل" : "Social Media",
    EMAIL_CAMPAIGN: isRTL ? "حملة بريدية" : "Email Campaign",
    COLD_CALL: isRTL ? "اتصال بارد" : "Cold Call",
    ...
    PARTNERSHIP: isRTL ? "شراكة" : "Partnership",
    ...
    try {
      ...
      if (res?.success) {
        ...
        const errorMsg =
          ...
          (isEditing
            ? isRTL
              ? "فشل تحديث العميل المحتمل"
              : "Failed to update lead"
            : isRTL
            ? "فشل إنشاء العميل المحتمل"
