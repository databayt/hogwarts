...
import { Ellipsis, Building2, Mail, Phone, Star } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export const getLeadColumns = (
  ...
  return [
    ...
    },
    {
      accessorKey: "company",
      id: "company",
      header: ({ column }) => <DataTableColumnHeader column={column} title={t.company} />,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ getValue }) => (
        <span className="text-xs tabular-nums text-muted-foreground">
          {new Date(getValue<string>()).toLocaleDateString(isRTL ? "ar-SA" : "en-US")}
        </span>
      ),
