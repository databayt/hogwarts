...
export function PasteImport({ onComplete, dictionary }: PasteImportProps) {
  ...
  const detectFields = useCallback(
    (text: string) => {
      setProgress({
        ...
        total: 100,
        status: "detecting",
        message: d?.analyzingData || "Analyzing data...",
      })
      ...
      if (multipleLeads.length > 1) {
        fields.push({
          ...
          confidence: 0.9,
          sampleValues: [
            `${multipleLeads.length} ${d?.entriesDetected || "entries detected"}`,
          ],
        })
      }
  ...
  const handleImport = useCallback(async () => {
    ...
    setProgress({
      ...
      total: 100,
      status: "validating",
      message: d?.validatingData || "Validating data...",
    })
    ...
    try {
      ...
      setProgress({
        ...
        total: 100,
        status: "validating",
        message: d?.checkingDuplicates || "Checking for duplicates...",
      })
      ...
      setProgress({
        ...
        total: 100,
        status: "importing",
        message: d?.importingLeads || "Importing leads...",
      })
...
Example: John Doe - CEO - john@example.com`
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm">
                    {d?.detectedFields || "Detected Fields"}
                  </CardTitle>
                </div>
              </CardHeader>
              <CardContent>
          ...
          {validationErrors.length > 0 && (
            <Alert variant="destructive">
              ...
              <AlertTitle>
                {d?.validationIssues || "Validation Issues"}
              </AlertTitle>
              <AlertDescription>
        ...
        <CardFooter className="flex justify-between">
          <p className="text-muted-foreground text-sm">
            ...
            {d?.linesDetected || "lines detected"}
          </p>
          <Button onClick={handleImport} disabled={!isReady} className="gap-2">
            {isProcessing ? (
              <>
