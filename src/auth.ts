...
/**
 * Helper: Construct environment-aware school URL on the SAME root domain the
 * request came from — a login on demo.balqalam.com must never bounce the
 * user to demo.databayt.org (session cookies are scoped per root).
 * Development: http://{subdomain}.localhost:3000{path}
 * Production: https://{subdomain}.{root-of-currentHost}{path}
 */
function constructSchoolUrl(
  ...
  return `${tenantOriginForHost(currentHost, subdomain)}${path}`
}
...
// Host → tenant / main-domain classification is shared with src/proxy.ts and
// the auth client components via src/lib/root-domain.ts so every root domain
// (databayt.org, balqalam.com) behaves identically.
const detectSubdomainFromHost = getSubdomainFromHost
const isMainDomain = isMainDomainHost
...
async function getSmartRedirectUrl(
  ...
  try {
    ...
    // Regular user → look up school subdomain (kept on the current root domain)
    ...
    if (schoolDomain) {
      ...
      return constructSchoolUrl(schoolDomain, `/${locale}/dashboard`, host)
    }
...
/**
 * Resolve the request host for cookie scoping. Prefers the lazy-init request
 * (auth API routes), falls back to next/headers (server actions like
 * signIn), then null (build/module scope → primary-root behavior).
 */
async function resolveRequestHost(
  ...
  try {
    ...
    // Outside request scope (build time, module init)
    return null
  }
}

/**
 * Build the NextAuth options for one request (Auth.js v5 lazy
 * initialization). The cookie Domain attribute must follow the root domain
 * the request arrived on — .databayt.org cookies on databayt hosts,
 * .balqalam.com cookies on balqalam hosts. A static domain would make
 * browsers silently reject the session cookie on every other root.
 */
function buildAuthOptions(cookieDomain: string | undefined): NextAuthConfig {
  return {
     ...
     *
     * CRITICAL: cookieDomain is derived per request from the root domain the
     * request arrived on (.databayt.org / .balqalam.com — see
     * src/lib/root-domain.ts), so cookies are shared across all subdomains of
     * THAT root (school1.databayt.org ↔ ed.databayt.org,
     * demo.balqalam.com ↔ balqalam.com) and sessions never leak across roots.
     *
     * Without this, users would need to re-authenticate for each subdomain.
     * In development (and on custom domains) it is undefined → host-only cookie.
     */
    cookies: {
      pkceCodeVerifier: {
    ...
    callbacks: {
      ...
      async session({ session, token, trigger }) {
        ...
        if (token) {
          ...
          try {
            ...
            const effectiveRole = (session.user as any).role as
              | string
              | undefined
      ...
      async redirect({ url, baseUrl }) {
        ...
        if (subdomain) {
          const locale = extractLocaleFromUrl(url)
          return constructSchoolUrl(subdomain, `/${locale}/dashboard`, host)
        }
        ...
        if (isMainDomain(host)) {
          ...
          if (tenant) {
            const locale = extractLocaleFromUrl(url)
            return constructSchoolUrl(tenant, `/${locale}/dashboard`, host)
          }
