  ...
  callbacks: {
    ...
    async redirect({ url, baseUrl }) {
      ...
      if (!callbackUrl) {
        try {
          ...
          if (oauthCallbackCookie) {
            ...
            const oauthRelatedCookies = allCookies.filter(
              (c) =>
                c.name.toLowerCase().includes("oauth") ||
                c.name.toLowerCase().includes("callback")
            )
            log(
              "🔍 OAuth-related cookies found:",
              oauthRelatedCookies.length > 0 ? oauthRelatedCookies : "NONE"
            )
          }
        } catch (error) {
      ...
      try {
        ...
        if (!callbackUrl && typeof document !== "undefined") {
          try {
            ...
            const callbackCookie = cookies.find((cookie) =>
              cookie.trim().startsWith("authjs.callback-url=")
            )
            if (callbackCookie) {
            ...
            if (!callbackUrl) {
              const oauthCookie = cookies.find((cookie) =>
                cookie.trim().startsWith("oauth_callback_intended=")
              )
              if (oauthCookie) {
                callbackUrl = decodeURIComponent(oauthCookie.split("=")[1])
                log("🔍 Method 4b - OAuth intended cookie callback:", {
                  callbackUrl,
                })
                // Clear the cookie after use
        ...
        if (
          ...
          try {
            const intendedCallback = window.sessionStorage.getItem(
            ...
            log("💾 Session Storage Check:", {
              ...
              allValues: allStorageKeys.reduce(
                (acc, key) => {
            ...
            if (intendedCallback) {
              ...
              log("🔍 Method 5 - session storage intended callback:", {
                callbackUrl,
              })
              // Clear it after use
            ...
            log(
              "❌ Error reading intended callback from session storage:",
              error
            )
          }
        }
      ...
      try {
        ...
        if (
          originalHost.endsWith(".databayt.org") &&
        ...
        if (originalHost.includes("ed.databayt.org")) {
          ...
          if (callbackUrl) {
            log(
              "✅ Using callback URL on main domain (safety check):",
              callbackUrl
            )
            // Continue to validate and use the callback URL below
          } else {
      ...
      if (originalHost === "ed.databayt.org") {
        ...
        if (callbackUrl) {
          log(
            "✅ Using callback URL on main domain (final check):",
            callbackUrl
          )
          // Continue to validate and use the callback URL below
        } else {
      ...
      if (tenant) {
        ...
        log("🔄 Redirecting to tenant via parameter:", {
          tenant,
          tenantUrl,
          originalUrl: url,
        })
      ...
      log("Current state:", {
        ...
        isSameOrigin: url.startsWith("http")
      ...
      try {
        ...
        if (token) {
          ...
          if (!userSchoolId || userRole === "DEVELOPER") {
            ...
            console.log(
              "[AUTH-REDIRECT] 👑 Platform admin detected - redirecting to main domain:",
              {
          ...
          console.log(
            "[AUTH-REDIRECT] 👤 Regular user detected - looking up school domain for schoolId:",
            userSchoolId
          )
          ...
          if (schoolDomain) {
            ...
            const schoolUrl = constructSchoolUrl(
              schoolDomain,
              `/${locale}/dashboard`
            )

            console.log(
              "[AUTH-REDIRECT] ✅ School subdomain redirect SUCCESS:",
              {
                schoolId: userSchoolId,
                schoolDomain,
                locale,
            ...
            console.error(
              "[AUTH-REDIRECT] ⚠️ School domain NOT FOUND for schoolId:",
              userSchoolId
            )
          ...
          console.error(
            "[AUTH-REDIRECT] ⚠️ No token found - user might not be authenticated yet"
          )
