  ...
  trackEvent(event: MonitoringEvent) {
    try {
      ...
      if (typeof window !== "undefined" && Sentry.addBreadcrumb) {
        ...
        const sentryLevel =
          event.severity === "critical" ? "error" : event.severity || "info"
...
const monitoringServiceInstance =
  typeof window === "undefined" ? new MonitoringService() : null
...
export const trackApiCall = (
  ...
  if (typeof window === "undefined" && monitoringServiceInstance) {
    monitoringServiceInstance.trackApiCall(
      method,
      path,
      statusCode,
      duration,
