...
class PerformanceMonitor {
  ...
  endTimer(name: string, context?: Record<string, any>): number {
    ...
    if (!startTime) {
      logger.warn('Timer not found for operation', { action: 'timer_not_found', name });
      return 0;
    }
  ...
  async trackApiCall<T>(
    ...
    try {
      ...
      logger.error('API call failed', error instanceof Error ? error : new Error('Unknown API error'), {
        action: 'api_error',
        endpoint,
        duration,
        ...context
  ...
  trackPageLoad(pageName: string): void {
    ...
    try {
      ...
      if (navigation) {
        this.recordMetric(`page_load_${pageName}`, navigation.loadEventEnd - navigation.fetchStart, 'ms', {
          type: 'page_load',
          pageName,
      ...
      logger.error('Failed to track page load performance', error instanceof Error ? error : new Error('Unknown page load error'), {
   ...
   */
  getSummary(timeWindow: number = 300000): { // Default 5 minutes
...
export const endTimer = (name: string, context?: Record<string, any>) => performanceMonitor.endTimer(name, context);
...
export const trackQuery = <T>(name: string, queryFn: () => Promise<T>, context?: Record<string, any>) =>
  performanceMonitor.trackQuery(name, queryFn, context);
export const trackApiCall = <T>(endpoint: string, apiFn: () => Promise<T>, context?: Record<string, any>) =>
