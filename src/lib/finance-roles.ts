...
export const financePermissions: Record<
  string,
  Record<FinanceModule, FinanceAction[]>
> = {
 ...
 */
export async function filterFinanceDataByRole<
  T extends { userId?: string; studentId?: string; teacherId?: string },
>(data: T[], module: FinanceModule): Promise<T[]> {
  const session = await auth()
  if (!session?.user) return []
