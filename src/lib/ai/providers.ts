...
export function selectProvider(task: TaskType, priority: Priority = "cost") {
  // Check if priority override should be applied
  if (priority !== "cost") {
    const override = priorityOverrides[priority]
    try {
      const provider =
        providers[override.preferredProvider as keyof typeof providers]
      return provider[override.preferredTier as keyof typeof provider]
    } catch {
      console.warn(
        `Priority provider failed, falling back to task-based selection`
      )
    }
  }
  ...
  try {
    ...
    const fallbackProvider =
      providers[strategy.fallback.provider as keyof typeof providers]
    return fallbackProvider[
      strategy.fallback.model as keyof typeof fallbackProvider
    ]
  }
}
