...
function parseElements(
  html: string
): Array<{ tag: string; classes: string[]; content: string }> {
 ...
 */
function hasHardcodedTypography(classes: string[]): {
  found: boolean
  ...
  classes.forEach((cls) => {
    if (
      HARDCODED_TEXT_SIZES.includes(cls) ||
...
function suggestSemanticElement(classes: string[]): string {
  ...
  if (classes.includes("text-xl") && !hasFontWeight)
    return 'Use <p className="lead"> instead'
    ...
    if (classes.includes("text-left") || classes.includes("text-right")) {
      violations.push({
        ...
        classes: classes.filter(
          (cls) => cls === "text-left" || cls === "text-right"
        ),
