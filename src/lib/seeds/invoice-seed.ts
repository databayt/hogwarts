  ...
  try {
    ...
    const invoiceTemplates = [
      ...
      },
      {
        ...
        tax_percentage: 5,
        notes:
          "Multiple payment methods available. Contact finance office for assistance.",
      },
      // Overdue invoices
      {
        ...
        tax_percentage: 10,
        notes:
          "URGENT: This invoice is overdue. Please contact the finance office immediately.",
      },
      {
        status: InvoiceStatus.OVERDUE,
      ...
      },
      {
        ...
        tax_percentage: 0,
        notes:
          "Please settle this overdue amount to avoid service interruption.",
      },
      // Small paid invoices
      {
        status: InvoiceStatus.PAID,
    ...
    for (const template of invoiceTemplates) {
      ...
      const taxAmount = template.tax_percentage
        ? (subTotal - discountAmount) * (template.tax_percentage / 100)
        : 0
      ...
      console.log(
        `  ✓ Created invoice ${invoice.invoice_no} - ${template.status} - $${total.toFixed(2)}`
      )
    }
...
export async function main() {
  try {
    ...
    if (!userId || !schoolId) {
      throw new Error(
        "Please provide SEED_USER_ID and SEED_SCHOOL_ID environment variables"
      )
    }
