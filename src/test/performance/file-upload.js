...
const schoolIds = new SharedArray("schools", function () {
  return ["school-001", "school-002", "school-003", "school-004", "school-005"]
})
...
export function uploadChunkedFile(data) {
  ...
  group("Chunked File Upload", () => {
    ...
    for (let i = 0; i < totalChunks; i++) {
      const chunkData = generateFile(
        Math.min(chunkSize, totalSize - i * chunkSize)
      )

      const formData = new FormData()
      formData.append(
        "chunk",
        http.file(chunkData, `chunk-${i}`, "application/octet-stream")
      )
...
export function uploadBurst(data) {
  ...
  const response = http.post(`${BASE_URL}/api/upload/single`, formData.body(), {
    headers: {
      Authorization: `Bearer ${token}`,
      ...formData.headers(),
    },
