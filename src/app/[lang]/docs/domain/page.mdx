...
- Dev (no subdomains): add `?x-school=khartoum` to any page, e.g. `/dashboard?x-school=khartoum`.
- Dev (with local subdomains): set `NEXT_PUBLIC_ROOT_DOMAIN=localhost` and visit `http://khartoum.localhost:3000/dashboard`.
- Prod: visit the subdomain directly, e.g., `https://khartoum.hogwarts.app/dashboard`.
...
- In production: navigate to the school’s subdomain (e.g., `khartoum.hogwarts.app/operator/tenant-debug`).
