...
Create a shortcut to start Claude Code with bypass permissions mode using just `c`:
...
### Command Prompt Alias (cmd.exe)

Create a batch file `c.bat` in a directory that's in your PATH (e.g., `C:\Windows\System32` or a custom bin folder):
...
The `--dangerously-skip-permissions` flag bypasses all permission prompts. Use this alias only when:

- You trust the project you're working on
- You understand the operations Claude will perform
- You're in a controlled development environment

For production or sensitive projects, use the full command without the bypass flag.
