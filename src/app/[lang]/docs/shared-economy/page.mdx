...
- Anti-spam: Reviews <50 words or just "LGTM" = 0 points
...
- Require verified identity (GitHub account >6 months, verified email)
...
- If 2+ contributors only review each other (>80% overlap), flag for human review
...
- If total contributor earnings < local minimum wage equivalent → alert
- If profit margin > 40% while contributors earn <$30/hour → redistribution
...
- Git commit → hash → Ethereum/Polygon timestamp (low cost: <$0.01/commit)
...
- No single contributor can have >10% voting power (decentralization)
