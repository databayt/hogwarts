...
export default async function CommunityQbankPage({
  ...
  const [options, items] = await Promise.all([
    getCommunityFilterOptions(),
    getCommunityQuestions(filters),
  ])

  const title = dictionary?.community?.types?.qbank?.label ?? "Question Bank"
      ...
      <div className="mt-8">
        {items.length === 0 ? (
          <CommunityEmptyState
            ...
            resetHref={`/${lang}/community/qbank`}
            hasFilters={hasFilters}
          />
        ) : (
          <CommunityResourceGrid type="qbank" items={items} />
        )}
      </div>
    </div>
  )
