...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const label = dictionary?.community?.types?.books?.label ?? "Library Books"
  return { title: label }
}
...
export default async function CommunityBooksPage({
  ...
  const [options, items] = await Promise.all([
    getCommunityFilterOptions(),
    getCommunityBooks(filters),
  ])

  const title = dictionary?.community?.types?.books?.label ?? "Library Books"
      ...
      <div className="mt-8">
        {items.length === 0 ? (
          <CommunityEmptyState
            ...
            resetHref={`/${lang}/community/books`}
            hasFilters={hasFilters}
          />
        ) : (
          <CommunityResourceGrid type="books" items={items} />
        )}
      </div>
    </div>
  )
