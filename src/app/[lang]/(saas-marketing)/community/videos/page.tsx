...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const label = dictionary?.community?.types?.videos?.label ?? "Videos"
  return { title: label }
}
...
export default async function CommunityVideosPage({
  ...
  const [options, items] = await Promise.all([
    getCommunityFilterOptions(),
    getCommunityVideos(filters),
  ])

  const title = dictionary?.community?.types?.videos?.label ?? "Videos"
      ...
      <div className="mt-8">
        {items.length === 0 ? (
          <CommunityEmptyState
            ...
            resetHref={`/${lang}/community/videos`}
            hasFilters={hasFilters}
          />
        ) : (
          <CommunityResourceGrid type="videos" items={items} />
        )}
      </div>
    </div>
  )
