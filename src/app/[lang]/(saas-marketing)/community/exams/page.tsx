...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const label = dictionary?.community?.types?.exams?.label ?? "Mock Exams"
  return { title: label }
}
...
export default async function CommunityExamsPage({
  ...
  const [options, items] = await Promise.all([
    getCommunityFilterOptions(),
    getCommunityExams(filters),
  ])

  const title = dictionary?.community?.types?.exams?.label ?? "Mock Exams"
      ...
      <div className="mt-8">
        {items.length === 0 ? (
          <CommunityEmptyState
            ...
            resetHref={`/${lang}/community/exams`}
            hasFilters={hasFilters}
          />
        ) : (
          <CommunityResourceGrid type="exams" items={items} />
        )}
      </div>
    </div>
  )
