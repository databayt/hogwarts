...
export async function updateAnnouncement(
  input: Partial<z.infer<typeof announcementUpdateSchema>> & { id: string }
) {
  const { schoolId } = await getTenantContext()
  if (!schoolId) throw new Error("Missing school context")
    ...
    .superRefine((val, ctx) => {
      if (val.scope === "class" && !val.classId)
        ctx.addIssue({
          ...
          path: ["classId"],
          message: "classId required for class scope",
        })
      if (val.scope === "role" && !val.role)
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["role"],
          message: "role required for role scope",
        })
    })
  ...
  await db.announcement.updateMany({
    where: { id, schoolId },
    data: data as unknown as Prisma.AnnouncementUpdateManyMutationInput,
  })
  revalidatePath("/lab/announcements")
  return { success: true as const }
}
