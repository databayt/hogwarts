...
import { AnnouncementConfigForm } from "@/components/platform/announcements/config-form"
import {
...
export default async function AnnouncementsConfigPage({ params }: Props) {
  ...
  if (!configResult.success) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-2">{configResult.error}</p>
      </div>
    )
  }
  ...
  const configDictionary = {
    ...
    trackingRetention: lang === 'ar' ? 'التتبع والاحتفاظ' : 'Tracking & Retention',
    ...
    autoPublishDesc: lang === 'ar' ? 'نشر الإعلانات الجديدة تلقائياً' : 'Automatically publish new announcements',
    defaultExpiryDays: lang === 'ar' ? 'مدة الانتهاء (أيام)' : 'Default Expiry (days)',
    emailOnPublish: lang === 'ar' ? 'البريد عند النشر' : 'Email on Publish',
    emailOnPublishDesc: lang === 'ar' ? 'إرسال إشعار بريدي عند النشر' : 'Send email notifications when published',
    pushNotifications: lang === 'ar' ? 'إشعارات الدفع' : 'Push Notifications',
    pushNotificationsDesc: lang === 'ar' ? 'إرسال إشعارات دفع (قريباً)' : 'Send push notifications (coming soon)',
    ...
    allowCustomTemplates: lang === 'ar' ? 'السماح بالقوالب المخصصة' : 'Allow Custom Templates',
    allowCustomTemplatesDesc: lang === 'ar' ? 'السماح للمستخدمين بإنشاء قوالب مخصصة' : 'Let users create custom announcement templates',
    readTracking: lang === 'ar' ? 'تتبع القراءة' : 'Read Tracking',
    readTrackingDesc: lang === 'ar' ? 'تتبع قراءة المستخدمين للإعلانات' : 'Track when users read announcements',
    retentionDays: lang === 'ar' ? 'فترة الاحتفاظ (أيام)' : 'Retention Period (days)',
    autoArchive: lang === 'ar' ? 'الأرشفة التلقائية' : 'Auto-archive',
    autoArchiveDesc: lang === 'ar' ? 'أرشفة الإعلانات المنتهية تلقائياً' : 'Automatically archive expired announcements',
    archiveAfterDays: lang === 'ar' ? 'الأرشفة بعد (أيام)' : 'Archive After (days)',
    ...
    saveFailed: lang === 'ar' ? 'فشل في حفظ الإعدادات' : 'Failed to save settings',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="p-3 bg-muted rounded-lg">
          <AnthropicIcons.Gear className="h-6 w-6 text-muted-foreground" />
        </div>
        <div>
