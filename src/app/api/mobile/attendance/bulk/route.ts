...
export async function POST(request: NextRequest) {
  try {
    ...
    if (records.length > MAX_BULK_SIZE) {
      return NextResponse.json(
        { error: `bulk limit is ${MAX_BULK_SIZE} records` },
        { status: 400 }
      )
    }

    // Pre-validate every studentId belongs to this tenant before any write.
    // The previous code accepted body-supplied student_ids and inserted them
    // with auth.schoolId, so a teacher in school A could mark school B's
    // students PRESENT in school A's ledger.
