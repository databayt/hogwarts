...
test.describe("Authentication Flows", () => {
  test.describe("Join/Register Page", () => {
    test("should display registration form with all fields", async ({
      ...
      // Check submit button
      await expect(
        page.getByRole("button", { name: /sign up|join/i })
      ).toBeVisible()
    ...
    test("should register successfully with valid credentials", async ({
      ...
      await expect(
        page.getByText(/confirmation email sent|email sent|success/i)
    ...
    test("should show error for existing email", async ({ page }) => {
      ...
      await expect(
        page.getByText(/email already in use|already exists|taken/i)
      ).toBeVisible({ timeout: 10000 })
    })
  ...
  test.describe("Login Page", () => {
    ...
    test("should show error for non-existent email", async ({ page }) => {
      ...
      // Should show error message
      await expect(
        page.getByText(/email does not exist|not found|invalid/i)
      ).toBeVisible({ timeout: 10000 })
    })
  ...
  test.describe("Password Reset Flow", () => {
    ...
    test("should show error for non-existent email", async ({ page }) => {
      ...
      // Should show error message
      await expect(
        page.getByText(/email not found|does not exist|not found/i)
      ).toBeVisible({ timeout: 10000 })
    })
    ...
    test("should send reset email for valid user", async ({ page }) => {
      ...
      await expect(
        page.getByText(/reset email sent|email sent|success/i)
      ).toBeVisible({ timeout: 10000 })
    })
  ...
  test.describe("OAuth Buttons", () => {
    ...
    test("should display Facebook OAuth button on login page", async ({
      page,
    }) => {
      await page.goto(getUrl("/login"))
    ...
    test.skip("should initiate Google OAuth flow on click", async ({
      page,
    }) => {
      await page.goto(getUrl("/login"))
      ...
      await page.waitForURL(/accounts\.google\.com|api\/auth/, {
        timeout: 10000,
      })
    })
  })
  ...
  test.describe("Email Verification Page", () => {
    test("should display verification page with invalid token", async ({
      page,
    }) => {
      ...
      await expect(
        page.getByText(/token does not exist|invalid|expired/i)
      ).toBeVisible({ timeout: 10000 })
    })
